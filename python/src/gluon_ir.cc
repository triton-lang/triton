...
void init_gluon_ir(py::module &&m) {
  ...
  py::class_<GluonOpBuilder, TritonOpBuilder>(
      ...
      .def("create_buffer_atomic_rmw",
           [](GluonOpBuilder &self, tt::RMWOp op, Value ptr, Value offsets, Value mask,
              Value value, tt::MemSemantic sem, tt::MemSyncScope scope, Value stride) -> Value {
             return self.create<ttag::BufferAtomicRMWOp>(value.getType(), op, ptr, offsets, value,
                                                         stride, sem, scope, mask);
         })
