...
py::list getTensorDescMetadata(ModuleOp &mod) {
  ...
  for (auto [i, arg] : llvm::enumerate(kernelFunc.getArguments())) {
    ...
    py::dict metadata;
    if (encoding && isa<ttg::NVMMASharedEncodingAttr>(encoding)) {
      ...
      if (encoding && isa<ttg::PaddedSharedEncodingAttr>(encoding)) {
        auto paddedEnc = cast<ttg::PaddedSharedEncodingAttr>(encoding);
