...
void init_plugin_passes(py::module &&m) {
  ...
  for (unsigned i = 0; i < passNames.size(); ++i) {
    ...
    m.def(passName, [passName](mlir ::PassManager &pm, int num_warps, int threadsPerWarp, int numCTAs) {
      std::string filename =
          mlir::triton::tools::getStrEnv("TRITON_PASS_PLUGIN_PATH");
      TritonPlugin TP(filename);
      if (auto result = TP.addPass(&pm, passName, num_warps, threadsPerWarp, numCTAs); !result)
        throw TP.err2exp(result.takeError());
    });
  }
}
