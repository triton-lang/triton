...
void init_triton_llvm(py::module_ &m) {
  ...
  m.def(
      ...
      [](llvm::Module *mod, const llvm::OptimizationLevel &opt,
         ...
         unsigned simplify_vector_elts_depth) {
        if (mlir::triton::tools::getBoolEnv("DISABLE_LLVM_OPT"))
          return;
        ...
        options.enableFlagsFromDisableLLVMOptEnv();
        options.enablePrintAfterAllIfRequested();
        // How deep SimplifyDemandedVectorElts walks an extract/insert chain
        // before giving up. 0 keeps LLVM's default.
        if (simplify_vector_elts_depth != 0)
          options.set("instcombine-simplify-vector-elts-depth",
                      std::to_string(simplify_vector_elts_depth));
        ScopedLLVMOptions optionScope(options.settings);
        using namespace llvm;
      ...
      py::arg("simplify_vector_elts_depth") = 0,
      py::call_guard<py::gil_scoped_release>());
