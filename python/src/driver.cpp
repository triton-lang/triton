...
struct model_map_indexing
{
    ...
    {
        isc::expression_type expression = detail::extract_template_type(i_[0]);
        isc::numeric_type dtype = detail::extract_dtype(i_[1]);
...
bp::tuple flush(isc::array_expression const & expression, unsigned int queue_id, bp::list dependencies, bool tune, int label, std::string const & program_name, bool force_recompile)
{
    ...
    std::vector<isc::driver::Event> cdependencies = detail::to_vector<isc::driver::Event>(dependencies);
