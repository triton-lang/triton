...
The producer, scale packing, and TMA epilogue are shared with example 04.
The five typed issue operations below express the two crossings between
producer slots; each slot is released after its last consuming instruction.
Grouped tile traversal preserves L2 reuse. Static persistence wins the smaller
problems; the largest NVFP4 case retains cluster launch control.

Run a same-input comparison against the original native K96 implementation::

    python 07-pure-k96-matmul.py --size 16384 --k 16128 --format nvfp4 \
        --modes native --compare-native --output /tmp/dense-k96
...
@gluon.jit
def _copy_scales(p, idx, sa, sb, OFFSET: gl.constexpr, VEC: gl.constexpr):
...
def _mma_partition(p):
    ...
    while scheduler.has_work:
        ...
        for k in range(0, K, 768):
            ...
            _copy_scales(p, s0.index, sa, sb, 0, VEC)
            tcgen05_mma_scaled(a0, b0, acc, sa, sb, "e2m1", "e2m1", use_acc=use_acc, k_range=(0, 192), instruction_k=96,
                               scale_block_size=VEC, a_scale_offset=0, b_scale_offset=0, multicast=True, mbarriers=[],
                               is_async=True)
            mbarrier.wait(p.load_ready_bars.index(s1.index), s1.phase)
            _copy_scales(p, s1.index, sa, sb, 256 // VEC, VEC)
            ...
            _copy_scales(p, s2.index, sa, sb, 512 // VEC, VEC)
...
def matmul(A, B, A_scale, B_scale, vec, *, buffers=None, epilogue=None, scheduler=None, out_dtype=torch.float16,
    ...
    if scheduler is None:
        scheduler = SCHEDULER_CLC if vec == 16 and K > 16384 else SCHEDULER_SPS
    ...
    compiled = dense_k96_kernel[grid](ad, bd, cd, asd, bsd, M, N, K, 2, buffers, 256, 256, 256, epilogue, 1, 0,
                                      tile_width, ((1, 0), ), scheduler, num_ctas=2)
