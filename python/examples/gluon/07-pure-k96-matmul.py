...
uses exact K768 descriptor views when the data ring groups into threes. Other
ring depths use continuation operands to handle wraparound. Both paths retain
K256 producer readiness and release points; loading a whole K768 tile at once
reduces overlap. NVFP4 stages scales independently and drains the accumulator early.
...
#
# k_range selects each issue group within a native K768 descriptor when the
# ring groups into threes. The packed uint8 view has shape (256, 384), with no
# fourth sector allocated. Other ring depths select ranges within K256 views
# and use continuations for crossings, including ring wraparound. Scale offsets
# always refer to the macrotile's independently staged TMEM scale stream.
#
...
def mma_scaled_mma_partition(p, scale_empty_bars):
    ...
    PACK_MACROTILES: gl.constexpr = p.a_bufs.shape[0] % 3 == 0
    K1: gl.constexpr = 256 if PACK_MACROTILES else 0
    K2: gl.constexpr = 512 if PACK_MACROTILES else 0
    if PACK_MACROTILES:
        macro_state = Counter.create(0, p.a_bufs.shape[0] // 3)
        macro_layout: gl.constexpr = gl.NVMMASharedLayout(128, 8, rank=3, cga_layout=((0, 1, 0), ))
        a_macros = p.a_bufs.reinterpret(gl.uint8, [p.a_bufs.shape[0] // 3, 3, 256, 128], macro_layout)
        b_macros = p.b_bufs.reinterpret(gl.uint8, [p.b_bufs.shape[0] // 3, 3, 256, 128], macro_layout)
    ...
    while scheduler.has_work:
        ...
        for k in range(0, K, 768):
            ...
            data2 = data1.next()
            if PACK_MACROTILES:
                a0 = a_macros.index(macro_state.index).permute((1, 0, 2)).reshape((256, 384))
                b0 = b_macros.index(macro_state.index).permute((1, 0, 2)).reshape((256, 384)).permute((1, 0))
                a1, b1 = a0, b0
                a2, b2 = a0, b0
                a0_next, b0_next = None, None
                a1_next, b1_next = None, None
            else:
                ...
                a0_next, b0_next = a1, b1
                a1_next, b1_next = a2, b2
            ...
            tcgen05_mma_scaled(a0, b0, acc, a_scale_tmem, b_scale_tmem, "e2m1", "e2m1",
                               a_next=a0_next, b_next=b0_next, k_range=(192, 288), instruction_k=96,
            ...
            if SPLIT_SCALES:
                tcgen05_mma_scaled(a1, b1, acc, a_scale_tmem, b_scale_tmem, "e2m1", "e2m1", k_range=(K1 + 32, K1 + 128),
                                   instruction_k=96, scale_block_size=SCALE_BLOCK_SIZE,
                ...
                tcgen05_mma_scaled(a1, b1, acc, a_scale_tmem, b_scale_tmem, "e2m1", "e2m1", k_range=(K1 + 128, K1 + 224),
                                   instruction_k=96, scale_block_size=SCALE_BLOCK_SIZE,
                                   a_scale_offset=384 // SCALE_BLOCK_SIZE, b_scale_offset=384 // SCALE_BLOCK_SIZE,
                                   multicast=True, mbarriers=[], is_async=True)
            else:
                tcgen05_mma_scaled(a1, b1, acc, a_scale_tmem, b_scale_tmem, "e2m1", "e2m1", k_range=(K1 + 32, K1 + 224),
                                   instruction_k=96, scale_block_size=SCALE_BLOCK_SIZE,
                                   a_scale_offset=288 // SCALE_BLOCK_SIZE, b_scale_offset=288 // SCALE_BLOCK_SIZE,
                                   multicast=True, mbarriers=[], is_async=True)
            ...
            tcgen05_mma_scaled(a1, b1, acc, a_scale_tmem, b_scale_tmem, "e2m1", "e2m1",
                               a_next=a1_next, b_next=b1_next, k_range=(K1 + 224, K1 + 320), instruction_k=96,
                               scale_block_size=SCALE_BLOCK_SIZE,
            ...
            tcgen05_mma_scaled(a2, b2, acc, a_scale_tmem, b_scale_tmem, "e2m1", "e2m1", k_range=(K2 + 64, K2 + 256),
                               instruction_k=96, scale_block_size=SCALE_BLOCK_SIZE,
...
def dense_k96_kernel(a_desc, b_desc, c_desc, a_scale_desc, b_scale_desc, M, N, K, A_ELEM_PER_BYTE,
    ...
    if num_buffers % 3 == 0:
        # Three complete swizzle sectors form an exact K768 allocation. The
        # producer indexes K256 pages; the consumer also sees whole macrotiles.
        macro_layout: gl.constexpr = gl.NVMMASharedLayout(128, 8, rank=3, cga_layout=((0, 1, 0), ))
        a_storage = gl.allocate_shared_memory(a_desc.dtype, [num_buffers // 3, 3, BLOCK_M, BLOCK_K // 2], macro_layout)
        b_storage = gl.allocate_shared_memory(b_desc.dtype, [num_buffers // 3, 3, BLOCK_N, BLOCK_K // 2], macro_layout)
        a_bufs = a_storage.reinterpret(a_desc.dtype, [num_buffers] + a_desc.block_shape, a_desc.layout)
        b_bufs = b_storage.reinterpret(b_desc.dtype, [num_buffers] + b_desc.block_shape, b_desc.layout)
    else:
...
@pytest.mark.parametrize("buffers", [None, 3])
def test_matmul(format, out_dtype, buffers):
    torch.manual_seed(0)
    A, B, A_scale, B_scale, C_ref = make_problem(512, 512, 2304, format)
    C = matmul(A, B, A_scale, B_scale, out_dtype=out_dtype, buffers=buffers)
    # Compare before output rounding: independent BF16 rounding can straddle a midpoint.
    rtol = max(1e-3, torch.finfo(out_dtype).eps / 2)
    torch.testing.assert_close(C.float(), C_ref, atol=2e-3, rtol=rtol)
