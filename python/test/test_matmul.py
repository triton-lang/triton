...
@pytest.mark.parametrize(
    ...
    itertools.chain(
        *[
            ...
            ] for DTYPE in ["float16"] for AT in [False] for BT in [False]
        ]
    ),
)
def test_op(BLOCK_M, BLOCK_N, BLOCK_K, SPLIT_K, NWARP, M, N, K, AT, BT, DTYPE):
    ...
    #no_autotune = triton.autotune(configs, [])(triton.ops._matmul.kernel)
    #triton.ops._matmul.kernel.kernel_decorators[0] = no_autotune
    # get matrix shape
