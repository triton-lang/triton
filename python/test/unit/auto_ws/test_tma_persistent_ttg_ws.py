...
def _test_experimental_matmul(
    ...
    # XXX: Need to clear hook, otherwise this test fail with smem check
    ...
    # NOTE: When using ttg.warp_specialize, it results in additional shared memory allocation.
    #       As ttg.warp_specialize is a work-in-progress, we skip the check for now
    #       and reduce the number of stages in the unit test to avoid running out of shared memory.
