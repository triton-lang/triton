...
def test_config_utils() -> None:
    ...
    assert instance.bar == False
    assert instance.baz == None
    assert instance.quux == True
    assert instance.knobs == {
        "foo": "triton",
        "bar": False,
        "baz": None,
    ...
    assert second.bar == False
    assert second.baz == None
    assert second.quux == True
...
def test_read_env(truthy_falsey, fresh_config, monkeypatch):
    ...
    assert fresh_config.runtime.debug == False
    # bool defaulting to True
    assert fresh_config.language.default_fp_fusion == True
    # str defaulting to None
    assert fresh_config.compilation.use_ir_loc == None
    # str defaulting to not None
    assert fresh_config.cache.dir.endswith(".triton/cache")
    # class defaulting to None
    assert fresh_config.cache.manager_class == None
    ...
    assert fresh_config.runtime.debug == True
    assert fresh_config.language.default_fp_fusion == False
...
def test_set_config_directly(fresh_config, monkeypatch):
    ...
    assert fresh_config.language.default_fp_fusion == False
    assert fresh_config.amd.use_block_pingpong == True
    ...
    assert fresh_config.language.default_fp_fusion == True
    assert fresh_config.amd.use_block_pingpong == False
...
def test_opt_bool(fresh_config, monkeypatch):
    assert fresh_config.amd.use_block_pingpong == None
    monkeypatch.setenv("TRITON_HIP_USE_BLOCK_PINGPONG", "0")
    assert fresh_config.amd.use_block_pingpong == False
