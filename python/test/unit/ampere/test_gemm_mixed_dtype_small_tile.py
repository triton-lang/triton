...
@pytest.mark.parametrize('M,N,K,block_m,block_n,block_k,dtype_a,dtype_b,mode', [
    (256, 256, 256, block_m, block_n, block_k, dtype_a, dtype_b, '16bx8b')
...
] + [(256, 256, 256, block_m, block_n, block_k, dtype_a, dtype_b, '8bx16b')
     for block_m in [16, 32, 64, 128]  # block size m
     for block_n in [16, 32, 64, 128]  # block size n
     for block_k in [16, 32]  # block size k
...
def test_gemm_mixed_dtype_small_tile_16bx8b(M, N, K, block_m, block_n, block_k, dtype_a, dtype_b, mode):
    match mode:
        case '16bx8b':
            ...
            matmul_kernel_16bx8b[(triton.cdiv(M, block_m), triton.cdiv(N, block_n))](lhs, rhs, out, M=M, N=N, K=K,
                                                                                     block_m=block_m, block_n=block_n,
                                                                                     block_k=block_k, dtype_a=dtype_a,
                                                                                     dtype_b=dtype_b, mode=mode)
