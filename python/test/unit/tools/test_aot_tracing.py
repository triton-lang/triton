...
def test_signature(dtypes, hints, constants, expected_signature):
    ...
    assert (signature == expected_signature), f"Expected signature: {expected_signature}, Actual signature: {signature}"


## Test wrapper for `triton.tools.compile` for generating reference kernels
def test_kernel_compilation(tmp_path):
    AOTScriptRunner.compile_matmul_kernels("matmul", NO_HINT_SIGNATURE, 1, "M/16, N/16, 1", out_dir=tmp_path)
...
def check_codegen(actual: str, expected: str, skip: List[str] = None, verbose=False):
    ...
    assert (not mismatches), f'Mismatch in generated code at lines {", ".join(str(l) for l in mismatches)}'


def check_linked_source(actual: str, expected: str, skip: List[str] = None, verbose=False):
    ...
    for i in range(len(actual_lines)):
        ...
        if actual_line.lstrip().startswith("if") and "return" in actual_lines[i + 1]:
            if verbose:
                print(f"Checking dispatch condition:\nActual: {actual_line}\n{actual_lines[i+1]}")
...
class TestMatMulCodegen:
    ...
    def test_dir(self, configs):
        test_dir = (Path(__file__).parent / "matmul_codegen_test" / "_".join(cfg for cfg in configs)).absolute()
        if test_dir.exists():
    ...
    @pytest.fixture(scope="class")
    def expected_kernels(self, kernel_name, reference_dir: Path, kernel_configs, kernel_path: Path):
        signatures = [
            AOTScriptRunner.generate_signature(kernel_config.dtypes, kernel_config.hints, kernel_config.constants)
    ...
    def test_aot_compiler_params(
        ...
        for actual in actual_params:
            ...
            expected = [p for p in expected_kernels.compiler_params if kernel_sig in str(p)][0]
            expected = json.load(expected.open())
            for k in actual.keys():
                assert (
                    actual[k] == expected[k]), f"{k.upper()} not equal\n\tExpected: {expected[k]}, Actual: {actual[k]}"
    ...
    def test_aot_codegen_kernel_headers(
        ...
        for actual, expected in zip(sorted(actual_headers), sorted(expected_kernels.kernel_headers)):
    ...
    def test_aot_codegen_kernel_sources(
        ...
        for actual, expected in zip(sorted(actual_sources), sorted(expected_kernels.kernel_sources)):
            expected = expected.read_text()
            check_codegen(actual, expected)
...
class TestMatMulTrace:
    ...
    @pytest.fixture(scope="class")
    def test_dir(self, configs):
        test_dir = (Path(__file__).parent / "matmul_trace_test" / "_".join(cfg for cfg in configs)).absolute()
        if test_dir.exists():
            shutil.rmtree(test_dir)
        test_dir.mkdir(parents=True, exist_ok=True)
    ...
    def traced_kernels(
        ...
        for kernel_config in kernel_configs:
            ...
            do_not_specialize = [k for k in kernel_config.hints if kernel_config.hints[k] is None]
        ...
        traces, *_ = matmul_tracer.trace(kernel_configs=matmul_configs, trace_configs=trace_configs)
        # Save jit args
        for trace in traces:
            ...
            with open(trace_dir / f"{trace.kernel_name}-compiler_params.json", "w") as fp:
                json.dump({k: str(v) for k, v in trace._compiler_params.items()}, fp, indent=2)
        return traces
    ...
    def test_kernel_header_match(self, traced_kernels, expected_kernels):
        for trace in traced_kernels:
            ...
            expected_header = [h for h in expected_kernels.kernel_headers if sig_hash in str(h)][0].read_text()
    ...
    def test_kernel_source_match(self, traced_kernels, expected_kernels):
        for trace in traced_kernels:
            sig_hash = self.extract_sig_hash(trace)
            expected_source = [s for s in expected_kernels.kernel_sources if sig_hash in str(s)][0].read_text()
