import pytest
import torch
from triton._internal_testing import is_hip, is_hip_cdna3, is_hip_cdna4


@pytest.mark.parametrize("m, n, k", [(16, 16, 16), (32, 16, 16), (16, 32, 16), (16, 16, 32)])
@pytest.mark.parametrize("dtype_str", ["float8_e4m3fnuz", "float16"])
def test_hipblas(m, n, k, dtype_str, device):
    dtype = getattr(torch, dtype_str)
    if not is_hip():
        pytest.skip("test_hipblas is only supported on HIP")
    if dtype == torch.float8_e4m3fnuz and not (is_hip_cdna3() or is_hip_cdna4()):
        pytest.skip("float8_e4m3fnuz is only supported on HIP CDNA3 or CDNA4")
