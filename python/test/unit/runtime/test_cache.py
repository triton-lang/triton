...
def test_annotation(device):
    ...
    kernel[(1, )](x, 16)
    kernel[(1, )](x, 17)
    assert len(kernel.device_cache[device][0]) == 3
...
def test_kernel_default_arg(device):
    ...
    device = getattr(torch, device).current_device()
    assert len(kernel.device_cache[device][0]) == 1
...
def test_jit_warmup_cache(device) -> None:
    ...
    device = getattr(torch, device).current_device()
    assert len(kernel_add.device_cache[device][0]) == 0
    kernel_add.warmup(torch.float32, torch.float32, torch.float32, 32, grid=(1, ))
    assert len(kernel_add.device_cache[device][0]) == 1
...
def test_jit_debug(device) -> None:
    ...
    assert len(kernel.device_cache[device][0]) == 0
    kernel[(1, )](tmp, debug=False)
    assert len(kernel.device_cache[device][0]) == 1
    kernel[(1, )](tmp, debug=True)
    assert len(kernel.device_cache[device][0]) == 2
    bins = list(kernel.device_cache[device][0].values())
...
def test_jit_noinline(device) -> None:
    ...
    device = getattr(torch, device).current_device()
    assert len(kernel_add_device.device_cache[device][0]) == 0
    kernel_add_device.warmup(torch.float32, torch.float32, torch.float32, 32, grid=(1, ))
    assert len(kernel_add_device.device_cache[device][0]) == 1
    bins = list(kernel_add_device.device_cache[device][0].values())
    inline_ttir = bins[0].asm['ttir']
    add_fn.noinline = True
    add_fn.hash = None
    kernel_add_device.hash = None
    kernel_add_device.device_cache[device][0].clear()
    kernel_add_device.warmup(torch.float32, torch.float32, torch.float32, 32, grid=(1, ))
    assert len(kernel_add_device.device_cache[device][0]) == 1
    bins = list(kernel_add_device.device_cache[device][0].values())
...
def test_preload(device, fresh_triton_cache) -> None:
    ...
    kernel_add.device_cache[device][0].clear()
...
def test_hooks(device, fresh_triton_cache) -> None:
    ...
    assert key in kernel_add.device_cache[getattr(torch, device).current_device()][0]
