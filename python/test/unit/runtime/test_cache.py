...
def test_reuse(device, fresh_triton_cache):
    ...
    config.runtime.jit_cache_hook = inc_counter
...
def test_preload(device, fresh_triton_cache) -> None:
    ...
    config.runtime.jit_cache_hook = cache_hook
    ...
    config.runtime.jit_cache_hook = None
    assert counter == 0
    assert len(kernel_add.device_caches[device][0]) == 1
...
def test_hooks(device, fresh_triton_cache) -> None:
    ...
    config.runtime.jit_cache_hook = cache_hook
    config.runtime.jit_post_compile_hook = compiled_hook
...
def test_function_arguments(device):
    ...
    config.runtime.jit_cache_hook = None
    config.runtime.jit_post_compile_hook = None
