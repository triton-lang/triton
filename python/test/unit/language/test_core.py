...
def test_histogram_broadcast(M, N, device):
    ...
    @triton.jit
    def histogram_kernel_broadcast(x_ptr, z_ptr, M: tl.constexpr, N: tl.constexpr):
        offset1 = tl.arange(0, M)
        offset2 = tl.arange(0, N)
        x = tl.load(x_ptr + offset1)
        z = tl.histogram(x, N)
        ...
        biased = z + bias
        tl.store(z_ptr + offset1[:, None] + offset2[None, :], biased)

    torch.manual_seed(17)
    x = torch.randint(0, N, (M, ), device=device, dtype=torch.int32)
    z = torch.empty(N, dtype=torch.int32, device=device)
    z_torch = torch.histc(x.float(), bins=N, min=0, max=N - 1)
    histogram_kernel_broadcast[(1, )](x, z, M=M, N=N)
    assert (z_torch == z).all()
