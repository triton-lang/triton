...
def _loop_single_pass_reference(case, n, initial):
    x = y = initial
    if case == 0:
        for i in range(n):
            for j in range(3):
...
@pytest.mark.parametrize('kind', ['triton', 'gluon'])
@pytest.mark.parametrize('case', range(5))
@pytest.mark.parametrize('n', [0, 1, 5])
def test_loop_single_pass_values(kind, case, n, device):
    from triton.experimental import gluon
    from triton.experimental.gluon import language as gl

    language = tl if kind == 'triton' else gl
    jit = triton.jit if kind == 'triton' else gluon.jit

    @jit
    def kernel(In, Out, n, CASE: language.constexpr):
        x = language.load(In)
        y = x
        if CASE == 0:
            for i in range(n):
            ...
            for i in range(n):
                ...
                x = y
                y += 1
        language.store(Out, x)
        language.store(Out + 1, y)

    initial = torch.tensor([2], dtype=torch.int32, device=device)
    out = torch.full((2, ), -1, dtype=torch.int32, device=device)
    kernel[(1, )](initial, out, n, case)
    assert out.tolist() == _loop_single_pass_reference(case, n, 2)


@pytest.mark.parametrize('kind', ['triton', 'gluon'])
@pytest.mark.parametrize('use_while', [False, True])
def test_loop_single_pass_rejects_changed_type(kind, use_while, device):
    from triton.experimental import gluon
    from triton.experimental.gluon import language as gl
    ...
    @jit
    def kernel(In, Out, n, WHILE: language.constexpr):
        x = language.load(In)
        if WHILE:
            i = 0
            while i < n:
    ...
    value = torch.tensor([2], dtype=torch.int32, device=device)
    with pytest.raises(triton.CompilationError, match='Loop-carried variable x'):
        kernel[(1, )](value, value, 3, use_while)
