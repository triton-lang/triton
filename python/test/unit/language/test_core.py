...
def test_convert2d(M, N, src_layout, interm_layout, dst_layout, dtype, device, tmp_path: pathlib.Path):
    ...
    %13 = ttg.convert_layout %11 : tensor<{M}x{N}x{mlir_dtype}, #src> -> tensor<{M}x{N}x{mlir_dtype}, #dst>
    """ if interm_layout is None else f"""
    ...
    %17 = ttg.local_alloc %11 : (tensor<{M}x{N}x{mlir_dtype}, #src>) -> !ttg.memdesc<{M}x{N}x{mlir_dtype}, #interm, #smem>
    %18 = ttg.local_load %17 : !ttg.memdesc<{M}x{N}x{mlir_dtype}, #interm, #smem> -> tensor<{M}x{N}x{mlir_dtype}, #src>

    %12 = ttg.convert_layout %16 : tensor<{M}x{N}xi32, #src> -> tensor<{M}x{N}xi32, #dst>
    %13 = ttg.convert_layout %18 : tensor<{M}x{N}x{mlir_dtype}, #src> -> tensor<{M}x{N}x{mlir_dtype}, #dst>
    """

    ir = layouts + f"""
    module attributes {{"ttg.num-warps" = 4 : i32, "ttg.num-ctas" = 1 : i32, "ttg.threads-per-warp" = {THREADS_PER_WARP} : i32}} {{
  tt.func public @kernel_0d1d(%arg0: !tt.ptr<{mlir_dtype}> {{tt.divisibility = 16 : i32}}, %arg1: !tt.ptr<{mlir_dtype}> {{tt.divisibility = 16 : i32}}) {{
    %cst = arith.constant dense<{N}> : tensor<{M}x1xi32, #src>
    ...
    %2 = tt.splat %arg0 : !tt.ptr<{mlir_dtype}> -> tensor<{M}x{N}x!tt.ptr<{mlir_dtype}>, #src>
    ...
    %10 = tt.addptr %2, %9 : tensor<{M}x{N}x!tt.ptr<{mlir_dtype}>, #src>, tensor<{M}x{N}xi32, #src>
    %11 = tt.load %10 : tensor<{M}x{N}x!tt.ptr<{mlir_dtype}>, #src>
    %3 = tt.splat %arg1 : !tt.ptr<{mlir_dtype}> -> tensor<{M}x{N}x!tt.ptr<{mlir_dtype}>, #dst>
    """ + conversion + f"""
    %14 = tt.addptr %3, %12 : tensor<{M}x{N}x!tt.ptr<{mlir_dtype}>, #dst>, tensor<{M}x{N}xi32, #dst>
    tt.store %14, %13 : tensor<{M}x{N}x!tt.ptr<{mlir_dtype}>, #dst>
    tt.return
  }}
}}
"""
...
layouts_3d = [
    ...
    DotOperandLayout(parent=MmaLayout([2, 0], [4, 1, 1], [1, 1, 1], [1, 1, 1], [2, 1, 0], [1, 16, 8]), op_idx=0,
                     k_width=4),
    DotOperandLayout(parent=MmaLayout([2, 0], [4, 1, 1], [1, 1, 1], [1, 1, 1], [2, 1, 0], [1, 16, 8]), op_idx=1,
                     k_width=4),
    DotOperandLayout(parent=MmaLayout([2, 0], [4, 1, 1], [1, 1, 1], [1, 1, 1], [2, 1, 0], [1, 16, 8]), op_idx=0,
                     k_width=2),
    DotOperandLayout(parent=MmaLayout([2, 0], [4, 1, 1], [1, 1, 1], [1, 1, 1], [2, 1, 0], [1, 16, 8]), op_idx=1,
...
def test_local_load_store(M, N, K, dtype, dist_layout, shared_layout, device, tmp_path: pathlib.Path):
    if dtype == "float16":
        mlir_dtype = "f16"
    elif dtype == "float8e5":
        mlir_dtype = "f8e5"
  ...
  tt.func public @kernel(%arg0: !tt.ptr<{mlir_dtype}> {{tt.divisibility = 16 : i32}}, %arg1: !tt.ptr<{mlir_dtype}> {{tt.divisibility = 16 : i32}}) attributes {{noinline = false}} {{
    ...
    %3 = tt.splat %arg0 : !tt.ptr<{mlir_dtype}> -> tensor<1x1x{K}x!tt.ptr<{mlir_dtype}>, #dist>
    %4 = tt.addptr %3, %2 : tensor<1x1x{K}x!tt.ptr<{mlir_dtype}>, #dist>, tensor<1x1x{K}xi32, #dist>
    ...
    %9 = tt.broadcast %4 : tensor<1x1x{K}x!tt.ptr<{mlir_dtype}>, #dist> -> tensor<1x{N}x{K}x!tt.ptr<{mlir_dtype}>, #dist>
    %10 = tt.broadcast %8 : tensor<1x{N}x1xi32, #dist> -> tensor<1x{N}x{K}xi32, #dist>
    %11 = tt.addptr %9, %10 : tensor<1x{N}x{K}x!tt.ptr<{mlir_dtype}>, #dist>, tensor<1x{N}x{K}xi32, #dist>
    ...
    %16 = tt.broadcast %11 : tensor<1x{N}x{K}x!tt.ptr<{mlir_dtype}>, #dist> -> tensor<{M}x{N}x{K}x!tt.ptr<{mlir_dtype}>, #dist>
    %17 = tt.broadcast %15 : tensor<{M}x1x1xi32, #dist> -> tensor<{M}x{N}x{K}xi32, #dist>
    %18 = tt.addptr %16, %17 : tensor<{M}x{N}x{K}x!tt.ptr<{mlir_dtype}>, #dist>, tensor<{M}x{N}x{K}xi32, #dist>
    %19 = tt.load %18 : tensor<{M}x{N}x{K}x!tt.ptr<{mlir_dtype}>, #dist>
    %20 = ttg.local_alloc %19 : (tensor<{M}x{N}x{K}x{mlir_dtype}, #dist>) -> !ttg.memdesc<{M}x{N}x{K}x{mlir_dtype}, #shared, #smem>
    %21 = ttg.local_load %20 : !ttg.memdesc<{M}x{N}x{K}x{mlir_dtype}, #shared, #smem> -> tensor<{M}x{N}x{K}x{mlir_dtype}, #dist>
    ...
    %25 = tt.splat %arg1 : !tt.ptr<{mlir_dtype}> -> tensor<1x1x{K}x!tt.ptr<{mlir_dtype}>, #dist>
    %26 = tt.addptr %25, %24 : tensor<1x1x{K}x!tt.ptr<{mlir_dtype}>, #dist>, tensor<1x1x{K}xi32, #dist>
    ...
    %31 = tt.broadcast %26 : tensor<1x1x{K}x!tt.ptr<{mlir_dtype}>, #dist> -> tensor<1x{N}x{K}x!tt.ptr<{mlir_dtype}>, #dist>
    %32 = tt.broadcast %30 : tensor<1x{N}x1xi32, #dist> -> tensor<1x{N}x{K}xi32, #dist>
    %33 = tt.addptr %31, %32 : tensor<1x{N}x{K}x!tt.ptr<{mlir_dtype}>, #dist>, tensor<1x{N}x{K}xi32, #dist>
    ...
    %38 = tt.broadcast %33 : tensor<1x{N}x{K}x!tt.ptr<{mlir_dtype}>, #dist> -> tensor<{M}x{N}x{K}x!tt.ptr<{mlir_dtype}>, #dist>
    %39 = tt.broadcast %37 : tensor<{M}x1x1xi32, #dist> -> tensor<{M}x{N}x{K}xi32, #dist>
    %40 = tt.addptr %38, %39 : tensor<{M}x{N}x{K}x!tt.ptr<{mlir_dtype}>, #dist>, tensor<{M}x{N}x{K}xi32, #dist>
    tt.store %40, %21 : tensor<{M}x{N}x{K}x!tt.ptr<{mlir_dtype}>, #dist>
    tt.return
  }}
}}
"""
