...
@pytest.mark.parametrize("K", [None, 16])
@pytest.mark.parametrize("descending", [False, True])
@pytest.mark.parametrize("dtype_str", ['int32', 'float16', 'float32', 'bfloat16'])
def test_sort(M, N, k, descending, dtype_str, device):

    @triton.jit
    def sort_kernel(X, Z, N: tl.constexpr, M: tl.constexpr, k: tl.constexpr, descending: tl.constexpr):
        offx = tl.arange(0, M)
    ...
    x = numpy_random((N, k or M), dtype_str=dtype_str)
    ...
    sort_kernel[(1, )](x, z, N, M, k, descending, num_warps=8)
    assert (y == z).all(), (y, z)
