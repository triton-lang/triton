...
def test_complex_conditionals(device):
    ...
    def test_if_or(x_ptr, out_ptr):
        x = tl.load(x_ptr)
        if (
            ((x.dtype == tl.float32) or (x.dtype == tl.float16))
            and not (x.dtype == tl.float32)
        ) and (x.dtype == tl.float16 and x.dtype == tl.float32):
            tl.store(out_ptr, 0)
        else:
            tl.store(out_ptr, 1)
    ...
    out_ptr = torch.empty_like((1,), dtype=torch.int32, device=device)
