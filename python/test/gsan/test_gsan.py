...
def _shadow_cells_for_tensor(tensor: torch.Tensor) -> list[list[object]]:
    ...
    for row_idx in range(tensor.shape[0]):
        ...
        for col_idx in range(tensor.shape[1]):
            real_ptr = tensor[row_idx, col_idx].data_ptr()
...
def _gather_scatter_change_mask(storage: torch.Tensor, x_offsets: torch.Tensor, y_offset: int, m_size: int,
                                n_size: int, block_y: int) -> torch.Tensor:
    changed_mask = torch.zeros(storage.shape, dtype=torch.bool)
...
def test_host_tma_gather_updates_shadow(with_gsan):
    ...
    compiled = _host_tma_gather_kernel[(1, )](out, out.stride(0), out.stride(1), target_desc, x_offsets, y_offset, BLOCK_X=block_x)
