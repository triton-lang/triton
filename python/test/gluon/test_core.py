...
@pytest.mark.skipif(not is_blackwell(), reason="Requires Blackwell")
def test_tmem_unaligned_subslice_load_store():

    @gluon.jit
    def kernel(inp, out):
        parent = allocate_tensor_memory(ttgl.float32, [128, 512], TensorMemoryLayout([128, 128], col_stride=1))
        layout: ttgl.constexpr = parent.get_reg_layout()
        rows = ttgl.arange(0, 128, layout=ttgl.SliceLayout(1, layout))[:, None]
        cols = ttgl.arange(0, 512, layout=ttgl.SliceLayout(0, layout))[None, :]
        parent.store(ttgl.load(inp + rows * 512 + cols))

        view = parent.slice(208, 256)
        view.store(view.load() + 7.0)
        ttgl.store(out + rows * 512 + cols, parent.load(layout))

    inp = torch.arange(128 * 512, device="cuda", dtype=torch.int32).remainder(64).to(torch.float32).reshape(128, 512)
    out = torch.empty_like(inp)
...
def test_tmem_pipeline_stage_subslice_index_reinterpret():
    ...
    @gluon.jit
    def kernel(inp, out):
        layout: ttgl.constexpr = TensorMemoryLayout([128, 64], col_stride=1)
        parent = allocate_tensor_memory(ttgl.float32, [5, 128, 64], layout)
        reg_layout: ttgl.constexpr = parent.index(0).get_reg_layout()
        rows = ttgl.arange(0, 128, layout=ttgl.SliceLayout(1, reg_layout))[:, None]
        cols = ttgl.arange(0, 64, layout=ttgl.SliceLayout(0, reg_layout))[None, :]

        for stage in ttgl.static_range(5):
            view = parent.index(stage)
            view.store(ttgl.load(inp + stage * 8192 + rows * 64 + cols))

        stages = parent.slice(2, 2, dim=0)
        first = stages._reinterpret().index(0)
        first.store(first.load() + 11.0)
        last = stages.slice(1, 1, dim=0).index(0)
        last.store(last.load() + 7.0)

        for stage in ttgl.static_range(5):
            view = parent.index(stage)
            ttgl.store(out + stage * 8192 + rows * 64 + cols, view.load(reg_layout))

    inp = torch.arange(5 * 128 * 64, device="cuda", dtype=torch.int32).remainder(64).to(torch.float32).reshape(
        5, 128, 64)
    out = torch.empty_like(inp)
    kernel[(1, )](inp, out, num_warps=4)
    ref = inp.clone()
