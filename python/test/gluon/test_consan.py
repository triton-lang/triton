...
@pytest.mark.skipif(not is_cuda() or torch.cuda.get_device_capability()[0] < 9, reason="Requires hopper or newer")
def test_async_copy_mbarrier_arrive_noinc(device, run_wrapper, monkeypatch):
    if run_wrapper:
        result = run_in_process(test_async_copy_mbarrier_arrive_noinc, (device, False, monkeypatch))
        assert result.exc is None
        assert result.driver_stderr_output == ""
        return
    ...
    def kernel(inp):
        ...
        smem = ttgl.allocate_shared_memory(ttgl.float32, [1], ttgl.SwizzledSharedLayout(1, 1, 1, order=[0]))
        offs = ttgl.arange(0, 1, layout)
        bar = mbarrier.allocate_mbarrier()
        mbarrier.init(bar, count=129)
        ampere.async_copy.async_copy_global_to_shared(smem, inp + offs)
        ampere.async_copy.mbarrier_arrive(bar, increment_count=False)
        mbarrier.arrive(bar, count=1)
        mbarrier.wait(bar, phase=0)

    inp = torch.ones((1, ), device=device, dtype=torch.float32)
    kernel[(1, )](inp, num_warps=4, num_ctas=1)
...
def test_tcgen5_commit_multicast_barrier_count(OVERCOUNTED, device, run_wrapper, monkeypatch):
    ...
    def kernel(a_desc, b_desc, OVERCOUNTED: ttgl.constexpr):
        ...
        count: ttgl.constexpr = blackwell.tcgen05_mma_barrier_count([smem_a, smem_b], True)
        mbarrier.init(tma_bar, count=1)
