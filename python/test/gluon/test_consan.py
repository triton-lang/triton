...
@pytest.mark.skipif(not is_cuda() or torch.cuda.get_device_capability() != (10, 3), reason="Requires sm103 K96")
@pytest.mark.parametrize("vec", [16, 32])
@pytest.mark.parametrize("case", ["correct", "unused_data", "reuse"])
def test_tcgen05_mma_scaled_k96_non_power_of_two(vec, case, run_wrapper, monkeypatch):
    if run_wrapper:
        result = run_in_process(test_tcgen05_mma_scaled_k96_non_power_of_two, (vec, case, False, monkeypatch))
        if case == "reuse":
            assert_expected_cuda_failure(result.exc)
            assert "outstanding reads" in result.driver_stderr_output
        else:
    ...
    monkeypatch.setenv("CUDA_LAUNCH_BLOCKING", "1")
    knobs.refresh_knobs()
    from test_core import mma_scaled_tcgen05_npot, random_quantized_tensor
    fmt = "nvfp4" if vec == 16 else "mxfp4"
    a, sa, ar = random_quantized_tensor(256, 768, fmt)
    b, sb, br = random_quantized_tensor(256, 768, fmt)
    window = (192, 288) if case == "unused_data" else None
    selected = slice(*window) if window is not None else slice(None)
    expected = ar[:, selected] @ br[:, selected].T
    actual, _ = mma_scaled_tcgen05_npot(a, b, sa, sb, vec, window=window, overwrite=case != "correct")
