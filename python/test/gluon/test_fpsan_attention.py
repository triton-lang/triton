...
def attention_forward(q, k, v, causal, sm_scale, BLOCK_M, BLOCK_N):
...
def compare_bitwise_equal(ref_out, tri_out):
...
def test_attention_fwd(monkeypatch):
    device = "cuda"
    ...
    ref_out, _ = attention_forward(q, k, v, causal, sm_scale, BLOCK_M=256, BLOCK_N=128)
    tri_out, _ = attention_forward(q, k, v, causal, sm_scale, BLOCK_M=256, BLOCK_N=64)
    assert compare_bitwise_equal(ref_out, tri_out), "Reference and Triton outputs should be bitwise equal in fpsan mode"
