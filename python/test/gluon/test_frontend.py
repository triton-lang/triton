...
def amd_wmma_layout_kernel():
    ttgl.full([64, 64], 0, ttgl.float16, layout=amd_layouts.AMDWMMALayout(version=2, reg_bases=[], warp_bases=[[0, 1],
                                                                                                               [0, 2]],
                                                                          transposed=True))
    ttgl.full([64, 64], 0, ttgl.float16, layout=amd_layouts.AMDWMMALayout(version=2, transposed=True, reg_bases=[],
                                                                          warp_bases=[[0, 1], [1, 0]]))
    ttgl.full([64, 64], 0, ttgl.float16, layout=amd_layouts.AMDWMMALayout(version=2, transposed=False, reg_bases=[],
                                                                          warp_bases=[[0, 1], [0, 2]]))
    ttgl.full([64, 64], 0, ttgl.float16, layout=amd_layouts.AMDWMMALayout(version=2, transposed=False, reg_bases=[],
                                                                          warp_bases=[[0, 1], [1, 0]]))
...
def test_amd_wmma_layout(target):
    ...
    expecttest.assert_expected_inline(
        anonymize_ir(module.str_nodebug()), """\
#mma = #ttg.amd_wmma<{version = 2, isTranspose = true, warpLayout = {register = [], warp = [[0, 1], [0, 2]]}}>
#mma1 = #ttg.amd_wmma<{version = 2, isTranspose = true, warpLayout = {register = [], warp = [[0, 1], [1, 0]]}}>
#mma2 = #ttg.amd_wmma<{version = 2, isTranspose = false, warpLayout = {register = [], warp = [[0, 1], [0, 2]]}}>
#mma3 = #ttg.amd_wmma<{version = 2, isTranspose = false, warpLayout = {register = [], warp = [[0, 1], [1, 0]]}}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "...", "ttg.threads-per-warp" = 32 : i32} {
...
def infer_layout_for_amd_wmma_kernel():
    layout: ttgl.constexpr = amd_layouts.AMDWMMALayout(version=2, transposed=True, reg_bases=[], warp_bases=[[1, 0],
                                                                                                             [2, 0]])
...
def test_infer_layout_for_amd_wmma(target):
    ...
    expecttest.assert_expected_inline(
        anonymize_ir(module.str_nodebug()), """\
#mma = #ttg.amd_wmma<{version = 2, isTranspose = true, warpLayout = {register = [], warp = [[1, 0], [2, 0]]}}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "...", "ttg.threads-per-warp" = 32 : i32} {
...
def test_amd_rdna3_wmma(target):
    ...
    @gluon.jit
    def kernel():
        wmma_layout: ttgl.constexpr = ttgl.amd.AMDWMMALayout(version=1, transposed=True, reg_bases=[],
                                                             warp_bases=[[1, 0], [2, 0]])
    ...
    expecttest.assert_expected_inline(
        anonymize_ir(module.str_nodebug()), """\
#mma = #ttg.amd_wmma<{version = 1, isTranspose = true, warpLayout = {register = [], warp = [[1, 0], [2, 0]]}}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "...", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @kernel() attributes {noinline = false} {
    %cst = arith.constant 1.000000e+00 : f16
...
def test_amd_rdna4_wmma(target):
    ...
    @gluon.jit
    def kernel():
        wmma_layout: ttgl.constexpr = ttgl.amd.AMDWMMALayout(version=2, transposed=True, reg_bases=[],
                                                             warp_bases=[[1, 0], [2, 0]])
...
def test_amd_wmma_scaled(target):
    ...
    def kernel():
        ...
        wmma_layout_packed: ttgl.constexpr = ttgl.amd.AMDWMMALayout(version=3, transposed=True, reg_bases=[],
                                                                    warp_bases=[[0, 1], [1, 0]], instr_shape=[16, 16, 64])
...
#mma = #ttg.amd_wmma<{version = 3, isTranspose = true, warpLayout = {register = [], warp = [[0, 1], [1, 0]]}, instrShape = [16, 16, 64]}>
#mma1 = #ttg.amd_wmma<{version = 3, isTranspose = true, warpLayout = {register = [], warp = [[0, 1], [1, 0]]}, instrShape = [16, 16, 128]}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "...", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @kernel() attributes {noinline = false} {
    %c17_i8 = arith.constant 17 : i8
...
#mma = #ttg.amd_wmma<{version = 3, isTranspose = true, warpLayout = {register = [], warp = []}, instrShape = [16, 16, 64]}>
#mma1 = #ttg.amd_wmma<{version = 3, isTranspose = true, warpLayout = {register = [], warp = []}, instrShape = [16, 16, 128]}>
