...
square_confs = [
    ...
    for AT in [False]
    for BT in [False]
    for op_mode in ["sdd", "dsd", "dds"]
    for layout_mode in ["tril", "dense"]
]


@triton.testing.perf_report(square_confs)
def bench_matmul(
    M, N, K, block, layout_mode, op_mode, AT, BT, dtype, provider, warmup=5, rep=5
):
    Z, H = 1, 1
    make_layout = {
    ...
    shape = {
        "sdd": (M, N),
        "dsd": (K, M) if AT else (M, K),
        "dds": (N, K) if BT else (K, N),
    }[op_mode]
    ...
    if provider == "triton":
        op = triton.ops.blocksparse.matmul(
            layout, block, op_mode, trans_a=AT, trans_b=BT
        )
        # inputs
        ...
        mean_ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: op(a, b), warmup=warmup, rep=rep
        )
        num_flops = {
...
def bench_softmax(M, N, block, layout_mode, dtype, provider, warmup=10, rep=50):
    ...
    inputs = torch.randn((Z, H, M, N), dtype=dtype, device="cuda")
    if provider == "triton":
        inputs = triton.testing.sparsify_tensor(inputs, layout, block)
        op = triton.ops.blocksparse.BlocksparseSoftmax(layout, block)
        gbps = lambda ms: (2 * inputs.numel() * inputs.element_size() * 1e-9) / (
            ms * 1e-3
        )
        mean_ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: op(inputs), warmup=warmup, rep=rep
        )
