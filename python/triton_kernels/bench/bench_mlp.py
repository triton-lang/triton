...
def bench_mlp(batch, dim1, dim2, n_expts_tot, n_expts_act, x_dtype, w_dtype, TP, EP, name):
    ...
    if w_dtype == "mx4" and not is_hip():
        is_hopper = torch.cuda.get_device_capability()[0] < 10
        opt1 = {
            "swizzle_mx_value": SwizzlingType.HOPPER if is_hopper else None, "swizzle_mx_scale":
            SwizzlingType.HOPPER if is_hopper else SwizzlingType.BLACKWELL
        }
        opt2 = deepcopy(opt1)
