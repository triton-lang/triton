...
from .layout_details.blackwell_scale import BlackwellActMXScaleLayout, BlackwellMXScaleLayout
...
def make_default_matmul_mxfp_w_scale_layout(mx_axis: int, num_warps: int = 8):
    if is_hip_cdna4():
...
def make_default_matmul_mxfp4_w_scale_layout(mx_axis: int, num_warps: int = 8):
    return make_default_matmul_mxfp_w_scale_layout(mx_axis, num_warps)


def make_default_matmul_mxfp8_act_scale_layout(ex_hist: torch.Tensor | None = None):
    if cuda_capability_geq(10):
        return BlackwellActMXScaleLayout, {"ex_hist": ex_hist}
    return StridedLayout, dict()
