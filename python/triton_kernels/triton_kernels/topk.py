...
def topk_forward(
    ...
    s = tensor_sharding.range_for_rank(rank)
    start_row, end_row = s.tolist()
    n_rows = end_row - start_row

    n_rows_out_max, n_cols = x.local_shape_max
    ...
    pids = cdiv(n_rows, BLOCK_M)
    _topk_forward[(pids, )](
        x.storage.data, x.storage.data.stride(0),  # inputs
        ...
        start_row, BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N,  # tunable parameter
    ...
    bitmatrix = wrap_torch_tensor(
        ...
        dtype=BIT,
        shape=x.shape[:],
        local_shape_max=x.local_shape_max[:],
    )
...
class TopK(torch.autograd.Function):
    ...
    def forward(
        ...
        if all_gather:
            ...
            x_triton = extend_sharding(x_triton, dim=0, sharding=RangeSharding(mesh=symm_mem_pool.mesh))
