...
class Storage:
    ...
    def is_tma_compliant(self):
        ...
        try:
            ...
            major_dim = -1
        ndim = self.data.ndim
        # TODO: for scale, bitwidth should be 8 even with torch.uint8
    ...
    def make_dense_tma(self, block_shape, is_scale: bool, transpose: bool | None = None):
        strides = list(self.data.stride())
        shape = list(self.data.shape)
        if transpose is None:
            # This will be False for Blackwell swizzled scale tensors
    ...
    def make_tma(self, block_shape, mode, is_scale: bool, transpose: bool | None = None):
        if mode in ["dense", "gather", "scatter"]:
            return self.make_dense_tma(block_shape, is_scale, transpose)
        assert mode == "ragged"
