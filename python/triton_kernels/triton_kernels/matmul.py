...
def matmul(a, b, bias,
    ...
    print(
        ...
        b.storage.layout.name,
        None if b_scale is None else b_scale.storage.layout.name,
        opt_flags.epilogue_subtile,
        opt_flags.split_k,
        even_K,
        opt_flags.w_cache_modifier,
        None if a_ragged_metadata is None else a_ragged_metadata.expected_slice_size,
        opt_flags.num_warps,
        opt_flags.num_stages,
        opt_flags.arch,
        should_upcast_indices(a, b, out_matmul),
        a_tma_mode,
        c_tma_mode,
        get_swap_xw(precision_config, opt_flags),
        epilogue.specs.name == FnName.QUANTIZE_MXFP8.name,
    ...
    if opt_flags.split_k > 1:
        ...
        c, y_mx_scale = reduce(
            ...
            c = memory["output"].view(-1, memory["output"].shape[-1]),
