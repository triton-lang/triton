...
def matmul_ogs(x, w, bias,
    ...
    if x_scale_has_tma:
    # temporary limitation for x scale tma: only support act scale layout is BlackwellActMXScaleLayout and no gather case
        if has_gather:
    ...
    if x_scale_has_tma:
        ...
        # Even in ragged case, each ragged chunk has been padded so we can just use dense tma here
        x_scale_tensor_or_tma = x_scale_storage.make_tma(x_scale_tma_block_size, "dense", is_scale=True)
    else:
        x_scale_tensor_or_tma = x_scale
    
    # # create a x_scale_output tensor to store the output of the x_scale tma
    # x_scale_output_torch = torch.zeros((4*128, 16), device=x_scale.device, dtype=x_scale.dtype)
    # x_scale_output = wrap_torch_tensor(x_scale_output_torch.view(math.prod(x_scale_output_torch.shape[:-1]), x_scale_output_torch.shape[-1]))
    # x_scale_output_storage = _canonicalize_storage(x_scale_output.storage, 2, None)
    # print("x_scale_output_storage", x_scale_output_storage)
    # x_scale_output_tma_block_size = [opt_flags.block_m, scale_block_k]
    # x_scale_output_tensor_or_tma = x_scale_output_storage.make_tma(x_scale_output_tma_block_size, "dense", is_scale=True)
