...
def matmul_ogs(x, w, bias,
    ...
    if w_has_mx: assert w.mT.is_contiguous(), "`w` must be column-major when it has data-type mxfp"
    if is_hopper_fp8: assert w.mT.is_contiguous(), "`w` must be column-major when it has data-type FP8 on capability < 10"
