...
def init_allocation(x, w, precision_config, fused_activation,
    ...
    if opt_flags.split_k > 1 or (scatter_indx is not None and routing_data.n_expts_act > 1):
...
def matmul_ogs(x, w, bias,
    ...
    if scatter_indx is not None and routing_data.n_expts_act > 1: # Matmul ogs kernel fuses scatter already, so only need for n_exps_act > 1.
