...
@pytest.mark.parametrize("num_warps", [4, 8])
@pytest.mark.parametrize("k_groups", [90, 94])
def test_mxfp4_scale_k_tail_padding(k_groups, num_warps):
    # Hopper matmul kernels read scale storage for a whole BLOCK_K tile
    # (block_k = 128 for mxfp4, i.e. 4 scale groups). The swizzled storage
    # must therefore zero-pad scale-K to a multiple of 4 or the last tile can
    # read past the allocation (#10772: K = 2880 -> 90 groups, previously
    # padded to 90, tile reads 92).
    n = 256
