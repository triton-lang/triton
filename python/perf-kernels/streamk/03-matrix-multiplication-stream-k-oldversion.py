...
def mac_loop(
    ...
    A = A + rm[:, None] * stride_am + rk[None, :] * stride_ak + BLOCK_K * stride_ak * (mod1)
    B = B + rk[:, None] * stride_bk + rn[None, :] * stride_bn + BLOCK_K * stride_bk * (mod1)
...
#    if mod2 == 0:# last iteration of the tile always happens before its start on another SM
...
#        if mod1 != 0:  # only if tile has been partially processed
...
def first_wave(
    ...
    while start_iter < last_iter:
        ...
        mod1 = start_iter % iters_per_tile
        mod2 = end_iter % iters_per_tile
        tile_id = start_iter // iters_per_tile
        if GROUP_M > 0:
...
triton_ms = triton.testing.do_bench(
    lambda: matmul.apply(A, B, total_sm, BLK_M, BLK_N, BLK_K, two_tiles, num_stages, num_warps))
print(f"hybrid stream-k (grid={total_sm}): {triton_ms:.3f} ms  {perf(triton_ms):.3f} tflops")

triton_ms = triton.testing.do_bench(
    lambda: matmul.apply(A, B, total_sm * 2, BLK_M, BLK_N, BLK_K, two_tiles, num_stages, num_warps))
print(f"hybrid stream-k (grid={total_sm * 2}): {triton_ms:.3f} ms  {perf(triton_ms):.3f} tflops")

triton_ms = triton.testing.do_bench(
    lambda: matmul.apply(A, B, 0, BLK_M, BLK_N, BLK_K, two_tiles, num_stages, num_warps))
