...
def get_tile_config(M, N, K,
                BLOCK_M: tl.constexpr, BLOCK_N: tl.constexpr, BLOCK_K: tl.constexpr,
                    two_tiles, total_programs_streamk):
...
#        total_tiles_streamk, 
    ...
    pid = tl.program_id(0)
    iters_per_tile, total_tiles_streamk, total_full_tiles_streamk, total_partial_tiles_streamk, total_iters_streamk = get_tile_config(M, N, K, BLOCK_M, BLOCK_N, BLOCK_K, two_tiles, total_programs_streamk)
...
for idx, (m, n, k) in enumerate(shapes):
    ...
    for two_tiles in [True, False]:
        ...
        for sm in nb_sm:
            triton_ms = triton.testing.do_bench(lambda: wrapper_matmul(A, B, sm, BLOCK_M, BLOCK_N, BLOCK_K, two_tiles, num_stages, num_warps, waves_per_eu))
