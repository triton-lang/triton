...
from ._layouts import AutoLayout, DistributedLayout, DistributedLinearLayout, SliceLayout, SharedLayout, EfficientLayout
...
class GluonSemantic(TritonSemantic[TensorTy]):
    ...
    def expand_dims(self, input: TensorTy, axis: int) -> TensorTy:
        ...
        _check(isinstance(layout, (SliceLayout, AutoLayout, EfficientLayout)),
               lambda: f"expected expand_dims input to have a SliceLayout, but got: {layout}")
        _check(
            isinstance(layout, (AutoLayout, EfficientLayout)) or layout.dim == axis,
