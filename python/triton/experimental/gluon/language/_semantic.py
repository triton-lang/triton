...
class GluonSemantic(TritonSemantic[TensorTy]):
    ...
    def make_tensor_descriptor(self, base: TensorTy, shape: List[TensorTy], strides: List[TensorTy],
                               block_shape: List[ttgl.constexpr], padding_option: str = "zero") -> ttgl.tensor_descriptor:
        ndim = len(shape)
        if not (1 <= ndim <= 5):
        ...
        shape = [self.make_scalar(x, ttgl.int32) for x in shape]
        strides = [self.make_scalar(ttgl._unwrap_if_constexpr(x), ttgl.int64) for x in strides]
        ...
        handle = self.builder.create_make_tensor_descriptor(base_handle, [s.handle for s in shape],
                                                            ...
                                                            padding)
        return ttgl.tensor_descriptor(handle, shape, strides, type)