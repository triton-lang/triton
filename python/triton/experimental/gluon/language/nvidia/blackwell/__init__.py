...
@builtin
def tcgen05_copy(src, dst, barrier=None, _semantic=None):
    """
    Start an asynchronous copy from shared memory to tensor memory. If a barrier
    is provided, the barrier will be arrived-on with a count of 1 when the copy
    is complete.

    Args:
        src (shared_memory_descriptor): Shared memory to copy from.
        dst (tensor_memory_descriptor): Tensor memory to copy to.
        barrier (shared_memory_descriptor, optional): Barrier to track completion of the copy. Defaults to None.
    """
    ...
    barrier_handle = barrier.handle if barrier is not None else ir.value()
    _semantic.builder.create_tmem_copy(src.handle, dst.handle, barrier_handle)
