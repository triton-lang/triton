...
from ..._core import _unwrap_if_constexpr, builtin, uint32
...
def expect(mbarrier, bytes_per_cta=None, pred=True, *, from_cta=None, _semantic=None):
    ...
    Args:
        ...
        bytes_per_cta (int or tensor): Scalar expected byte count per CTA.
    ...
    """
    bytes_per_cta = _semantic.to_tensor(bytes_per_cta)
    _check(bytes_per_cta.type.is_int() and bytes_per_cta.type.primitive_bitwidth <= 32,
           lambda: "bytes_per_cta must be a scalar integer of at most 32 bits")
    bytes_per_cta = _semantic.cast(bytes_per_cta, uint32)
    from_cta = _unwrap_if_constexpr(from_cta)
    pred = _semantic.to_tensor(pred)
    _semantic.builder.create_mbarrier_expect(mbarrier.handle, bytes_per_cta.handle, pred.handle, from_cta)
