...
def link_headers_to_dispatcher(headers: Sequence[str]) -> Callable[[str], Tuple[str, str]]:
...
def make_dispatcher_func_sig(name: str, metas: Sequence[KernelLinkerMeta]) -> str:
    ...
    return src.format(kernel_name=name, signature=signature)
...
{func_sig} {{
    ...
    conds = "\n\n".join(conds)
    dispatcher_fn_sig = make_dispatcher_func_sig(name, metas)
...
if __name__ == "__main__":
    ...
    _codegen_func = link_headers_to_dispatcher(args.headers)
