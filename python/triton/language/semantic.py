...
def reshape(input: tl.tensor, dst_shape: List[int], builder: ir.builder) -> tl.tensor:
    raise ValueError(
        "`reshape` is not supported yet. Please use `view` instead if applicable. "
    ...
    # make_shape_compatible(block, block)
        ...
        if len(lhs_shape) < len(rhs_shape):
            ...
            for dim in range(len(lhs_shape), len(rhs_shape)):
                lhs = tl.tensor(
                    builder.create_expand_dims(lhs.handle, 0), tl.block_type(lhs_ty.scalar, [1] + lhs_shape)
                )
                lhs_ty = lhs.type
                lhs_shape = lhs_ty.get_block_shapes()
            ...
            for dim in range(len(rhs_shape), len(lhs_shape)):
                rhs = tl.tensor(
                    builder.create_expand_dims(rhs.handle, 0), tl.block_type(rhs_ty.scalar, [1] + rhs_shape)
                )
                rhs_ty = rhs.type
                rhs_shape = rhs_ty.get_block_shapes()
...
def cast(input: tl.tensor, dst_ty: tl.dtype, builder: ir.builder) -> tl.tensor:
    ...
    if (
        _is_cuda(builder.target)
        and builder.target.capability < 89
        and (src_sca_ty.is_fp8e4nv() or dst_sca_ty.is_fp8e4nv())
    ):
...
def atomic_cas(ptr: tl.tensor, cmp: tl.tensor, val: tl.tensor, sem: str, builder: ir.builder) -> tl.tensor:
    sem = _str_to_sem(sem)
...
def atomic_max(ptr: tl.tensor, val: tl.tensor, mask: tl.tensor, sem: str, builder: ir.builder) -> tl.tensor:
    ...
    if sca_ty.is_int():
        if sca_ty.is_int_signed():
            return tl.tensor(
                builder.create_atomic_rmw(ir.ATOMIC_OP.MAX, ptr.handle, val.handle, mask.handle, sem), val.type
            )
        else:
            return tl.tensor(
                builder.create_atomic_rmw(ir.ATOMIC_OP.UMAX, ptr.handle, val.handle, mask.handle, sem), val.type
            )
    # for float
    ...
    pos_ret = tl.tensor(
        builder.create_atomic_rmw(ir.ATOMIC_OP.MAX, i_ptr.handle, i_val.handle, and_(mask, pos, builder).handle, sem),
        i_val.type,
    )
    neg_ret = tl.tensor(
        builder.create_atomic_rmw(ir.ATOMIC_OP.UMIN, i_ptr.handle, i_val.handle, and_(mask, neg, builder).handle, sem),
        i_val.type,
    )
...
def atomic_min(ptr: tl.tensor, val: tl.tensor, mask: tl.tensor, sem: str, builder: ir.builder) -> tl.tensor:
    ...
    if sca_ty.is_int():
        if sca_ty.is_int_signed():
            return tl.tensor(
                builder.create_atomic_rmw(ir.ATOMIC_OP.MIN, ptr.handle, val.handle, mask.handle, sem), val.type
            )
        else:
            return tl.tensor(
                builder.create_atomic_rmw(ir.ATOMIC_OP.UMIN, ptr.handle, val.handle, mask.handle, sem), val.type
            )
    # for float
    ...
    neg = less_than(val, zero, builder)
    pos_ret = tl.tensor(
        builder.create_atomic_rmw(ir.ATOMIC_OP.MIN, i_ptr.handle, i_val.handle, and_(mask, pos, builder).handle, sem),
        i_val.type,
    )
    neg_ret = tl.tensor(
        builder.create_atomic_rmw(ir.ATOMIC_OP.UMAX, i_ptr.handle, i_val.handle, and_(mask, neg, builder).handle, sem),
        i_val.type,
    )
    ret = where(pos, pos_ret, neg_ret, builder)
    return bitcast(ret, sca_ty, builder)
...
def atomic_add(ptr: tl.tensor, val: tl.tensor, mask: tl.tensor, sem: str, builder: ir.builder) -> tl.tensor:
...
def atomic_and(ptr: tl.tensor, val: tl.tensor, mask: tl.tensor, sem: str, builder: ir.builder) -> tl.tensor:
...
def atomic_or(ptr: tl.tensor, val: tl.tensor, mask: tl.tensor, sem: str, builder: ir.builder) -> tl.tensor:
...
def atomic_xor(ptr: tl.tensor, val: tl.tensor, mask: tl.tensor, sem: str, builder: ir.builder) -> tl.tensor:
...
def atomic_xchg(ptr: tl.tensor, val: tl.tensor, mask: tl.tensor, sem: str, builder: ir.builder) -> tl.tensor:
...
def dot(
    ...
    def assert_dtypes_valid(lhs_dtype, rhs_dtype, target):
        ...
        if not _is_cuda(target):
            assert (
                lhs_dtype == rhs_dtype
            ), f"First input ({lhs_dtype}) and second input ({rhs_dtype}) must have the same dtype!"
            return
        # Checks for cuda arch
        if target.capability < 90:
            assert (
                not lhs_dtype.is_fp8e4nv() and not rhs_dtype.is_fp8e4nv()
            ...
            assert (
                not lhs_dtype.is_fp8e4b15() and not rhs_dtype.is_fp8e4b15()
            ), "Dot op does not support fp8e4b15 on CUDA arch >= 90"
            assert (
                not lhs_dtype.is_fp8e4b15x4() and not rhs_dtype.is_fp8e4b15x4()
            ), "Dot op does not support fp8e4b15x4 on CUDA arch >= 90"
            if lhs_dtype.is_int() or rhs_dtype.is_int():
                assert (
                    lhs_dtype == rhs_dtype
                ), f"Both operands must be same type. First operand ({lhs_dtype}) and second operand ({rhs_dtype})"
                assert (
                    lhs_dtype.is_int8() or lhs_dtype.is_uint8()
                ), f"Both operands must be either int8 or uint8. Operand type ({lhs_dtype})"
            elif lhs_dtype.is_fp8() or rhs_dtype.is_fp8():
                assert (
                    lhs_dtype.is_fp8e4nv() or lhs_dtype.is_fp8e5()
                ), f"Only supports fp8e4nv or fp8e5. First operand ({lhs_dtype})"
                assert (
                    rhs_dtype.is_fp8e4nv() or rhs_dtype.is_fp8e5()
                ), f"Only supports fp8e4nv or fp8e5. Second operand ({rhs_dtype})"
            else:
                assert (
                    lhs_dtype.is_fp16() or lhs_dtype.is_bf16() or lhs_dtype.is_fp32() or lhs_dtype.is_int1()
    ...
    assert (
        lhs.shape[1].value == rhs.shape[0].value
    ), f"First input shape ({lhs.shape}) and second input shape {rhs.shape} are not compatible for matmul (second index of first shape ({lhs.shape[1].value}) must be equal to first index of second shape ({rhs.shape[0].value})"
    assert (
        lhs.shape[0].value >= 16 and lhs.shape[1].value >= 16 and rhs.shape[1].value >= 16
    ...
    if (
        is_hip()
        and mfma_supported(M, N, lhs.type.shape[1], allow_tf32, ret_scalar_ty)
        and ret_scalar_ty.primitive_bitwidth < 32
    ):
        if lhs.type.scalar.is_int():
    ...
    if not (
        _is_cuda(builder.target)
        and builder.target.capability == 90
        and lhs.dtype.is_fp8()
        and rhs.dtype.is_fp8()
