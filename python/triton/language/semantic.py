...
# ===----------------------------------------------------------------------===//
    ...
    def dot_sparse(self, lhs: tl.tensor, rhs: tl.tensor, lhs_meta: tl.tensor, acc: tl.tensor) -> tl.tensor:
        assert lhs.type.is_block() and rhs.type.is_block()
        ...
        if acc is None:
            ...
            if builder.codegen_fns.get("sparse_dot_acc_emulation") is not None:
                ...
                sparse_dot_result = tl.tensor(
                    self.builder.create_dot_sparse(lhs.handle, rhs.handle, acc_handle, lhs_meta.handle), ret_ty)
                return tl.tensor(self.builder.create_fadd(sparse_dot_result.handle, acc_handle), ret_ty)

        return tl.tensor(self.builder.create_dot_sparse(lhs.handle, rhs.handle, acc_handle, lhs_meta.handle), ret_ty)
