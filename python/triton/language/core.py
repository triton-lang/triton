...
class tensor:
    ...
    def sort(self, dim: constexpr | None = None, descending: constexpr = constexpr(False)) -> tensor:
        ...
...
def broadcast_to(input, *shape, _builder=None):
    ...
    :code:`shape` can be passed as a tuple or as individual parameters: ::
        ...
        broadcast_to(x, 32, 32)
    """
    # Unpack shape if the user passed a single tuple/list.
    if len(shape) == 1 and isinstance(shape[0], collections.abc.Iterable):
        shape = shape[0]
    shape = _shape_check_impl(shape)
...
def permute(input, *dims, _builder=None):
    ...
    """
    # Unpack dims if the user passed a single tuple/list.
    if len(dims) == 1 and isinstance(dims[0], collections.abc.Iterable):
        dims = dims[0]
    return semantic.permute(input, dims, _builder)
