...
def define_kernel(src, attrs=None, **extra_globals):
    '''
...
def specialize(fn, constants, name=None):
    ...
    non_const_args = [arg for arg in args if arg.split(":")[0].split("=")[0].strip() not in constants]
    # add global symbols
    ...
    new_signature = f"def {name}({', '.join(non_const_args)}):"
    constexpr_lines = [
        f"    {key}: tl.constexpr = {value.__name__ if callable(value) else value}" for key, value in constants.items()
    ]
    new_src = "\n".join(["@triton.jit", new_signature] + constexpr_lines + body_lines)
    ...
    ret = define_kernel(new_src, attrs, **extra_globals)
    return ret
