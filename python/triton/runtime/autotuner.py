...
def autotune(configs, key, prune_configs_by=None, reset_to_zero=None, restore_value=None, pre_hook=None, post_hook=None,
    ...
    def decorator(fn):
        ...
        if hasattr(fn, 'params'):
            for p in fn.params:
                if p.is_autotune_key and p.name not in merged_key:
                    merged_key.append(p.name)
        return Autotuner(fn, fn.arg_names, configs, merged_key, reset_to_zero, restore_value, pre_hook=pre_hook,
