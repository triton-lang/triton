...
def optimize_ttgir(mod, num_stages, num_warps, num_ctas, arch,
    ...
    ws_enabled = False
    if arch // 10 >= 9 and enable_warp_specialization:
...
def make_hash(fn, arch, env_vars, **kwargs):
    if isinstance(fn, JITFunction):
        ...
        get_conf_key = lambda conf: (sorted(conf.divisible_by_16), sorted(conf.equal_to_1), sorted(conf.ids_of_folded_args))
...
instance_descriptor = namedtuple("instance_descriptor", ["divisible_by_16", "equal_to_1", "ids_of_folded_args"], defaults=[set(), set(), set()])
...
class CompiledKernel:
    ...
    def __init__(self, fn, so_path, metadata, asm):
        ...
        # initialize metadata
        # TODO[shuhaoj]: find a more flexible way to modify num_warps for kernel
        if metadata['enable_warp_specialization']:
            metadata["num_warps"] = 2 * metadata["num_warps"]
