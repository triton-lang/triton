...
// %a_prefetch = triton_gpu_rocm.convert_layout %a_tmp
// scf.for %iv = ... iter_args(%a_buf = %a, ..., %a_prefetch_arg = %a_prefetch)
// {
//   %x = tt.dot %a_arg, %b, %c
//   %a_tmp_rem = tensor.extract_slice %a_buf[0, 16] [128, 16]
//   %a_prefetch_next = triton_gpu_rocm.convert_layout %a_tmp_rem
//   ...
...
Value Prefetcher::generatePrefetch(Value v, unsigned opIdx, bool isPrologue,
  ...
  Value newSmem = builder.create<triton::gpu_rocm::ExtractSliceOp>(
  ...
  auto dotOperandEnc = triton::gpu_rocm::DotOperandEncodingAttr::get(
      builder.getContext(), opIdx, dotEncoding, prefetchWidth / 8);
  Value prefetchSlice = builder.create<triton::gpu_rocm::ConvertLayoutOp>(
...
LogicalResult Prefetcher::initialize() {
  ...
  auto getPrefetchSrc = [](Value v) -> SmallVector<Value> {
    ...
    while (op) {
      ...
      if (auto cvt = dyn_cast_or_null<triton::gpu_rocm::ConvertLayoutOp>(op))
        if (triton::gpu_rocm::isSharedEncoding(cvt.getOperand())) {
  ...
  for (triton::DotOp dot : dotsInFor) {
    ...
    auto aEnc = aType.getEncoding().cast<triton::gpu_rocm::DotOperandEncodingAttr>();
    auto bEnc = bType.getEncoding().cast<triton::gpu_rocm::DotOperandEncodingAttr>();
...
std::unique_ptr<Pass> mlir::createTritonGPUROCMPrefetchPass() {
