...
struct CoalescePass : public TritonGPUCoalesceBase<CoalescePass> {
  Attribute getCoalescedEncoding(ModuleAxisInfoAnalysis &axisInfoAnalysis,
    ...
    auto queryAxisInfo = [&](const Value &val) -> AxisInfo {
      ...
      if (auto ptrType = valType.dyn_cast<PointerType>()) {
        ...
        auto tileShape = triton::gpu_rocm::getShapePerCTA(tensorTy);
    ...
    auto shapePerCTA = triton::gpu_rocm::getShapePerCTA(refTensorType);
    ...
    auto CTALayout = triton::gpu_rocm::getCTALayout(refTensorType.getEncoding());
    return triton::gpu_rocm::BlockedEncodingAttr::get(
  ...
  void coalesceOp(LayoutMap &layoutMap, Operation *op, Value ptr,
    ...
    for (auto operand : op->getOperands()) {
      ...
      if (tensorType &&
          !tensorType.getEncoding().isa<triton::gpu_rocm::SharedEncodingAttr>())
        newArgs.push_back(builder.create<triton::gpu_rocm::ConvertLayoutOp>(
    ...
    for (auto t : op->getResultTypes()) {
      bool isAsync = std::is_same<T, triton::gpu_rocm::InsertSliceAsyncOp>::value;
    ...
    for (size_t i = 0; i < op->getNumResults(); i++) {
      ...
      if (newTypes[i] != op->getResultTypes()[i]) {
        newResult = builder.create<triton::gpu_rocm::ConvertLayoutOp>(
  ...
  void runOnOperation() override {
    ...
    moduleOp.walk([&](Operation *curr) {
      ...
      if (auto op = dyn_cast<triton::AtomicCASOp>(curr))
        ptr = op.getPtr();
      if (auto op = dyn_cast<triton::gpu_rocm::InsertSliceAsyncOp>(curr))
        ptr = op.getSrc();
    ...
    moduleOp.walk([&](Operation *curr) {
      ...
      if (auto load = dyn_cast<triton::gpu_rocm::InsertSliceAsyncOp>(curr)) {
        coalesceOp<triton::gpu_rocm::InsertSliceAsyncOp>(layoutMap, curr,
                                                    load.getSrc(), builder);
        return;
      }
...
std::unique_ptr<Pass> mlir::createTritonGPUROCMCoalescePass() {
