...
public:
  ...
  mlir::LogicalResult processEncoding(encTy encoding,
                                      triton::gpu_rocm::ConvertLayoutOp convert,
    ...
    auto tmp = rewriter.create<triton::gpu_rocm::ConvertLayoutOp>(
        convert.getLoc(), tmpType, convert.getOperand());
    rewriter.replaceOpWithNewOp<triton::gpu_rocm::ConvertLayoutOp>(convert, dstType,
                                                                tmp);
    return mlir::success();
  }
  ...
  matchAndRewrite(mlir::Operation *op,
    ...
    if (!llvm::isa<triton::gpu_rocm::ConvertLayoutOp>(op))
      return mlir::failure();
    auto convert = llvm::cast<triton::gpu_rocm::ConvertLayoutOp>(op);
    auto srcType = convert.getOperand().getType().cast<RankedTensorType>();
    auto dstType = convert.getType().cast<RankedTensorType>();
    if (srcType.getEncoding().isa<triton::gpu_rocm::BlockedEncodingAttr>() &&
        dstType.getEncoding().isa<triton::gpu_rocm::DotOperandEncodingAttr>()) {
      auto dstDotOperand =
          dstType.getEncoding().cast<triton::gpu_rocm::DotOperandEncodingAttr>();
      auto dstParent = dstDotOperand.getParent();
      if (dstDotOperand.getOpIdx() == 1 ||
          (!dstParent.isa<triton::gpu_rocm::MmaEncodingAttr>() &&
           !dstParent.isa<triton::gpu_rocm::MfmaEncodingAttr>()))
        return mlir::failure();

      if (dstParent.isa<triton::gpu_rocm::MmaEncodingAttr>()) {
        auto dstParentMma = dstParent.cast<triton::gpu_rocm::MmaEncodingAttr>();
        if (dstParentMma.isVolta() || dstParentMma.getWarpsPerCTA()[1] > 1)
          return mlir::failure();
        return processEncoding(dstParentMma, convert, dstType, rewriter);
      }

      if (dstParent.isa<triton::gpu_rocm::MfmaEncodingAttr>()) {
        auto dstParentMfma = dstParent.cast<triton::gpu_rocm::MfmaEncodingAttr>();
...
public:
  ...
  matchAndRewrite(mlir::Operation *op,
    ...
    if (std::distance(dstOp->user_begin(), dstOp->user_end()) != 1 ||
      ...
      return mlir::failure();
    auto cvtOp =
        dotOp.getOperand(2).getDefiningOp<triton::gpu_rocm::ConvertLayoutOp>();
    if (!cvtOp)
      return mlir::failure();
    ...
    auto newCvt = rewriter.create<triton::gpu_rocm::ConvertLayoutOp>(
...
static bool hasConvertToMMATransisitiveUse(Operation *op, Attribute encoding) {
  ...
  while (!queue.empty()) {
    ...
    for (Operation *op : forwardSlice) {
      if (auto convertOp = dyn_cast<triton::gpu_rocm::ConvertLayoutOp>(op)) {
    ...
    if (isLayoutAnchor(op)) {
      for (auto result : op->getResults()) {
        if (auto tensorType = result.getType().dyn_cast<RankedTensorType>()) {
          ...
          if (tensorType.getEncoding().isa<triton::gpu_rocm::MmaEncodingAttr>() &&
          ...
          // into global memory.
          if (tensorType.getEncoding().isa<triton::gpu_rocm::MfmaEncodingAttr>() &&
  ...
  for (OpOperand &use : value.getUses()) {
    ...
    if (user->hasTrait<mlir::OpTrait::SameOperandsAndResultEncoding>() ||
        ...
        isa<triton::ReduceOp, triton::ExpandDimsOp,
            triton::gpu_rocm::ConvertLayoutOp>(user)) {
#if 1
      if (auto convertOp = dyn_cast<triton::gpu_rocm::ConvertLayoutOp>(user)) {
        if (triton::gpu_rocm::isSharedEncoding(convertOp.getResult()) ||
            triton::gpu_rocm::isSharedEncoding(convertOp.getOperand()))
          continue;
      }
#endif
...
void LayoutPropagation::resolveConflicts() {
  for (auto &it : layouts) {
    ...
    for (Attribute e : info.encodings) {
      if (e.isa<triton::gpu_rocm::BlockedEncodingAttr>()) {
        encoding = e;
        break;
      }
    }
...
Value LayoutPropagation::getValueAs(Value value, Attribute encoding) {
  if (auto tensorType = value.getType().dyn_cast<RankedTensorType>()) {
    ...
    Value converted = rewriter.create<triton::gpu_rocm::ConvertLayoutOp>(
...
Operation *LayoutPropagation::rewriteOp(Operation *op) {
  ...
  if (auto convertOp = dyn_cast<triton::gpu_rocm::ConvertLayoutOp>(op)) {
    ...
    auto newType = RankedTensorType::get(tensorType.getShape(),
                                         tensorType.getElementType(), encoding);
    auto cvt = rewriter.create<triton::gpu_rocm::ConvertLayoutOp>(op->getLoc(),
                                                             newType, src);
  ...
  if (canFoldIntoConversion(op, encoding)) {
    ...
    auto newType = RankedTensorType::get(tensorType.getShape(),
                                         tensorType.getElementType(), encoding);
    auto cvt = rewriter.create<triton::gpu_rocm::ConvertLayoutOp>(
  ...
  if (op->hasTrait<mlir::OpTrait::SameOperandsAndResultEncoding>() ||
      ...
      isa<triton::ReduceOp, triton::ExpandDimsOp, triton::gpu_rocm::ConvertLayoutOp>(
          op)) {
...
static bool canBeRemat(Operation *op) {
  if (isa<triton::LoadOp, triton::StoreOp>(op))
    return !isExpensiveLoadOrStore(op);
  if (isa<tensor::ExtractSliceOp, triton::gpu_rocm::AllocTensorOp,
          triton::gpu_rocm::InsertSliceAsyncOp, triton::AtomicRMWOp,
...
static void rewriteSlice(SetVector<Value> &slice,
  ...
  for (Operation *op : opsToRewrite) {
    ...
    if (isa<arith::ConstantOp>(op)) {
      ...
      auto cvt = builder.create<triton::gpu_rocm::ConvertLayoutOp>(
          op->getLoc(), newType, newOp->getResult(0));
...
static void backwardRematerialization(ConvertLayoutOp convertOp) {
  ...
  if (targetType.getEncoding().isa<triton::gpu_rocm::DotOperandEncodingAttr>())
    return;
...
std::unique_ptr<Pass> mlir::createTritonGPUROCMRemoveLayoutConversionsPass() {
