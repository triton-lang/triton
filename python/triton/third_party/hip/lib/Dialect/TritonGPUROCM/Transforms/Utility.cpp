...
triton::gpu_rocm::SharedEncodingAttr getSharedEncoding(RankedTensorType tensorTy) {
  auto blockedLayout =
      tensorTy.getEncoding().cast<triton::gpu_rocm::BlockedEncodingAttr>();
  return triton::gpu_rocm::SharedEncodingAttr::get(
...
std::string GraphLayoutMarker::getColor(const Type &type) const {
  if (auto tensorTy = type.dyn_cast<RankedTensorType>()) {
    ...
    if (layout.isa<triton::gpu_rocm::BlockedEncodingAttr>())
      return "green";
    else if (layout.isa<triton::gpu_rocm::SliceEncodingAttr>())
      return "yellow";
    else if (layout.isa<triton::gpu_rocm::MmaEncodingAttr>())
      return "lightslateblue";
    else if (layout.isa<triton::gpu_rocm::DotOperandEncodingAttr>())
      return "orange";
    else if (layout.isa<triton::gpu_rocm::SharedEncodingAttr>())
      return "orangered";
    else
...
static std::optional<Attribute> inferDstEncoding(triton::ReduceOp op,
                                                 Attribute encoding) {
  return triton::gpu_rocm::SliceEncodingAttr::get(op->getContext(), op.getAxis(),
                                             encoding);
}

static std::optional<Attribute> inferDstEncoding(triton::ExpandDimsOp op,
                                                 Attribute encoding) {
  auto sliceEncoding = encoding.dyn_cast<triton::gpu_rocm::SliceEncodingAttr>();
  if (!sliceEncoding)
    return std::nullopt;
...
bool isExpensiveToRemat(Operation *op, Attribute &targetEncoding) {
  ...
  if (isa<triton::CatOp>(op))
    return triton::gpu_rocm::isExpensiveCat(cast<triton::CatOp>(op), targetEncoding);
...
bool canFoldIntoConversion(Operation *op, Attribute targetEncoding) {
  if (isa<triton::CatOp>(op))
    return !triton::gpu_rocm::isExpensiveCat(cast<triton::CatOp>(op),
                                        targetEncoding);
  if (auto convert = dyn_cast<triton::gpu_rocm::ConvertLayoutOp>(op)) {
    if (targetEncoding.isa<triton::gpu_rocm::MmaEncodingAttr>()) {
      auto srcEncoding =
  ...
  return isa<triton::gpu_rocm::ConvertLayoutOp, arith::ConstantOp,
