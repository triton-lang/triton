...
struct ConvertLayoutOpConversion
    : public ConvertTritonGPUOpToLLVMPattern<triton::gpu_rocm::ConvertLayoutOp> {
public:
  using ConvertTritonGPUOpToLLVMPattern<
      triton::gpu_rocm::ConvertLayoutOp>::ConvertTritonGPUOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(triton::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
...
private:
  ...
  getMultiDimOffset(Attribute layout, Location loc,
    ...
    if (auto mmaLayout = layout.dyn_cast<MmaEncodingAttr>()) {
      ...
      unsigned iWaveSize = triton::gpu_rocm::getWarpSize(layout);
  ...
  LogicalResult
  lowerDistToDistWithDistSmem(triton::gpu_rocm::ConvertLayoutOp op,
                              OpAdaptor adaptor,
    ...
    auto srcCTAsPerCGA = triton::gpu_rocm::getCTAsPerCGA(srcLayout);
    auto srcCTAOrder = triton::gpu_rocm::getCTAOrder(srcLayout);
  ...
  LogicalResult
  lowerDistributedToDistributed(triton::gpu_rocm::ConvertLayoutOp op,
                                OpAdaptor adaptor,
                                ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
    Value src = op.getSrc();
  ...
  LogicalResult
  lowerSharedToDistributed(triton::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
                           ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
    Value src = op.getSrc();
    Value dst = op.getResult();
  ...
  LogicalResult
  lowerDistributedToShared(triton::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
                           ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
    Value src = op.getSrc();
    Value dst = op.getResult();
    ...
    auto srcShape = srcTy.getShape();
    auto dstTy = dst.getType().cast<RankedTensorType>();
    auto dstShapePerCTA = triton::gpu_rocm::getShapePerCTA(dstTy);
    ...
    if (mmaLayout && mmaLayout.isHopper() && elemSize == 16 &&
      ...
      unsigned inVec =
          inOrd == outOrd ? triton::gpu_rocm::getContigPerThread(mmaLayout)[inOrd[0]]
                          : 1;
  ...
  // shared -> mma_operand
  LogicalResult
  lowerSharedToDotOperand(triton::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
                          ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
    Value src = op.getSrc();
    Value dst = op.getResult();
...
#if 1
  LogicalResult
  lowerMfmaToDotOperand(triton::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
                        ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
  ...
  // mma -> dot_operand
  LogicalResult
  lowerMmaToDotOperand(triton::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
                       ConversionPatternRewriter &rewriter) const {
    auto loc = op.getLoc();
    auto srcTy = op.getSrc().getType().cast<RankedTensorType>();
    auto dstTy = op.getResult().getType().cast<RankedTensorType>();
  ...
  Value lowerSharedToDotOperandMFMA(
      triton::gpu_rocm::ConvertLayoutOp op, OpAdaptor adaptor,
...
}; // namespace triton::gpu_rocm::ConvertLayoutOp>
