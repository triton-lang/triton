...
using ::mlir::triton::gpu_rocm::getCTALayout;
...
struct StoreAsyncOpConversion
  ...
  lowerStoreAsyncWithSlice(triton::nvidia_gpu::StoreAsyncOp op,
    ...
    auto srcOrder = triton::gpu_rocm::getOrder(srcLayout);
    unsigned inVec =
        srcOrder == sharedLayout.getOrder()
            ? triton::gpu_rocm::getContigPerThread(srcLayout)[srcOrder[0]]
            : 1;
  ...
  int getNumTMADescs(LLVM::LLVMFuncOp func) const {
    if (!func->hasAttr(kAttrNumTMALoadDescsName)) {
      llvm::report_fatal_error("TritonGPU module should contain a "
                               "triton_gpu_rocm.num-tma-load attribute");
      return -1;
    }
    if (!func->hasAttr(kAttrNumTMAStoreDescsName)) {
      llvm::report_fatal_error("TritonGPU module should contain a "
                               "triton_gpu_rocm.num-tma-store attribute");
      return -1;
    }
...
struct InsertSliceAsyncOpConversion
    : public ConvertTritonGPUOpToLLVMPattern<triton::gpu_rocm::InsertSliceAsyncOp>,
      public LoadStoreConversionBase {
  using ConvertTritonGPUOpToLLVMPattern<
      triton::gpu_rocm::InsertSliceAsyncOp>::ConvertTritonGPUOpToLLVMPattern;
  ...
  InsertSliceAsyncOpConversion(
      ...
      : ConvertTritonGPUOpToLLVMPattern<triton::gpu_rocm::InsertSliceAsyncOp>(
            converter, allocation, indexCacheInfo, benefit),
        LoadStoreConversionBase(axisAnalysisPass) {}

  LogicalResult
  matchAndRewrite(triton::gpu_rocm::InsertSliceAsyncOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
