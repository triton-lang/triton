#ifndef TRITON_CONVERSION_TRITONGPUROCM_TO_LLVM_BASE_H
#define TRITON_CONVERSION_TRITONGPUROCM_TO_LLVM_BASE_H
...
constexpr ::llvm::StringLiteral kAttrNumTMALoadDescsName =
    "triton_gpu_rocm.num-tma-load";
constexpr ::llvm::StringLiteral kAttrNumTMAStoreDescsName =
    "triton_gpu_rocm.num-tma-store";
...
using ::mlir::triton::gpu_rocm::CTALayoutAttr;
...
public:
  ...
  getSwizzledSharedPtrs(Location loc, unsigned inVec, RankedTensorType srcTy,
                        triton::gpu_rocm::SharedEncodingAttr resSharedLayout,
    ...
    auto srcShapePerCTA = triton::gpu_rocm::getShapePerCTA(srcTy);
    unsigned numElems = triton::gpu_rocm::getTotalElemsPerThread(srcTy);
    // swizzling params as described in TritonGPUAttrDefs.td
    unsigned outVec = resSharedLayout.getVec();
    unsigned perPhase = resSharedLayout.getPerPhase();
    unsigned maxPhase = resSharedLayout.getMaxPhase();
    // Order
    auto inOrder = triton::gpu_rocm::getOrder(srcEncoding);
    auto outOrder = triton::gpu_rocm::getOrder(resSharedLayout);
  ...
  loadSharedToDistributed(Value dst, ArrayRef<SmallVector<Value>> dstIndices,
    ...
    auto srcSharedLayout =
        srcTy.getEncoding().cast<triton::gpu_rocm::SharedEncodingAttr>();
    auto srcElemTy = srcTy.getElementType();
    auto dstElemTy = dstTy.getElementType();
    auto inOrd = triton::gpu_rocm::getOrder(srcSharedLayout);
    auto outOrd = triton::gpu_rocm::getOrder(dstDistributedLayout);
    unsigned outVec =
        inOrd == outOrd
            ? triton::gpu_rocm::getContigPerThread(dstDistributedLayout)[outOrd[0]]
            : 1;
    unsigned inVec = srcSharedLayout.getVec();
    unsigned minVec = std::min(outVec, inVec);
    unsigned outElems = triton::gpu_rocm::getTotalElemsPerThread(dstTy);
  ...
  void storeDistributedToShared(Value src, Value llSrc,
    ...
    auto dstSharedLayout =
        dstTy.getEncoding().cast<triton::gpu_rocm::SharedEncodingAttr>();
    auto dstElemTy = dstTy.getElementType();
    auto inOrd = triton::gpu_rocm::getOrder(srcDistributedLayout);
    auto outOrd = dstSharedLayout.getOrder();
    unsigned inVec =
        inOrd == outOrd
            ? triton::gpu_rocm::getContigPerThread(srcDistributedLayout)[inOrd[0]]
            : 1;
  ...
  Value getMask(Type valueTy, ConversionPatternRewriter &rewriter,
    ...
    if (tensorTy) {
      ...
      auto threadsPerWarp = triton::gpu_rocm::getThreadsPerWarp(layout);
      auto warpsPerCTA = triton::gpu_rocm::getWarpsPerCTA(layout);
      auto order = triton::gpu_rocm::getOrder(layout);
      auto shapePerCTATile = triton::gpu_rocm::getShapePerCTATile(layout, shape);
      Value warpSize = i32_val(triton::gpu_rocm::getWarpSize(layout));
      ...
      if (triton::gpu_rocm::getNumCTAs(layout) > 1) {
        auto _0 = i32_val(0);
        auto CTAsPerCGA = triton::gpu_rocm::getCTAsPerCGA(layout);
        auto CTASplitNum = triton::gpu_rocm::getCTASplitNum(layout);
        auto CTAOrder = triton::gpu_rocm::getCTAOrder(layout);
  ...
  SmallVector<Value> emitCTAOffsetForLayout(Location loc,
    ...
    unsigned rank = shape.size();
    SmallVector<unsigned> CTAsPerCGA = triton::gpu_rocm::getCTAsPerCGA(layout);
    SmallVector<unsigned> CTASplitNum = triton::gpu_rocm::getCTASplitNum(layout);
    SmallVector<unsigned> CTAOrder = triton::gpu_rocm::getCTAOrder(layout);
    SmallVector<int64_t> shapePerCTA =
        triton::gpu_rocm::getShapePerCTA(CTASplitNum, shape);
...
private:
  ...
  SmallVector<Value> emitBaseIndexWithinCTAForBlockedLayout(
    ...
    Value warpSize = i32_val(triton::gpu_rocm::getWarpSize(blockedLayout));
    ...
    auto shapePerCTA = triton::gpu_rocm::getShapePerCTA(blockedLayout, shape);
    unsigned rank = shape.size();
  ...
  emitOffsetForBlockedLayout(const BlockedEncodingAttr &blockedLayout,
    ...
    unsigned elemsPerThread = triton::gpu_rocm::getTotalElemsPerThread(type);
  ...
  SmallVector<Value> emitBaseIndexWithinCTAForMmaLayoutV1(
    ...
    Value warpSize = i32_val(triton::gpu_rocm::getWarpSize(mmaLayout));
  ...
  emitBaseIndexForMfmaLayout(Location loc, ConversionPatternRewriter &rewriter,
    ...
    Value threadId = getThreadId(rewriter, loc);
    Value warpSize = i32_val(triton::gpu_rocm::getWarpSize(mfmaLayout));
    Value laneId = urem(threadId, warpSize);
