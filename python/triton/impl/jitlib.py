...
class JITFunction(KernelInterface[T]):
    ...
    @staticmethod
    def _type_of(key):
        if isinstance(key, (torch.dtype, base.dtype)):
            ty = {
                torch.bool: "i1",
                torch.float16: "fp16",
                torch.bfloat16: "bf16",
...
def {self.fn.__name__}({', '.join(self.arg_names)}, grid, num_warps=4, num_stages=3, extern_libs=None, stream=None, warmup=False):
    ...
    try:
      ...
      if not warmup:
          bin.c_wrapper(grid_0, grid_1, grid_2, bin.num_warps, bin.shared, stream, bin.cu_function, tr.CompiledKernel.launch_enter_hook, tr.CompiledKernel.launch_exit_hook, bin, {args})
      return bin
