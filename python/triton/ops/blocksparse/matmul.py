...
class _matmul(torch.autograd.Function):
    ...
    def make_sdd_lut(layout, block, dtype, device):
        ...
        superblocks = libtriton.superblock(layout.type(torch.int32), 1)
        luts, widths, packs = [], [], []
    ...
    @staticmethod
    def _sdd_matmul(
        a, b, trans_a, trans_b, trans_c, spdims, block, luts, num_locks, widths, packs
    ):
        ...
        for lut, width, pack in zip(luts, widths, packs):
            ...
            if key not in _matmul.sdd_cache:
                ...
                _matmul.sdd_cache[key] = triton.kernel(
                    src, device=device, defines=defines
                )
    ...
    def make_dxx_lut(layout, block, step, trans, device, transform=lambda idx: idx):
        ...
        for z in range(layout.size(0)):
            ...
            z_segments, z_column, z_lockid, z_maxid, z_offsets = _matmul.load_balance(
                sizes, block
            )
        ...
        offsets += 6 * width
        header = (
            torch.stack((offsets, segments, column, depth, lockid, maxid), dim=1)
            .view(-1)
            .contiguous()
        )
    ...
    @staticmethod
    def _dds_matmul(
        a, b, trans_a, trans_b, trans_c, spdims, block, lut, num_locks, width, packs
    ):
        # shapes / dtypes
        AS0 = a.size(0)
        AS1 = a.size(1)
        ...
        if key not in _matmul.dds_cache:
            ...
            _matmul.dds_cache[key] = triton.kernel(
                src, device=a.device, defines=defines
            )
    ...
    def _dsd_matmul(
        ...
        if key not in _matmul.dsd_cache:
            ...
            _matmul.dsd_cache[key] = triton.kernel(
                src, device=a.device, defines=defines
            )
...
class matmul:
    def make_lut(self, dtype, device):
        ...
        if self.mode == "sdd":
            c_lut, c_num_locks, c_width, c_packs = _matmul.make_sdd_lut(
                layout, block, dtype, device
            )
        elif self.mode == "dsd":
            c_lut, c_num_locks, c_width, c_packs = _matmul.make_dxx_lut(
        ...
        # DA look-up table
        if self.mode == "sdd":
            da_lut, da_num_locks, da_width, da_packs = _matmul.make_dxx_lut(
                layout, block, step, True, device
            )
        elif self.mode == "dsd":
            da_lut, da_num_locks, da_width, da_packs = _matmul.make_sdd_lut(
                layout, block, dtype, device
            )
        elif self.mode == "dds":
            da_lut, da_num_locks, da_width, da_packs = _matmul.make_dxx_lut(
        ...
        # DB look-up table
        if self.mode == "sdd":
            db_lut, db_num_locks, db_width, db_packs = _matmul.make_dxx_lut(
            ...
            db_lut, db_num_locks, db_width, db_packs = _matmul.make_sdd_lut(
                layout, block, dtype, device
            )
        self.lut_cache[key] = (
            c_lut,
    ...
    def __call__(self, a, b):
        c_lut, c_num_locks, c_width, c_packs, da_lut, da_num_locks, da_width, da_packs, db_lut, db_num_locks, db_width, db_packs = self.make_lut(
            a.dtype, a.device
        )
