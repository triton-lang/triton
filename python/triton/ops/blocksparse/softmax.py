...
# fmt: off
@triton.heuristics({"num_warps": lambda *_, **meta: num_warps(meta["SIZEMAX"] * meta["BLOCK"])})
@triton.heuristics({"TN": lambda *_, **meta: next_power_of_2(meta["SIZEMAX"] * meta["BLOCK"])})
@triton.jit
def _forward(
    X,
    scale,
...
@triton.heuristics({"TN": lambda *_, **meta: next_power_of_2(meta["SIZEMAX"] * meta["BLOCK"])})
@triton.jit
def _backward(X, scale, DX, LUT, stride_zx, stride_zdx, **meta):
    pidhm = tl.program_id(0)
    pidz = tl.program_id(1)
    TN = meta["TN"]
    BLOCK = meta["BLOCK"]
...
class _softmax(torch.autograd.Function):
    ...
    def forward(
        ...
        def grid(*_): return [spdims[0] * spdims[1] * block, M]
    ...
    def backward(ctx, dx):
        ...
        def grid(*_): return [ctx.spdims[0] * ctx.spdims[1] * ctx.block, M]

        _backward[grid](
            x, ctx.scale, dx, lut, x.stride(0), dx.stride(0), force_nc_cache=True, BLOCK=ctx.block, SIZEMAX=ctx.maxlut
        )
