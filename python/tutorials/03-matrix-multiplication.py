...
def benchmark(M, N, K, provider):
    a = torch.randint(128, (M, K), device='cuda', dtype=torch.int8).view(torch.float8_e5m2)
    b = torch.randint(128, (N, K), device='cuda', dtype=torch.int8).view(torch.float8_e5m2).T
    quantiles = [0.5, 0.2, 0.8]
    # if provider == 'cublas':
    #     ms, min_ms, max_ms = triton.testing.do_bench(lambda: torch.matmul(a, b), quantiles=quantiles)
    if provider == 'triton':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: triton.ops.matmul(a, b), quantiles=quantiles)
