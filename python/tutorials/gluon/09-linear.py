...
# os.environ["TRITON_LLVM_DEBUG_ONLY"] = "loop-rotate"
...
def matmul2(a, b, kernel):
    M, K = a.shape
    _, N = b.shape

    c = torch.empty((M, N), device=a.device, dtype=torch.float16)

    grid1 = (triton.cdiv(M, 128) * triton.cdiv(N, 128), 1, 1)
...
def benchmark(M, N, K, provider):
    ...
    w_t = w.t().contiguous()  # 提前转置并确保内存连续
    quantiles = [0.5, 0.2, 0.8]
    if provider == 'gluon':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: gluon_matmul(a, w_t), quantiles=quantiles)
    if provider == 'triton':
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: triton_matmul(a, w_t), quantiles=quantiles)
