...
def matmul_kernel(
        ...
        GROUP_SIZE_M: tl.constexpr, NUM_STAGES: tl.constexpr, WARP_SPECIALIZE: tl.constexpr):
...
def matmul(a, b, *, num_ctas, warp_specialize=False, out=None, block_m=256, block_n=128, block_k=64, group_size_m=8,
    ...
    return matmul_kernel[grid](
        a_desc, b_desc, c_desc, M, N, K, block_m, block_n, block_k, group_size_m, num_stages, num_warps=num_warps,
        num_ctas=num_ctas, WARP_SPECIALIZE=warp_specialize)
...
MATMUL_CONFIGS = [
    ...
    {"block_m": 128, "block_n": 256, "block_k": 64, "num_stages": 3, "num_warps": 4},
    {"block_m": 256, "block_n": 128, "block_k": 32, "num_stages": 4, "num_warps": 4},
    {"block_m": 256, "block_n": 128, "block_k": 64, "num_stages": 3, "num_warps": 4},
    {"block_m": 256, "block_n": 128, "block_k": 64, "num_stages": 4, "num_warps": 8},
]

TWO_CTA_CONFIGS = [
    {"block_m": 256, "block_n": 128, "block_k": 64, "num_stages": 2, "num_warps": 4},
    {"block_m": 256, "block_n": 128, "block_k": 64, "num_stages": 3, "num_warps": 4},
    {"block_m": 256, "block_n": 128, "block_k": 64, "num_stages": 3, "num_warps": 8},
    {"block_m": 512, "block_n": 64, "block_k": 64, "num_stages": 3, "num_warps": 4},
]

WS_CONFIGS = [
    {"block_m": 128, "block_n": 128, "block_k": 64, "num_stages": 2, "num_warps": 4},
    {"block_m": 128, "block_n": 128, "block_k": 64, "num_stages": 3, "num_warps": 4},
    {"block_m": 128, "block_n": 128, "block_k": 128, "num_stages": 3, "num_warps": 4},
    {"block_m": 128, "block_n": 256, "block_k": 64, "num_stages": 3, "num_warps": 4},
    {"block_m": 128, "block_n": 256, "block_k": 64, "num_stages": 4, "num_warps": 4},
]


def fmt_config(config):
    return (f"{config['block_m']}x{config['block_n']}x{config['block_k']}"
            f"s{config['num_stages']}w{config['num_warps']}")


def autotune(name, configs, run, validate=None):
    best = None
    for config in configs:
        try:
            compiled = run(config)
            if validate is not None and not validate(compiled):
                print(f"      skip {name} {fmt_config(config)}: does not match required IR/PTX", flush=True)
                continue
            ms = bench(lambda config=config: run(config))
        except Exception as e:
            print(f"      skip {name} {fmt_config(config)}: {str(e).splitlines()[0]}", flush=True)
            continue
        if best is None or ms < best[0]:
            best = (ms, config)
    if best is None:
        raise RuntimeError(f"no valid configuration for {name}")
    return best
...
def validate_and_inspect():
    ...
    compiled_2cta = matmul(a, b, num_ctas=2, out=out, block_m=256, block_n=128, block_k=64, num_stages=4)
    torch.testing.assert_close(ref.to(torch.float32), out.to(torch.float32), atol=0.06, rtol=0.06)

    compiled_ws = matmul(a, b, num_ctas=2, warp_specialize=True, out=out, block_m=128, block_n=128, block_k=128)
    torch.testing.assert_close(ref.to(torch.float32), out.to(torch.float32), atol=0.06, rtol=0.06)
...
def benchmark(shapes=BENCHMARK_SHAPES):
    if not is_blackwell():
        raise RuntimeError("This tutorial requires an NVIDIA Blackwell GPU.")
...
def benchmark_precision(shapes, fp8_inputs):
    ...
    for size in shapes:
        ...
        ms_1cta, cfg_1cta = autotune(
            "1CTA",
            MATMUL_CONFIGS,
            lambda config: matmul(a, b, num_ctas=1, out=c_triton, **config),
        )
        ms_2cta, cfg_2cta = autotune(
            "2CTA",
            TWO_CTA_CONFIGS,
            lambda config: matmul(a, b, num_ctas=2, out=c_triton, **config),
            validate=uses_mmav5_two_ctas,
        )
        ms_2cta_ws, cfg_2cta_ws = autotune(
            "2CTA+WS",
            WS_CONFIGS,
            lambda config: matmul(a, b, num_ctas=2, warp_specialize=True, out=c_triton, **config),
        )
