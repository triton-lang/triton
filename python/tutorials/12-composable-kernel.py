...
**epilogue** with a host-constructed ``@triton.aggregate`` passed as one ordinary
kernel argument. Two different launch functions build two different
...
* Constructing the object on the **host** and passing it as a single kernel
  argument -- the parameter needs no annotation. Its runtime members are *lifted
  to runtime kernel parameters*, while its ``tl.constexpr`` members are baked in
  -- so changing the bias tensor reuses the same compiled kernel.
...
# object is passed as a single argument, its runtime members are lifted to kernel
# parameters automatically and its compile-time members specialize the kernel.
...
#
# The constructor is ordinary Python: the object is built on the host (we pass a
# raw ``torch.Tensor`` for the runtime ``bias`` field) and passed in as a kernel
# argument. Constructing it inside a kernel is an error.
...
# The ``epilogue`` parameter carries **no annotation** -- the *caller* specializes
# it. Triton recognizes a host-built aggregate from the value it is handed and
# puts the aggregate's class, plus its ``tl.constexpr`` members, into the kernel's
# cache key; its runtime ``bias`` member is lifted to a runtime kernel parameter
# behind the scenes. The concrete type (``BiasAdd`` or ``BiasAddScale``) is
# therefore known at compile time and the call is resolved with **no runtime
# branching**.
#
# You may still annotate the parameter with the aggregate class if you want the
# signature to document what it expects; it changes nothing about how the kernel
# is specialized.
...
def matmul_kernel(a_ptr, b_ptr, c_ptr,  #
                  ...
                  stride_bk, stride_bn,  #
                  stride_cm, stride_cn,  #
                  epilogue,  # a host-built BiasAdd / BiasAddScale aggregate
...
#
# Building the epilogue object on the host is cheap (about 0.9 µs for
# ``BiasAdd`` -- a small fraction of the launch overhead), but it is repeated on
# every call. In a tight launch loop with a fixed ``bias``, construct the
# aggregate once outside the loop and pass the same object to each launch.
...
bias_add = BiasAdd(bias, N)
bias_add_scale = BiasAddScale(bias, N, scale)

ref = (a.float() @ b.float())
run_test(ref, lambda: matmul(a, b), "None epilogue")
run_test(ref + bias, lambda: matmul(a, b, bias_add), "BiasAdd epilogue")
run_test((ref + bias) * scale, lambda: matmul(a, b, bias_add_scale), "BiasAddScale epilogue (derived)")
...
bias2_add = BiasAdd(bias2, N)
bias2_add_scale = BiasAddScale(bias2, N, 0.25)

run_test(ref + bias2, lambda: matmul(a, b, bias2_add), "BiasAdd epilogue (new bias, same kernel)")
run_test((ref + bias2) * 0.25, lambda: matmul(a, b, bias2_add_scale),
...
#   epilogue.
#
# * Building the object on the **host** and passing it as one argument -- to a
#   parameter that needs no annotation -- lifts its runtime members to kernel
#   parameters while baking in its constexpr members. A single kernel signature
#   stays clean no matter how rich the epilogue is, and changing a runtime member
