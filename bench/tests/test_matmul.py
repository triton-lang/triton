...
def init_routing_data(m, n_expts_tot, n_expts_act, n_expt_shards, do_gather, do_scatter):
    ...
    if n_expt_shards > 1:
        ...
        _, routing_data, gather_idx, scatter_idx = simulate_expert_sharded_routing(
            m, routing_data, n_expt_shards, device=logits.device)
    ...
    if do_gather and do_scatter and n_expts_act == 1 and n_expt_shards == 1:
        ...
        expt_indx = torch.argsort(-torch.softmax(logits, dim=-1), dim=1, stable=True)[:,:n_expts_act].reshape(-1).to(torch.int32)
...
def init_compute_data(m, n, k, gindx, sindx, n_expts_tot, n_expts_act, n_expt_shards, mode, act_dtype, weight_dtype, has_y_gammas, requires_grad=True, dev="cuda"):
    torch.manual_seed(0)
...
def init_precision(out_dtype, act_use_flexpoint, weight_use_flexpoint, n_expts_tot=1, mx_ctx=MicroscalingCtx()):
    # flexpoint
    make_tensor = lambda val0, val1: torch.tensor([val0, val1] * (n_expts_tot // 2) + ([val0] if n_expts_tot % 2 else []),
                                                  dtype=torch.float32,
                                                  device="cuda")
    make_scalar = lambda val: torch.tensor([val], dtype=torch.float32, device="cuda")
    in_flex_data = lambda scale, use_flex: InFlexData(dtype=torch.float8_e5m2,
                                            scale=make_scalar(scale)) if use_flex else InFlexData()
    in_flex_edata = lambda scale0, scale1, use_flex: InFlexData(dtype=torch.float8_e5m2,
                                            scale=make_tensor(scale0, scale1)) if use_flex else InFlexData()
    out_flex_data = lambda scale, use_flex: OutFlexData(dtype=torch.float8_e5m2,
                                            expected_scale=make_scalar(scale),
                                            actual_scale=make_scalar(0),
                                            checksum_scale=make_scalar(0)) if use_flex else OutFlexData()
    flex_ctx = FlexCtx(
    ...
    return PrecisionConfig(
        flex_ctx=flex_ctx,
        acc_scale=2.0 if act_use_flexpoint or weight_use_flexpoint else 1.0,
        mx_ctx=mx_ctx,
        out_dtype=out_dtype
    )
...
@pytest.mark.parametrize(
    "do_gather, do_scatter, fused_scatter",
    [
        (False, False, False),
        (True, False, False),
        (False, True, False),
...
def test_op(m, n, k, split_k, do_gather, do_scatter, fused_scatter, has_y_gammas, is_persistent, n_expts_tot, n_expts_act, n_expt_shards, mode, act_dtype_str,
            weight_dtype_str, block_m, swizzle_mx_scale):
    ...
    precision_opt = init_precision(act_dtype, act_is_float8, weight_is_float8 and not is_mixed_input, n_expts_tot//n_expt_shards)
    # precision_opt.x_pad_trans_requires_flexpoint = False
    if mode == "ragged":
        m, rdata, gindx, sindx = init_routing_data(
            m, n_expts_tot, n_expts_act, n_expt_shards, do_gather, do_scatter
        )
    else:
        rdata = gindx = sindx = None
    x_tri, w_tri, bias_tri, gs0_tri, gs1_tri = init_compute_data(
        m, n, k, gindx, sindx, n_expts_tot, n_expts_act, n_expt_shards, mode, act_dtype, #
        torch.bfloat16 if is_mixed_input else weight_dtype,
        has_y_gammas,
        requires_grad=test_bwd
    )
    x_ref, w_ref, bias_ref, gs0_ref, gs1_ref = apply_precision(
        x_tri, w_tri, bias_tri, gs0_tri, gs1_tri, precision_opt
    )

    if is_mixed_input:
        swizzle_axis = 2 if swizzle_mx_scale else None
        w_tri, mx_scales_tri, weight_scale_shape = downcast_to_mxfp(w_tri, weight_dtype, axis=1, swizzle_axis=swizzle_axis)
        w_ref = upcast_from_mxfp(w_tri, mx_scales_tri, torch.bfloat16, axis=1, swizzle_axis=swizzle_axis)

        precision_opt.mx_ctx = MicroscalingCtx(
            weight_scale=mx_scales_tri, swizzle_mx=swizzle_mx_scale,
    ...
    # triton
    tri_y = matmul_ogs(x_tri, w_tri, bias_tri, rdata, 
    ...
    ref_y = matmul_ogs_torch(x_ref, w_ref, bias_ref,  #
                         rdata, gindx, sindx,
                         round_x=round_x, round_y=round_y,
                         gammas=gs1_ref)
    ...
    if act_is_float8:
        ...
        assert (ref_y_scale - tri_y_scale).abs() < 1e-10, f"ref_y_scale: {ref_y_scale}, tri_y_scale: {tri_y_scale.item()}"