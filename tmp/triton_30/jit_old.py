...
class DependenciesFinder(ast.NodeVisitor):
    ...
    def visit_Call(self, node):
        ...
        assert (
            func is None or is_triton_builtin(func) or isinstance(func, JITFunction)
...
class JITFunction(KernelInterface[T]):
    ...
    def __init__(
        ...
        self.src = self.src[
            re.search(r"^def\s+\w+\s*\(", self.src, re.MULTILINE).start() :
        ]
    ...
    def preload(self, specialization_data):
        from triton.compiler import AttrsDescriptor, compile, ASTSource
        import json
