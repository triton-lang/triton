...
module attributes {nvws.group.epilogue = {num_warps = 4 : i32, start_warp = 0 : i32}, nvws.group.mma = {num_warps = 1 : i32, start_warp = 4 : i32}, nvws.group.tma_load = {num_warps = 1 : i32, start_warp = 5 : i32}, "nvws.warp-specialized" = true, "ttg.num-ctas" = 1 : i32, "ttg.num-stages" = 4 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  tt.func public @gemm_persistent_flattened_fp8_epilogue_subtile(%arg0: !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>>, %arg1: i32, %arg2: i32, %arg3: i64, %arg4: i64, %arg5: !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>>, %arg6: i32, %arg7: i32, %arg8: i64, %arg9: i64, %arg10: !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>>, %arg11: i32, %arg12: i32, %arg13: i64, %arg14: i64, %arg15: i32 {tt.divisibility = 16 : i32}, %arg16: i32 {tt.divisibility = 16 : i32}, %arg17: i32 {tt.divisibility = 16 : i32}) attributes {noinline = false} {
    ...
    %15:7 = scf.for %arg18 = %c0_i32 to %14 step %c1_i32 iter_args(%arg19 = %c-1_i32, %arg20 = %12, %arg21 = %c0_i32, %arg22 = %c0_i32, %arg23 = %12, %arg24 = %false, %arg25 = %token) -> (i32, i32, i32, i32, i32, i1, !ttg.async.token)  : i32 {
      ...
      %23 = tt.descriptor_load %arg0[%21#1, %22] {groups = [@nvws.group.tma_load]} : !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>> -> tensor<256x128xf8E4M3FN, #blocked>
      %24 = ttg.local_alloc %23 {groups = [@nvws.group.tma_load]} : (tensor<256x128xf8E4M3FN, #blocked>) -> !ttg.memdesc<256x128xf8E4M3FN, #shared, #smem>
      %25 = tt.descriptor_load %arg5[%21#2, %22] {groups = [@nvws.group.tma_load]} : !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>> -> tensor<256x128xf8E4M3FN, #blocked>
      %26 = ttg.local_alloc %25 {groups = [@nvws.group.tma_load]} : (tensor<256x128xf8E4M3FN, #blocked>) -> !ttg.memdesc<256x128xf8E4M3FN, #shared, #smem>
      %27 = ttg.memdesc_trans %26 {groups = [@nvws.group.mma], order = array<i32: 1, 0>} : !ttg.memdesc<256x128xf8E4M3FN, #shared, #smem> -> !ttg.memdesc<128x256xf8E4M3FN, #shared1, #smem>
      %28 = ttng.tc_gen5_mma %24, %27, %result[%arg25], %arg24, %true {groups = [@nvws.group.mma]} : !ttg.memdesc<256x128xf8E4M3FN, #shared, #smem>, !ttg.memdesc<128x256xf8E4M3FN, #shared1, #smem>, !ttg.memdesc<256x256xf32, #tmem, #ttng.tensor_memory, mutable>
      ...
      %31:2 = scf.if %29 -> (i32, !ttg.async.token) {
        ...
        %43 = tt.reshape %result_0 {groups = [@nvws.group.epilogue]} : tensor<256x256xf32, #blocked1> -> tensor<256x2x128xf32, #blocked2>
        %44 = tt.trans %43 {groups = [@nvws.group.epilogue], order = array<i32: 0, 2, 1>} : tensor<256x2x128xf32, #blocked2> -> tensor<256x128x2xf32, #blocked3>
        %outLHS, %outRHS = tt.split %44 {groups = [@nvws.group.epilogue]} : tensor<256x128x2xf32, #blocked3> -> tensor<256x128xf32, #blocked4>
        %45 = tt.fp_to_fp %outLHS {groups = [@nvws.group.epilogue]}, rounding = rtne : tensor<256x128xf32, #blocked4> -> tensor<256x128xf8E4M3FN, #blocked4>
        %46 = ttg.convert_layout %45 {groups = [@nvws.group.epilogue]} : tensor<256x128xf8E4M3FN, #blocked4> -> tensor<256x128xf8E4M3FN, #blocked>
        tt.descriptor_store %arg10[%41, %42], %46 {groups = [@nvws.group.epilogue]} : !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>>, tensor<256x128xf8E4M3FN, #blocked>
        %47 = tt.fp_to_fp %outRHS {groups = [@nvws.group.epilogue]}, rounding = rtne : tensor<256x128xf32, #blocked4> -> tensor<256x128xf8E4M3FN, #blocked4>
        %48 = ttg.convert_layout %47 {groups = [@nvws.group.epilogue]} : tensor<256x128xf8E4M3FN, #blocked4> -> tensor<256x128xf8E4M3FN, #blocked>
        %49 = arith.addi %42, %c128_i32 {groups = [@nvws.group.epilogue]} : i32
        tt.descriptor_store %arg10[%41, %49], %48 {groups = [@nvws.group.epilogue]} : !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>>, tensor<256x128xf8E4M3FN, #blocked>
