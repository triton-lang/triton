  ...
  tt.func public @gemm_persistent_nested_fp8_epilogue_subtile(%arg0: !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>>, %arg1: i32, %arg2: i32, %arg3: i64, %arg4: i64, %arg5: !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>>, %arg6: i32, %arg7: i32, %arg8: i64, %arg9: i64, %arg10: !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>>, %arg11: i32, %arg12: i32, %arg13: i64, %arg14: i64, %arg15: i32 {tt.divisibility = 16 : i32}, %arg16: i32 {tt.divisibility = 16 : i32}, %arg17: i32 {tt.divisibility = 16 : i32}) attributes {noinline = false} {
    ...
    scf.for %arg18 = %0 to %7 step %9  : i32 {
      ...
      %20:3 = scf.for %arg19 = %c0_i32 to %6 step %c1_i32 iter_args(%arg20 = %c0_i32, %arg21 = %false, %arg22 = %token) -> (i32, i1, !ttg.async.token)  : i32 {
        %28 = tt.descriptor_load %arg0[%18, %arg20] {ttg.partition = array<i32: 0>} : !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>> -> tensor<256x128xf8E4M3FN, #blocked>
        %29 = ttg.local_alloc %28 {ttg.partition = array<i32: 0>} : (tensor<256x128xf8E4M3FN, #blocked>) -> !ttg.memdesc<256x128xf8E4M3FN, #shared, #smem>
        %30 = tt.descriptor_load %arg5[%19, %arg20] {ttg.partition = array<i32: 0>} : !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>> -> tensor<256x128xf8E4M3FN, #blocked>
        %31 = ttg.local_alloc %30 {ttg.partition = array<i32: 0>} : (tensor<256x128xf8E4M3FN, #blocked>) -> !ttg.memdesc<256x128xf8E4M3FN, #shared, #smem>
        %32 = ttg.memdesc_trans %31 {order = array<i32: 1, 0>, ttg.partition = array<i32: 1>} : !ttg.memdesc<256x128xf8E4M3FN, #shared, #smem> -> !ttg.memdesc<128x256xf8E4M3FN, #shared1, #smem>
        %33 = ttng.tc_gen5_mma %29, %32, %result[%arg22], %arg21, %true {ttg.partition = array<i32: 1>} : !ttg.memdesc<256x128xf8E4M3FN, #shared, #smem>, !ttg.memdesc<128x256xf8E4M3FN, #shared1, #smem>, !ttg.memdesc<256x256xf32, #tmem, #ttng.tensor_memory, mutable>
        %34 = arith.addi %arg20, %c128_i32 {ttg.partition = array<i32: 0>} : i32
      ...
      %21 = tt.reshape %result_0 {ttg.partition = array<i32: 2>} : tensor<256x256xf32, #blocked1> -> tensor<256x2x128xf32, #blocked2>
      %22 = tt.trans %21 {order = array<i32: 0, 2, 1>, ttg.partition = array<i32: 2>} : tensor<256x2x128xf32, #blocked2> -> tensor<256x128x2xf32, #blocked3>
      %outLHS, %outRHS = tt.split %22 {ttg.partition = array<i32: 2>} : tensor<256x128x2xf32, #blocked3> -> tensor<256x128xf32, #blocked4>
      %23 = tt.fp_to_fp %outLHS {ttg.partition = array<i32: 2>}, rounding = rtne : tensor<256x128xf32, #blocked4> -> tensor<256x128xf8E4M3FN, #blocked4>
      %24 = ttg.convert_layout %23 {ttg.partition = array<i32: 2>} : tensor<256x128xf8E4M3FN, #blocked4> -> tensor<256x128xf8E4M3FN, #blocked>
      tt.descriptor_store %arg10[%18, %19], %24 {ttg.partition = array<i32: 2>} : !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>>, tensor<256x128xf8E4M3FN, #blocked>
      %25 = tt.fp_to_fp %outRHS {ttg.partition = array<i32: 2>}, rounding = rtne : tensor<256x128xf32, #blocked4> -> tensor<256x128xf8E4M3FN, #blocked4>
      %26 = ttg.convert_layout %25 {ttg.partition = array<i32: 2>} : tensor<256x128xf8E4M3FN, #blocked4> -> tensor<256x128xf8E4M3FN, #blocked>
      %27 = arith.addi %19, %c128_i32 {ttg.partition = array<i32: 2>} : i32
      tt.descriptor_store %arg10[%18, %27], %26 {ttg.partition = array<i32: 2>} : !tt.tensordesc<tensor<256x128xf8E4M3FN, #shared>>, tensor<256x128xf8E4M3FN, #blocked>
