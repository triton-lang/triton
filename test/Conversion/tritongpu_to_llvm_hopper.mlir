...
module attributes {"ttg.target" = "cuda:90", "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, "ttg.threads-per-warp" = 32 : i32} {
  tt.func @linear_to_swizzled_st_matrix_local_store(%a: tensor<64x32xf16, #linear>) {
    // CHECK-COUNT-2: nvgpu.stmatrix.x4
    //          CHECK: llvm.return
...
#linear = #ttg.linear<{register = [[0, 1], [8, 0], [0, 8], [0, 16]], lane = [[0, 2], [0, 4], [1, 0], [2, 0], [4, 0]], warp = [[16, 0], [32, 0]], block = []}>
#smem = #ttg.shared_memory
// CHECK-LABEL: linear_to_swizzled_st_matrix_local_store_fp8
module attributes {"ttg.target" = "cuda:90", "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, "ttg.threads-per-warp" = 32 : i32} {
  tt.func @linear_to_swizzled_st_matrix_local_store_fp8(%a: tensor<64x32xf8E4M3FNUZ, #linear>) {
    // CHECK-COUNT-2: nvgpu.stmatrix.x2
    //          CHECK: llvm.return
    %b = ttg.local_alloc {allocation.offset = 0 : i32} : () -> !ttg.memdesc<64x32xf8E4M3FNUZ, #shared, #smem, mutable>
    ttg.local_store %a, %b : tensor<64x32xf8E4M3FNUZ, #linear> -> !ttg.memdesc<64x32xf8E4M3FNUZ, #shared, #smem, mutable>
    tt.return
  }
}
...
#shared = #ttg.swizzled_shared<{vec = 1, perPhase = 1, maxPhase = 1, order = [1, 0]}>
#linear = #ttg.linear<{register = [[8, 0], [0, 8], [0, 16]], lane = [[0, 2], [0, 4], [1, 0], [2, 0], [4, 0]], warp = [[16, 0], [32, 0]], block = []}>
#smem = #ttg.shared_memory
// CHECK-LABEL: linear_to_swizzled_st_matrix_local_store
module attributes {"ttg.target" = "cuda:90", "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, "ttg.threads-per-warp" = 32 : i32} {
  tt.func @linear_to_swizzled_st_matrix_local_store_fp32(%a: tensor<64x32xf32, #linear>) {
    // CHECK-COUNT-2: nvgpu.stmatrix.x4
    //          CHECK: llvm.return
    %b = ttg.local_alloc {allocation.offset = 0 : i32} : () -> !ttg.memdesc<64x32xf32, #shared, #smem, mutable>
    ttg.local_store %a, %b : tensor<64x32xf32, #linear> -> !ttg.memdesc<64x32xf32, #shared, #smem, mutable>
    tt.return
  }
}
