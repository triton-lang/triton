...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 8 : i32} {
  ...
  tt.func @dot_high_precision_acc(%a: !triton_gpu.memdesc<128x128xf8E5M2, #shared, #triton_gpu.shared_memory>, %b: !triton_gpu.memdesc<128x256xf8E5M2, #shared1, #triton_gpu.shared_memory>, %c: tensor<128x256xf32, #mma>) {
    // CHECK: nvgpu.wgmma
    ...
    %m = triton_nvidia_gpu.warp_group_dot %a, %b, %c
      ...
      !triton_gpu.memdesc<128x128xf8E5M2, #shared, #triton_gpu.shared_memory> * !triton_gpu.memdesc<128x256xf8E5M2, #shared1, #triton_gpu.shared_memory> -> tensor<128x256xf32, #mma>
    tt.return
  }
}
...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 8 : i32} {
  ...
  tt.func @dot_low_precision_acc(%a: !triton_gpu.memdesc<128x128xf8E5M2, #shared, #triton_gpu.shared_memory>, %b: !triton_gpu.memdesc<128x256xf8E5M2, #shared1, #triton_gpu.shared_memory>, %c: tensor<128x256xf32, #mma>) {
    // CHECK: nvgpu.wgmma
    // CHECK-NOT: llvm.fadd
    // CHECK: nvgpu.wgmma
    // CHECK-NOT: llvm.fadd
...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 8 : i32} {
  ...
  tt.func @dot_mix_precision_acc(%a: !triton_gpu.memdesc<128x128xf8E5M2, #shared, #triton_gpu.shared_memory>, %b: !triton_gpu.memdesc<128x256xf8E5M2, #shared1, #triton_gpu.shared_memory>, %c: tensor<128x256xf32, #mma>) {
    // CHECK: nvgpu.wgmma
    // CHECK-NOT: llvm.fadd
    // CHECK: nvgpu.wgmma
    // CHECK-COUNT-128: llvm.fadd
...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 4 : i32} {
  ...
  // CHECK: nvgpu.wgmma %{{.*}}, %{{.*}} {
  tt.func @dot_zero_acc(%a: !triton_gpu.memdesc<128x64xf16, #shared, #triton_gpu.shared_memory>, %b: !triton_gpu.memdesc<64x64xf16, #shared1, #triton_gpu.shared_memory>) {
    ...
    %m = triton_nvidia_gpu.warp_group_dot %a, %b, %cst {inputPrecision = 0 : i32, maxNumImpreciseAcc = 0 : i32} :
      !triton_gpu.memdesc<128x64xf16, #shared, #triton_gpu.shared_memory> * !triton_gpu.memdesc<64x64xf16, #shared1, #triton_gpu.shared_memory> -> tensor<128x64xf32, #mma>
    tt.return
  }
}
...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 4 : i32} {
  ...
  tt.func @dot_reg_operand_A(%a: tensor<128x64xf16, #mma>, %b: !triton_gpu.memdesc<64x64xf16, #shared, #triton_gpu.shared_memory>) {
    %cst = arith.constant dense<0.000000e+00> : tensor<128x64xf32, #mma>
    ...
    %m = triton_nvidia_gpu.warp_group_dot %opA, %b, %cst { inputPrecision = 0 : i32 }:
      tensor<128x64xf16,  #triton_gpu.dot_op<{opIdx = 0, parent = #mma, kWidth = 2}>> * !triton_gpu.memdesc<64x64xf16, #shared, #triton_gpu.shared_memory> -> tensor<128x64xf32, #mma>
    tt.return
  }
}
...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 4 : i32} {
  ...
  tt.func @dot_reg_operand_A_fp8(%a: tensor<128x128xf8E5M2, #triton_gpu.dot_op<{opIdx = 0, parent = #mma, kWidth = 4}>>, %b: !triton_gpu.memdesc<128x256xf8E5M2, #shared, #triton_gpu.shared_memory>) {
    ...
    %m = triton_nvidia_gpu.warp_group_dot %a, %b, %cst { maxNumImpreciseAcc = 1073741824 : i32, inputPrecision = 0 : i32 } :
      tensor<128x128xf8E5M2, #triton_gpu.dot_op<{opIdx = 0, parent = #mma, kWidth = 4}>> * !triton_gpu.memdesc<128x256xf8E5M2, #shared, #triton_gpu.shared_memory> -> tensor<128x256xf32, #mma1>
    tt.return
  }
}
//
...
#shared = #triton_gpu.shared<{vec = 8, perPhase = 1, maxPhase = 8, order = [0, 1], CTAsPerCGA = [1, 1], CTASplitNum = [1, 1], CTAOrder = [0, 1], hasLeadingOffset = true}>
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 4 : i32} {
  tt.func @dot_reg_operand_upcast(%a_desc: !triton_gpu.memdesc<128x64xi8, #shared, #triton_gpu.shared_memory>, %b: !triton_gpu.memdesc<64x64xf16, #shared, #triton_gpu.shared_memory>, %acc: tensor<128x64xf32, #mma>) {
    %a_dotop = triton_gpu.local_load %a_desc : !triton_gpu.memdesc<128x64xi8, #shared, #triton_gpu.shared_memory> -> tensor<128x64xi8, #triton_gpu.dot_op<{opIdx = 0, parent = #mma, kWidth = 2}>>
    ...
    %res = triton_nvidia_gpu.warp_group_dot %a_casted, %b, %acc : tensor<128x64xf16, #triton_gpu.dot_op<{opIdx = 0, parent = #mma, kWidth = 2}>> * !triton_gpu.memdesc<64x64xf16, #shared, #triton_gpu.shared_memory> -> tensor<128x64xf32, #mma>
    tt.return
  }
}
...
// CHECK-COUNT-128: llvm.fadd
  tt.func @dot_zero_acc_operand(%a: !triton_gpu.memdesc<128x128xf8E5M2, #shared, #triton_gpu.shared_memory>, %b: !triton_gpu.memdesc<128x128xf8E5M2, #shared1, #triton_gpu.shared_memory>) {
    %cst = arith.constant dense<0.000000e+00> : tensor<128x128xf32, #mma>
    %m = triton_nvidia_gpu.warp_group_dot %a, %b, %cst {maxNumImpreciseAcc = 64 : i32, inputPrecision = 0 : i32} :
      !triton_gpu.memdesc<128x128xf8E5M2, #shared, #triton_gpu.shared_memory> * !triton_gpu.memdesc<128x128xf8E5M2, #shared1, #triton_gpu.shared_memory> -> tensor<128x128xf32, #mma>
    tt.return
  }
}
...
module attributes {"triton_gpu.target" = "cuda:90", "triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 4 : i32, "triton_gpu.threads-per-warp" = 32 : i32} {
  tt.func @distribute_to_shared_st_matrix(%a: tensor<128x128xf16, #mma>) {
    // CHECK-COUNT-16: nvgpu.stmatrix
    //          CHECK: llvm.return
    %b = triton_gpu.local_alloc %a {allocation.offset = 0 : i32} : (tensor<128x128xf16, #mma>) -> !triton_gpu.memdesc<128x128xf16, #shared, #triton_gpu.shared_memory, mutable>
    tt.return
  }
}
