...
// Check that a barrier IS inserted between local_load and async_tdm_gather
// when the loop-carried token does not come from async_tdm_wait.

#AL_TDM_UNSYNC = #ttg.blocked<{sizePerThread = [1, 4], threadsPerWarp = [4, 8], warpsPerCTA = [4, 1], order = [1, 0]}>
#TDM_SHARED_UNSYNC = #ttg.swizzled_shared<{vec = 1, perPhase = 1, maxPhase = 1, order = [1, 0]}>

module attributes {"ttg.num-warps" = 4 : i32, "ttg.num-ctas" = 1 : i32, ttg.target = "hip:gfx1250", "ttg.threads-per-warp" = 32 : i32} {
// CHECK-LABEL: tdm_gather_token_not_from_async_wait
tt.func @tdm_gather_token_not_from_async_wait(
    %tensorDesc: !tt.tensordesc<tensor<16x16xf16>>,
    %row_indices: tensor<16xi32>, %loopIterCount: i32
) {
  %c0_i32 = arith.constant 0 : i32
  %c1_i32 = arith.constant 1 : i32
  %alloc = ttg.local_alloc : () -> !ttg.memdesc<16x16xf16, #TDM_SHARED_UNSYNC, #ttg.shared_memory, mutable>

  %1 = amdg.async_tdm_gather %tensorDesc[%row_indices, %c0_i32] to %alloc : tensor<16xi32>, !ttg.memdesc<16x16xf16, #TDM_SHARED_UNSYNC, #ttg.shared_memory, mutable> -> !tt.tensordesc<tensor<16x16xf16>>
  %2 = amdg.async_tdm_wait %1 {num = 0 : i32}

  %loop_result:1 = scf.for %arg14 = %c0_i32 to %loopIterCount step %c1_i32 iter_args(%arg10 = %2) -> (!ttg.async.token)  : i32 {
    %6 = ttg.local_load %alloc token %arg10 : !ttg.memdesc<16x16xf16, #TDM_SHARED_UNSYNC, #ttg.shared_memory, mutable> -> tensor<16x16xf16, #AL_TDM_UNSYNC>
    // CHECK: ttg.local_load
    // CHECK: ttg.barrier local
    // CHECK: amdg.async_tdm_gather
    %7 = amdg.async_tdm_gather %tensorDesc[%row_indices, %c0_i32] to %alloc : tensor<16xi32>, !ttg.memdesc<16x16xf16, #TDM_SHARED_UNSYNC, #ttg.shared_memory, mutable> -> !tt.tensordesc<tensor<16x16xf16>>
    scf.yield %7: !ttg.async.token
  }
  // CHECK: tt.return
  tt.return
