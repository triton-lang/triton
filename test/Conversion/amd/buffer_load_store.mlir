// RUN: triton-opt %s -split-input-file --convert-triton-amdgpu-to-llvm=gfx-arch=gfx942 | FileCheck %s --check-prefixes=CHECK,ATOMIC
// RUN: triton-opt %s -split-input-file --convert-triton-amdgpu-to-llvm=gfx-arch=gfx950 | FileCheck %s --check-prefixes=CHECK,ATOMIC
// RUN: triton-opt %s -split-input-file --convert-triton-amdgpu-to-llvm=gfx-arch=gfx1100 | FileCheck %s --check-prefixes=CHECK,ATOMIC
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 1 : i32} {
    ...
    tt.func @buffer_atomic_rmw_fadd(%arg0: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %offset : tensor<128xi32, #blocked0>{tt.divisibility=16:i32}, %N: i32, %values : tensor<128xf32, #blocked0>, %stride: i32 {tt.divisibility=16:i32}) {
        ...
        // CHECK: %[[mask0:.*]] = llvm.extractvalue %{{.*}} : !llvm.struct<(i1, i1, i1, i1)>
        // GFX1170: %[[FLAGS:.*]] = llvm.mlir.constant(822243328 : i32) : i32
        // GFX1170: rocdl.make.buffer.rsrc {{.*}}, %[[FLAGS]]
        // There should be a single release fence before any atomics
        // CHECK: llvm.fence syncscope("agent") release
        // CHECK: %[[mask1:.*]] = llvm.mlir.constant(true) : i1
        // CHECK: %[[mask2:.*]] = llvm.and %[[mask1]], %[[mask0]]
        // CHECK: %[[offset:.*]] = llvm.select %[[mask2]]
        // GFX1170-NEXT: %[[SOFFSET:.*]] = llvm.mlir.constant(0 : i32) : i32
        // GFX1170-NEXT: %[[CPOL:.*]] = llvm.mlir.constant(0 : i32) : i32
        // GFX1170-NEXT: %[[result:.*]] = llvm.call_intrinsic "llvm.amdgcn.raw.ptr.buffer.atomic.fadd"({{.*}}, {{.*}}, %[[offset]], %[[SOFFSET]], %[[CPOL]]) : (f32, !llvm.ptr<8>, i32, i32, i32) -> f32
        // GFX1170-COUNT-3: llvm.call_intrinsic "llvm.amdgcn.raw.ptr.buffer.atomic.fadd"
        // GFX1170-NOT: llvm.call_intrinsic "llvm.amdgcn.raw.ptr.buffer.atomic.fadd"
        ...
        // ATOMIC-COUNT-4: llvm.call_intrinsic "llvm.amdgcn.raw.ptr.buffer.atomic.fadd"({{.*}}) : (f32, !llvm.ptr<8>, i32, i32, i32) -> f32
