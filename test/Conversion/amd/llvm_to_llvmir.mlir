// RUN: triton-opt %s -o - --mlir-print-debuginfo --convert-to-llvm | mlir-translate --mlir-to-llvmir | FileCheck %s

// CHECK: call i32 @llvm.amdgcn.workgroup.id.x()
// CHECK: declare noundef i32 @llvm.amdgcn.workgroup.id.x()
// CHECK: !DILocation
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.shared = 0 : i32, ttg.target = "hip:gfx942", "ttg.threads-per-warp" = 64 : i32} {
  ...
  llvm.func @add_kernel(%arg0: !llvm.ptr<1> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32} loc("vector-add.py":30:0), %arg1: !llvm.ptr<1> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32} loc("vector-add.py":30:0), %arg2: !llvm.ptr<1> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32} loc("vector-add.py":30:0), %arg3: i32 {tt.divisibility = 16 : i32} loc("vector-add.py":30:0), %arg4: !llvm.ptr<1> loc("vector-add.py":30:0)) attributes {noinline = false, nvvm.kernel = 1 : ui1, nvvm.reqntid = array<i32: 128>} {
...
#di_file = #llvm.di_file<"01-vector-add.py" in "/python/tutorials">
