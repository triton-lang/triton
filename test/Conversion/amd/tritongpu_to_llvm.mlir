...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32} {
  ...
  tt.func @atomicrmw_scope_memsemantics(%arg0 : tensor<128x!tt.ptr<f32>, #blocked0>, %arg1 : tensor<128xi1, #blocked0>, %arg2 : tensor<128xf32, #blocked0>) {
    // relaxed
    // CHECK: llvm.atomicrmw {{.*}}, {{.*}} syncscope({{"system"}}) monotonic
    ...
    // acquire
    // CHECK: llvm.atomicrmw {{.*}}, {{.*}} syncscope({{"system"}}) acquire
    ...
    // release
    // CHECK: llvm.atomicrmw {{.*}}, {{.*}} syncscope({{"system"}}) release
    ...
    // acq_rel
    // CHECK: llvm.atomicrmw {{.*}}, {{.*}} syncscope({{"system"}}) acq_rel
