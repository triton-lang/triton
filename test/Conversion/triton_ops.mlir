...
tt.func @load_store_ops_scalar(%ptr: !tt.ptr<f32> {tt.divisibility = 16 : i32}, %mask : i1) {
  ...
  // load scalar
  // CHECK: %[[L0:.*]] = tt.load %{{.*}} : !tt.ptr<f32, 1>
  %a = tt.load %ptr : !tt.ptr<f32, 1>
  // CHECK: %[[L1:.*]] = tt.load %{{.*}}, %{{.*}} : !tt.ptr<f32, 1>
  %b = tt.load %ptr, %mask : !tt.ptr<f32, 1>
  // CHECK: %[[L2:.*]] = tt.load %{{.*}}, %{{.*}}, %{{.*}} : !tt.ptr<f32, 1>
  %c = tt.load %ptr, %mask, %other : !tt.ptr<f32, 1>

  // store scalar
  // CHECK: tt.store %{{.*}}, %[[L0]] : !tt.ptr<f32, 1>
  tt.store %ptr, %a : !tt.ptr<f32, 1>
  // CHECK: tt.store %{{.*}}, %[[L1]], %{{.*}} : !tt.ptr<f32, 1>
  tt.store %ptr, %b, %mask : !tt.ptr<f32, 1>
  // CHECK: tt.store %{{.*}}, %[[L2]], %{{.*}} : !tt.ptr<f32, 1>
  tt.store %ptr, %c, %mask : !tt.ptr<f32, 1>
  tt.return
}
...
tt.func @reduce_ops_infer(%ptr: !tt.ptr<f32>, %v : tensor<1x2x4xf32>) {
  ...
  tt.store %ptr1x2, %c : tensor<1x2x!tt.ptr<f32, 1>>
  tt.store %ptr1, %e : tensor<1x!tt.ptr<f32, 1>>
  tt.store %ptr, %g : !tt.ptr<f32, 1>
  tt.return
}
...
tt.func @dot_ops_infer(%ptr: !tt.ptr<f32>, %v : f32) {
  ...
  tt.store %ptr128x128, %r1 : tensor<128x128x!tt.ptr<f32, 1>>
  tt.store %ptr32x32, %r2 : tensor<32x32x!tt.ptr<f32, 1>>
  tt.store %ptr128x128, %r3 : tensor<128x128x!tt.ptr<f32, 1>>
  tt.store %ptr1x1, %r4 : tensor<1x1x!tt.ptr<f32, 1>>
  tt.return
}
...
// CHECK: tt.print "test"
  ...
  %0 = tt.load %arg0 : !tt.ptr<f32, 1>
  tt.store %arg0, %0 : !tt.ptr<f32, 1>
  tt.return
}
...
tt.func @scan_op(%ptr: tensor<1x2x4x!tt.ptr<f32>>, %v : tensor<1x2x4xf32>) {
  ...
  tt.store %ptr, %a : tensor<1x2x4x!tt.ptr<f32, 1>>
  tt.return
}
