...
// CHECK-LABEL: @barrier_init
llvm.func @barrier_init() {
  // CHECK: ttg.warp_specialize
  ttg.warp_specialize() attributes {warpGroupStartIds = array<i32: 4, 8, 10>}
  // CHECK: default
  default {
    // CHECK: "use"()
    "use"() : () -> ()
  ...
  partition0() num_warps(4) {
    ...
    %2 = ttg.local_alloc {allocation.offset = 98368 : i32} : () -> !ttg.memdesc<3x1xi64, #shared, #smem, mutable>
    %3 = ttg.memdesc_index %2[%c0_i32] : !ttg.memdesc<3x1xi64, #shared, #smem, mutable> -> !ttg.memdesc<1xi64, #shared, #smem, mutable>

    // CHECK: ttng.init_barrier %{{.*}}, 64
    ttng.init_barrier %3, 3 {dependentPartitionIds = array<i32: 1>} : !ttg.memdesc<1xi64, #shared, #smem, mutable>
    ttg.warp_return
  }
  // CHECK: partition1
  partition1() num_warps(2) {
    ...
    ttng.init_barrier %3, 3 {dependentPartitionIds = array<i32: 0>} : !ttg.memdesc<1xi64, #shared, #smem, mutable>
    ttg.warp_return
  }
  // CHECK: partition2
  partition2() num_warps(2) {
    ...
    // CHECK: ttng.init_barrier %{{.*}}, 192
    ttng.init_barrier %3, 3 {dependentPartitionIds = array<i32: 0, 1>} : !ttg.memdesc<1xi64, #shared, #smem, mutable>
    ttg.warp_return
  } : () -> ()
  llvm.return
}
