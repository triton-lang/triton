...
#tmem_wide = #ttng.tensor_memory_encoding<blockM = 128, blockN = 128, colStride = 1, CGALayout = [[0, 0]]>
...
module attributes {"ttg.target" = "cuda:103", "ttg.num-ctas" = 2 : i32, "ttg.num-warps" = 4 : i32, "ttg.total-num-warps" = 8 : i32, ttg.tensor_memory_size = 128 : i32, "ttng.two-ctas" = true} {
  ...
  // CHECK-LABEL: @tmem_unaligned_subslice
  // CHECK: llvm.mlir.constant(208 : i32)
  // CHECK: llvm.add
  tt.func private @tmem_unaligned_subslice(%parent: !ttg.memdesc<128x512xf32, #tmem_wide, #ttng.tensor_memory, mutable>) -> !ttg.memdesc<128x256xf32, #tmem_wide, #ttng.tensor_memory, mutable, 128x512> attributes {"ws_num_warps" = 4 : i32} {
    %view = ttng.tmem_subslice %parent {offset = 208 : i32} : !ttg.memdesc<128x512xf32, #tmem_wide, #ttng.tensor_memory, mutable> -> !ttg.memdesc<128x256xf32, #tmem_wide, #ttng.tensor_memory, mutable, 128x512>
    tt.return %view : !ttg.memdesc<128x256xf32, #tmem_wide, #ttng.tensor_memory, mutable, 128x512>
  }

  // CHECK-LABEL: @tmem_scales_subslice
  // CHECK: llvm.mlir.constant(4 : i32)
  // CHECK: llvm.add
  tt.func private @tmem_scales_subslice(%parent: !ttg.memdesc<128x8xi8, #tmem_scales, #ttng.tensor_memory, mutable>) -> !ttg.memdesc<128x4xi8, #tmem_scales, #ttng.tensor_memory, mutable, 128x8> attributes {"ws_num_warps" = 4 : i32} {
    %view = ttng.tmem_subslice %parent {offset = 4 : i32} : !ttg.memdesc<128x8xi8, #tmem_scales, #ttng.tensor_memory, mutable> -> !ttg.memdesc<128x4xi8, #tmem_scales, #ttng.tensor_memory, mutable, 128x8>
    tt.return %view : !ttg.memdesc<128x4xi8, #tmem_scales, #ttng.tensor_memory, mutable, 128x8>
  }
}
