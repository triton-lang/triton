...
#blocked0 = #triton_gpu.blocked<{sizePerThread = [1, 4], threadsPerWarp = [8, 4], warpsPerCTA = [1, 1], order = [1, 0]}>
#shared0 = #triton_gpu.shared<{vec = 1, perPhase=2, maxPhase=8 ,order = [1, 0]}>
#mma0 = #triton_gpu.mma<{version=1, warpsPerCTA=[1,1]}>
#dot_operand_a = #triton_gpu.dot_op<{opIdx=0, parent=#mma0}>
#dot_operand_b = #triton_gpu.dot_op<{opIdx=1, parent=#mma0}>
module attributes {"triton_gpu.num-warps" = 1 : i32} {
  // CHECK-LABEL: convert_dot
  func @convert_dot_mma884(%A: tensor<16x16xf16, #shared0>, %B: tensor<16x16xf16, #shared0>) {
    %cst0 = arith.constant dense<0.000000e+00> : tensor<16x16xf32, #mma0>
    %a_mat = triton_gpu.convert_layout %a : (tensor<16x16xf16, #shared0>) -> tensor<16x16xf16, #dot_operand_a>
    %b_mat = triton_gpu.convert_layout %b : (tensor<16x16xf16, #shared0>) -> tensor<16x16xf16, #dot_operand_b>
    ...
    // CHECK: llvm.inline_asm
    // CHECK-SAME: mma.sync.aligned.m16n8k16.row.col.f32.f16.f16.f32
    %D = tt.dot %AA, %BB, %cst0 {allowTF32 = true, transA = false, transB = false} : tensor<16x16xf16, #dot_operand_a> * tensor<16x16xf16, #dot_operand_b> -> tensor<16x16xf32, #mma0>
