...
module attributes {"ttg.num-warps" = 4 : i32, ttg.target = "cuda:100"} {
  ...
  tt.func @warp_specialize_tma_matmul(%arg0: i32, %arg1: i32, %arg2: i32, %arg3: !tt.tensordesc<tensor<128x64xf16, #shared>>, %arg4: !tt.tensordesc<tensor<128x64xf16, #shared>>) {
    ...
    %1 = scf.for %arg5 = %c0_i32 to %arg0 step %c1_i32 iter_args(%arg6 = %0) -> (!ttg.async.token)  : i32 {
      ...
      // CHECK: [[PUT_BUF1:%.*]] = nvws.aref.put.enter [[AREF1]][[[C_ZERO1]], [[C_ZERO1]]] {aref_tag = "aref_0", loop.cluster = 1 : i32, loop.stage = 0 : i32, ttg.partition = 2 : i32}
      // CHECK-NEXT: nvws.descriptor_load {{.*}} 16384 [[PUT_BUF1]]
      // CHECK: [[C_ZERO2:%.*]] = arith.constant {ttg.partition = 2 : i32} 0
      // CHECK: nvws.aref.put.exit [[AREF1]][[[C_ZERO2]]] [#nvws.async_op<tma_load>] {aref_tag = "aref_0", loop.cluster = 1 : i32, loop.stage = 0 : i32, ttg.partition = 2 : i32}
      // CHECK: [[C_ZERO3:%.*]] = arith.constant {ttg.partition = 1 : i32} 0
      // CHECK: [[GET_BUF1:%.*]] = nvws.aref.get.enter [[AREF1]][[[C_ZERO3]], [[C_ZERO3]]] {aref_tag = "aref_0", loop.cluster = 0 : i32, loop.stage = 1 : i32, ttg.partition = 1 : i32}
      ...
      // CHECK: nvws.aref.get.exit [[AREF1]][[[C_ZERO4]]] [#nvws.async_op<tc5mma>] {aref_tag = "aref_0", loop.cluster = 0 : i32, loop.stage = 1 : i32, ttg.partition = 1 : i32}
  ...
  tt.func @load_used_as_reg_and_smem(%arg0: !tt.tensordesc<tensor<128x64xf16, #shared>>, %arg1: i32) {
    ...
    scf.for %arg2 = %c0_i32 to %arg1 step %c1_i32  : i32 {
      ...
      // CHECK: "use1"([[REG]])
      // CHECK: "use2"([[GET_BUF2]])
      // CHECK: nvws.aref.get.exit {{.*}} [#nvws.async_op<none>] {aref_tag = "aref_0", loop.cluster = 0 : i32, loop.stage = 1 : i32, ttg.partition = 1 : i32}
