...
tt.func @default_stages(%lb : index, %ub : index, %step : index,
                  %a_ptr_init : tensor<128x32x!tt.ptr<f16>, #AL> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 32] : tensor<2xi32>},
                  %b_ptr_init : tensor<32x128x!tt.ptr<f16>, #BL> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 32] : tensor<2xi32>}) -> tensor<128x128xf32, #C> {
...
tt.func @load_into_shared(%lb : index, %ub : index, %step : index,
                  %a_ptr_init : tensor<128x32x!tt.ptr<f16>, #AL> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 32] : tensor<2xi32>},
                  %b_ptr_init : tensor<32x128x!tt.ptr<f16>, #BL> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 32] : tensor<2xi32>}) -> tensor<128x128xf32, #mma> {
...
tt.func @indirect_load(%lb : index, %ub : index, %step : index,
                  %a_ind_ptr_init : tensor<128x32x!tt.ptr<i32>, #AL> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 32] : tensor<2xi32>},
                  %b_ind_ptr_init : tensor<32x128x!tt.ptr<i32>, #BL> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 32] : tensor<2xi32>},
                  %a_ptr_init : tensor<128x32x!tt.ptr<f16>, #AL> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 32] : tensor<2xi32>},
                  %b_ptr_init : tensor<32x128x!tt.ptr<f16>, #BL> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 32] : tensor<2xi32>}) -> tensor<128x128xf32, #C> {
  %c_init = arith.constant dense<0.00e+00> : tensor<128x128xf32, #C>
...
tt.func @per_loop_stages(%lb : index, %ub : index, %step : index,
                  %a_ptr_init : tensor<128x32x!tt.ptr<f16>, #AL> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 32] : tensor<2xi32>},
                  %b_ptr_init : tensor<32x128x!tt.ptr<f16>, #BL> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 32] : tensor<2xi32>}) -> (tensor<128x128xf32, #C>, tensor<128x128xf32, #C>) {
  %c_init = arith.constant dense<0.00e+00> : tensor<128x128xf32, #C>
  %a_off = arith.constant dense<4> : tensor<128x32xi32, #AL>
  %b_off = arith.constant dense<4> : tensor<32x128xi32, #BL>
...
tt.func @non_dot_pipeline(%lb : index, %ub : index, %step : index,
                  %a_ptr_init : tensor<128x32x!tt.ptr<f16>, #AL> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 32] : tensor<2xi32>}) -> tensor<128x32xf16, #A> {
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @select_after_mma(%arg0: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>}, %arg1: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>}, %arg2: i32) -> tensor<128x128xf16, #blocked1> {
    %true = arith.constant true
...
tt.func @tc_gen5_mma_scaled_tmem_scales(%lb : index, %ub : index, %step : index,
                  %A_ptr: tensor<128x128x!tt.ptr<f16>, #blocked1> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                  %B_ptr: tensor<128x128x!tt.ptr<f16>, #blocked1> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                  %A_sc_ptr: tensor<128x8x!tt.ptr<i8>, #scales> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                  %B_sc_ptr: tensor<128x8x!tt.ptr<i8>, #scales> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                  %acc_init : tensor<128x128xf32, #blocked1>) -> () {
  %true = arith.constant true
  %acc_tm = ttng.tmem_alloc %acc_init : (tensor<128x128xf32, #blocked1>) -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
  scf.for %iv = %lb to %ub step %step : index {
