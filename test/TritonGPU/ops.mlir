...
// CHECK-DAG: [[$LINEAR_SLICE:#.*]] = #ttg.linear
#linear_slice = #ttg.linear<{register = [[16], [32], [64]], lane = [[0], [0], [0], [1], [2]], warp = [[4], [8]], block = []}>
...
tt.func @split_join_linear_mix(%arg: tensor<128x2xf32, #linear>) attributes {"ttg.num-warps" = 4 : i32} {
  // CHECK-NEXT: tt.split %{{.*}} : tensor<128x2xf32, [[$LINEAR]]> -> tensor<128xf32, [[$LINEAR_SLICE]]>
  %lhs, %rhs = tt.split %arg : tensor<128x2xf32, #linear> -> tensor<128xf32, #linear_slice>
  // CHECK-NEXT: tt.join %{{.*}}, %{{.*}} : tensor<128xf32, [[$LINEAR_SLICE]]> -> tensor<128x2xf32, [[$LINEAR]]>
  %j = tt.join %lhs, %rhs : tensor<128xf32, #linear_slice> -> tensor<128x2xf32, #linear>
  tt.return
}
}
