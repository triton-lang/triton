...
// RUN: triton-opt %s -split-input-file -tritonamdgpu-schedule-loops="num_stages=2" -tritonamdgpu-stream-pipeline -canonicalize | FileCheck %s --check-prefixes=COMMON,AMD
// RUN: triton-opt %s -split-input-file -tritonamdgpu-schedule-loops="num_stages=3 global_prefetch=1 local_prefetch=1" -tritonamdgpu-stream-pipeline -canonicalize | FileCheck %s --check-prefixes=COMMON,AMD_PREFETCH
...
// CHECK: %[[NEXT_BUFFER_1:.*]] = tt.addptr %{{.*}}, {{.*}}
// CHECK: ttg.async_copy_global_to_local %[[NEXT_BUFFER_1]]
// CHECK: %[[IND_BUFFER_0:.*]] = ttg.memdesc_index {{.*}} : !ttg.memdesc<1x16xi64, #[[$SHARED_LAYOUT]], #smem, mutable> -> !ttg.memdesc<16xi64, #[[$SHARED_LAYOUT]], #smem, mutable, 1x16>
