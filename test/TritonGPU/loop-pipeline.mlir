...
// CHECK: %[[NEXT_BUFFER_1:.*]] = tt.addptr %{{.*}}, {{.*}}
// CHECK: %[[OUTER_LOAD:.*]] = tt.load %[[NEXT_BUFFER_1]]
// CHECK: tt.addptr %{{.*}}, {{.*}}
// CHECK: %[[NEXT_BUFFER_2:.*]] = tt.addptr %{{.*}}, {{.*}}
// CHECK: %[[SPLAT_1:.*]] = tt.splat %{{.*}}
// CHECK: scf.for
// CHECK:   %[[LOAD_1:.*]] = tt.load %[[NEXT_BUFFER_2]]
// CHECK:   %[[NEXT_BUFFER_3:.*]] = tt.addptr %[[SPLAT_1]], %[[LOAD_1]]
// CHECK:   tt.load %[[NEXT_BUFFER_3]]
// CHECK:   %[[CONVERT_LAYOUT_1:.*]] = triton_gpu.convert_layout %[[OUTER_LOAD]]
// CHECK:   %[[CONVERT_LAYOUT_2:.*]] = triton_gpu.convert_layout
// CHECK:   %[[DOT:.*]] = tt.dot %[[CONVERT_LAYOUT_1]], %[[CONVERT_LAYOUT_2]]
// CHECK:   triton_gpu.convert_layout %[[DOT]]
...
module attributes {"triton_gpu.compute-capability" = 80 : i32, "triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 2 : i32, "triton_gpu.threads-per-warp" = 32 : i32} {
  tt.func @matmul_indirect_pipeline(%arg0: !tt.ptr<f32, 1> {tt.divisibility = 16 : i32}, %arg1: !tt.ptr<i64, 1> {tt.divisibility = 16 : i32}, %arg2: !tt.ptr<f32, 1> {tt.divisibility = 16 : i32}, %arg3: !tt.ptr<f32, 1> {tt.divisibility = 16 : i32}) attributes {noinline = false} {
    %cst = arith.constant dense<0.000000e+00> : tensor<32x32xf32, #mma>
    %c1_i32 = arith.constant 1 : i32
    %c2_i32 = arith.constant 2 : i32
    %c0_i32 = arith.constant 0 : i32
    ...
    scf.for %arg4 = %c0_i32 to %c2_i32 step %c1_i32  : i32 {
      ...
      %23 = tt.dot %21, %22, %cst {allowTF32 = true, maxNumImpreciseAcc = 0 : i32} : tensor<32x32xf32, #triton_gpu.dot_op<{opIdx = 0, parent = #mma, kWidth = 1}>> * tensor<32x32xf32, #triton_gpu.dot_op<{opIdx = 1, parent = #mma, kWidth = 1}>> -> tensor<32x32xf32, #mma>
