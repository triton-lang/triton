...
//       AMD:   %{{.*}}:6 = scf.for %[[ARG5:.*]] = %{{.*}} to %[[UB1]] step %{{.*}} iter_args(%[[ARG6:.*]] = %{{.*}}, %[[ARG7:.*]] = %{{.*}}, %[[ARG8:.*]] = %{{.*}}, %[[ARG9:.*]] = %{{.*}}, %[[ARG10:.*]] = %{{.*}}, %[[ARG11:.*]] = %{{.*}})
...
//       AMD:   }
//       AMD:   %[[CMPI_21:.*]] = arith.cmpi slt, %{{.*}}, %{{.*}}
//       AMD:   %[[SELECT_22:.*]] = arith.select %[[CMPI_21]], %{{.*}}, %{{.*}}-1
//       AMD:   %[[SUBI_23:.*]] = arith.subi %{{.*}}, %{{.*}}
//       AMD:   %[[ADDI_24:.*]] = arith.addi %[[SUBI_23]], %{{.*}}
//       AMD:   %[[ADDI_25:.*]] = arith.addi %[[ADDI_24]], %[[SELECT_22]]
//       AMD:   %[[DIVSI_26:.*]] = arith.divsi %[[ADDI_25]], %{{.*}}
//       AMD:   %[[ADDI_27:.*]] = arith.addi %[[DIVSI_26]], %{{.*}}-1
//       AMD:   %[[CMPI_28:.*]] = arith.cmpi sge, %[[ADDI_27]], %{{.*}}
//       AMD:   %[[LOCAL_LOAD_29:.*]] = triton_gpu.local_load %{{.*}}#4
//       AMD:   %[[LOCAL_LOAD_30:.*]] = triton_gpu.local_load %{{.*}}#5
//       AMD:   %[[MULF_31:.*]] = arith.mulf %[[LOCAL_LOAD_30]], %{{.*}}
//       AMD:   %[[IF_32:.*]] = scf.if %[[CMPI_28]]
//       AMD:     %[[DOT_34:.*]] = tt.dot %[[LOCAL_LOAD_29]], %[[MULF_31]], %{{.*}}#2
//       AMD:     scf.yield %[[DOT_34]]
//       AMD:   } else {
//       AMD:     scf.yield %{{.*}}#2
//       AMD:   }
//       AMD:   %[[SELECT_33:.*]] = arith.select %[[CMPI_28]], %[[IF_32]], %{{.*}}#2
...
//       AMD:   %[[ADDPTR_25:.*]] = tt.addptr %{{.*}}#1, %{{.*}}
//       AMD:   %[[SPLAT_26:.*]] = tt.splat %[[CMPI_24]]
//       AMD:   %[[LOAD_27:.*]] = tt.load %[[ADDPTR_25]], %[[SPLAT_26]]
//       AMD:   %[[LOCAL_LOAD_28:.*]] = triton_gpu.local_load %{{.*}}#4
//       AMD:   %[[EXPAND_DIMS_29:.*]] = tt.expand_dims %{{.*}}#5 {axis = 1 : i32}
//       AMD:   %[[BROADCAST_30:.*]] = tt.broadcast %[[EXPAND_DIMS_29]]
//       AMD:   %[[MULI_31:.*]] = arith.muli %{{.*}}, %[[BROADCAST_30]]
//       AMD:   %[[ADDPTR_32:.*]] = tt.addptr %{{.*}}, %[[MULI_31]]
//       AMD:   %[[SPLAT_33:.*]] = tt.splat %[[CMPI_24]]
//       AMD:   %[[LOAD_34:.*]] = tt.load %[[ADDPTR_32]], %[[SPLAT_33]]
//       AMD:   %[[LOCAL_LOAD_35:.*]] = triton_gpu.local_load %{{.*}}#6
//       AMD:   %[[IF_27:.*]] = scf.if %[[CMPI_22]]
//       AMD:     %[[DOT_47:.*]] = tt.dot %[[LOCAL_LOAD_28]], %[[LOCAL_LOAD_35]], %{{.*}}#0
...
//       AMD:   triton_gpu.local_store %[[LOAD_27]], %[[MEMDESC_SUBVIEW_40]]
//       AMD:   %[[MEMDESC_SUBVIEW_41:.*]] = triton_gpu.memdesc_subview %{{.*}}[%[[SELECT_39]], %{{.*}}, %{{.*}}]
//       AMD:   triton_gpu.local_store %[[LOAD_34]], %[[MEMDESC_SUBVIEW_41]]
