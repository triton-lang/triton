...
#blocked2d = #ttg.blocked<{sizePerThread = [1, 1], threadsPerWarp = [1, 32], warpsPerCTA = [1, 4], order = [1, 0]}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, "ttg.threads-per-warp" = 32 : i32} {
  tt.func @require_sliced_non_slice(%arg0: tensor<128xi32, #blocked1d>) {
    // expected-error @+1 {{require_sliced reshape expects a matching slice encoding}}
    %0 = tt.reshape %arg0 require_sliced : tensor<128xi32, #blocked1d> -> tensor<1x128xi32, #blocked2d>
    tt.return
  }
}
