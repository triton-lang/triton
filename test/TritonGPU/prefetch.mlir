...
tt.func @matmul_loop_mixed(%lb : index, %ub : index, %step : index, %A : !tt.ptr<f8E5M2>, %B : !tt.ptr<f16>) -> tensor<128x128xf32, #C>{
  ...
  %a_init = triton_gpu.local_alloc %a_ : (tensor<128x32xf8E5M2, #AL>) -> !triton_gpu.memdesc<128x32xf8E5M2, #A, #triton_gpu.shared_memory>
  %b_ = tt.load %b_ptr_init, %b_mask, %b_other : tensor<32x128x!tt.ptr<f16>, #BL>
  %b_init = triton_gpu.local_alloc %b_ : (tensor<32x128xf16, #BL>) -> !triton_gpu.memdesc<32x128xf16, #B, #triton_gpu.shared_memory>

  %loop:5 = scf.for %iv = %lb to %ub step %step iter_args(%a_ptr = %a_ptr_init, %b_ptr = %b_ptr_init, %a = %a_init, %b = %b_init, %prev_c = %c_init) -> (tensor<128x32x!tt.ptr<f8E5M2>, #AL>, tensor<32x128x!tt.ptr<f16>, #BL>, !triton_gpu.memdesc<128x32xf8E5M2, #A, #triton_gpu.shared_memory>, !triton_gpu.memdesc<32x128xf16, #B, #triton_gpu.shared_memory>, tensor<128x128xf32, #C>) {
    %a_op_ = triton_gpu.local_load %a : !triton_gpu.memdesc<128x32xf8E5M2, #A, #triton_gpu.shared_memory> -> tensor<128x32xf8E5M2, #A_OP>
    %a_op = tt.fp_to_fp %a_op_ : tensor<128x32xf8E5M2, #A_OP> -> tensor<128x32xf16, #A_OP>
    %b_op = triton_gpu.local_load %b : !triton_gpu.memdesc<32x128xf16, #B, #triton_gpu.shared_memory> -> tensor<32x128xf16, #B_OP>
    ...
    %next_a = triton_gpu.local_alloc %next_a_ : (tensor<128x32xf8E5M2, #AL>) -> !triton_gpu.memdesc<128x32xf8E5M2, #A, #triton_gpu.shared_memory>
    %next_b_ = tt.load %next_b_ptr, %b_mask, %b_other : tensor<32x128x!tt.ptr<f16>, #BL>
    %next_b = triton_gpu.local_alloc %b_ : (tensor<32x128xf16, #BL>) -> !triton_gpu.memdesc<32x128xf16, #B, #triton_gpu.shared_memory>

    scf.yield %next_a_ptr, %next_b_ptr, %next_a, %next_b, %c : tensor<128x32x!tt.ptr<f8E5M2>, #AL>, tensor<32x128x!tt.ptr<f16>, #BL>, !triton_gpu.memdesc<128x32xf8E5M2, #A, #triton_gpu.shared_memory>, !triton_gpu.memdesc<32x128xf16, #B, #triton_gpu.shared_memory>, tensor<128x128xf32, #C>
  }
...
tt.func @matmul_loop_mixed(%lb : index, %ub : index, %step : index, %A : !tt.ptr<f8E5M2>, %B : !tt.ptr<f16>) -> tensor<128x128xf32, #C>{
  ...
  %a_init = triton_gpu.local_alloc %a_ : (tensor<128x16xf8E5M2, #AL>) -> !triton_gpu.memdesc<128x16xf8E5M2, #A, #triton_gpu.shared_memory>
  %b_ = tt.load %b_ptr_init, %b_mask, %b_other : tensor<16x128x!tt.ptr<f16>, #BL>
  %b_init = triton_gpu.local_alloc %b_ : (tensor<16x128xf16, #BL>) -> !triton_gpu.memdesc<16x128xf16, #B, #triton_gpu.shared_memory>

  %loop:5 = scf.for %iv = %lb to %ub step %step iter_args(%a_ptr = %a_ptr_init, %b_ptr = %b_ptr_init, %a = %a_init, %b = %b_init, %prev_c = %c_init) -> (tensor<128x16x!tt.ptr<f8E5M2>, #AL>, tensor<16x128x!tt.ptr<f16>, #BL>, !triton_gpu.memdesc<128x16xf8E5M2, #A, #triton_gpu.shared_memory>, !triton_gpu.memdesc<16x128xf16, #B, #triton_gpu.shared_memory>, tensor<128x128xf32, #C>) {
    %a_op_ = triton_gpu.local_load %a : !triton_gpu.memdesc<128x16xf8E5M2, #A, #triton_gpu.shared_memory> -> tensor<128x16xf8E5M2, #A_OP>
    %a_op = tt.fp_to_fp %a_op_ : tensor<128x16xf8E5M2, #A_OP> -> tensor<128x16xf16, #A_OP>
    %b_op = triton_gpu.local_load %b : !triton_gpu.memdesc<16x128xf16, #B, #triton_gpu.shared_memory> -> tensor<16x128xf16, #B_OP>
    ...
    %next_a = triton_gpu.local_alloc %next_a_ : (tensor<128x16xf8E5M2, #AL>) -> !triton_gpu.memdesc<128x16xf8E5M2, #A, #triton_gpu.shared_memory>
    %next_b_ = tt.load %next_b_ptr, %b_mask, %b_other : tensor<16x128x!tt.ptr<f16>, #BL>
    %next_b = triton_gpu.local_alloc %b_ : (tensor<16x128xf16, #BL>) -> !triton_gpu.memdesc<16x128xf16, #B, #triton_gpu.shared_memory>

    scf.yield %next_a_ptr, %next_b_ptr, %next_a, %next_b, %c : tensor<128x16x!tt.ptr<f8E5M2>, #AL>, tensor<16x128x!tt.ptr<f16>, #BL>, !triton_gpu.memdesc<128x16xf8E5M2, #A, #triton_gpu.shared_memory>, !triton_gpu.memdesc<16x128xf16, #B, #triton_gpu.shared_memory>, tensor<128x128xf32, #C>
  }
  tt.return %loop#4 : tensor<128x128xf32, #C>
}
}  // end module
...
#A = #triton_gpu.shared<{vec = 2, perPhase = 2, maxPhase = 4, order = [1, 0]}>
#B = #triton_gpu.shared<{vec = 2, perPhase = 2, maxPhase = 4, order = [1, 0]}>
#C = #triton_gpu.amd_mfma<{versionMajor = 3, versionMinor = 0, warpsPerCTA = [2, 4], instrShape = [32, 32], isTransposed = false}>
#A_OP = #triton_gpu.dot_op<{opIdx = 0, parent = #C, kWidth = 2}>
#B_OP = #triton_gpu.dot_op<{opIdx = 1, parent = #C, kWidth = 2}>
