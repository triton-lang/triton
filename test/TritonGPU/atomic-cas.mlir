// RUN: triton-opt %s -convert-triton-to-tritongpu=target=cuda:80 --allocate-shared-memory 2>&1 | FileCheck %s --check-prefix=GPU
// RUN: triton-opt %s -convert-triton-to-tritongpu=target=cuda:80 --allocate-shared-memory -convert-triton-gpu-to-llvm 2>&1 | FileCheck %s --check-prefix=LLVM

// GPU: %9 = tt.atomic_cas acq_rel, cta, %8, %cst_0, %cst {allocation.offset = 0 : i32} : (tensor<2x!tt.ptr<i64>, #blocked>, tensor<2xi64, #blocked>, tensor<2xi64, #blocked>) -> tensor<2xi64, #blocked>
