...
#blocked_repro_a3 = #ttg.blocked<{sizePerThread = [1, 1, 2], threadsPerWarp = [32, 1, 1], warpsPerCTA = [4, 2, 1], order = [0, 1, 2]}>
#blocked_repro_b3 = #ttg.blocked<{sizePerThread = [1, 1, 2], threadsPerWarp = [16, 2, 1], warpsPerCTA = [8, 1, 1], order = [0, 1, 2]}>
#blocked_repro_c3 = #ttg.blocked<{sizePerThread = [1, 1], threadsPerWarp = [16, 2], warpsPerCTA = [8, 1], order = [0, 1]}>
...
module attributes {ttg.target = "cuda:100", "ttg.num-warps" = 8 : i32} {
// CHECK-LABEL: @convert_layout_split_in_partition
tt.func @convert_layout_split_in_partition(%arg0: i32) {
  ttg.warp_specialize(%arg0)
  default {
    ttg.warp_yield
  }
  partition0(%arg1: i32) num_warps(8) {
    %0 = tt.splat %arg1 : i32 -> tensor<128x4x2xi32, #blocked_repro_a3>
    %1 = ttg.convert_layout %0 : tensor<128x4x2xi32, #blocked_repro_a3> -> tensor<128x4x2xi32, #blocked_repro_b3>
    %2, %3 = tt.split %1 : tensor<128x4x2xi32, #blocked_repro_b3> -> tensor<128x4xi32, #blocked_repro_c3>
    "use"(%2, %3) : (tensor<128x4xi32, #blocked_repro_c3>, tensor<128x4xi32, #blocked_repro_c3>) -> ()
    ttg.warp_return
  } : (i32) -> ()
  tt.return
}
