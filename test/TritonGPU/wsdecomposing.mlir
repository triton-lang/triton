...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 4 : i32, "triton_gpu.enable-warp-specialization" = 1 : i32} {
  ...
  tt.func public @simple_gemm(%arg0: !tt.ptr<f16, 1> {tt.divisibility = 16 : i32}, %arg1: !tt.ptr<f16, 1> {tt.divisibility = 16 : i32}, %arg2: !tt.ptr<f16, 1> {tt.divisibility = 16 : i32}, %arg3: i32 {tt.divisibility = 16 : i32}, %arg4: i32 {tt.divisibility = 16 : i32}, %arg5: i32 {tt.divisibility = 16 : i32}, %arg6: i32 {tt.divisibility = 16 : i32}, %arg7: i32 {tt.divisibility = 16 : i32}, %arg8: i32 {tt.divisibility = 16 : i32}) {
    ...
    // CHECK-NEXT:     %81 = "triton_gpu.cmpi"(%28, %37) {async_agent = dense<1> : vector<1xi32>, predicate = 2 : i64} : (tensor<128xi32, #triton_gpu.slice<{dim = 1, parent = #blocked2}>>, tensor<128xi32, #triton_gpu.slice<{dim = 1, parent = #blocked2}>>) -> tensor<128xi1, #triton_gpu.slice<{dim = 1, parent = #blocked2}>>
    ...
    // CHECK-NEXT:     %83 = "triton_gpu.cmpi"(%35, %40) {async_agent = dense<1> : vector<1xi32>, predicate = 2 : i64} : (tensor<128xi32, #triton_gpu.slice<{dim = 0, parent = #blocked2}>>, tensor<128xi32, #triton_gpu.slice<{dim = 0, parent = #blocked2}>>) -> tensor<128xi1, #triton_gpu.slice<{dim = 0, parent = #blocked2}>>
...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 4 : i32, "triton_gpu.enable-warp-specialization" = 1 : i32} {
  ...
  tt.func public @nested_for_gemm(%arg0: !tt.ptr<f16, 1> {tt.divisibility = 16 : i32}, %arg1: !tt.ptr<f16, 1> {tt.divisibility = 16 : i32}, %arg2: !tt.ptr<f16, 1> {tt.divisibility = 16 : i32}, %arg3: i32 {tt.divisibility = 16 : i32}, %arg4: i32 {tt.divisibility = 16 : i32}, %arg5: i32 {tt.divisibility = 16 : i32}, %arg6: i32 {tt.divisibility = 16 : i32}, %arg7: i32 {tt.divisibility = 16 : i32}, %arg8: i32 {tt.divisibility = 16 : i32}) {
    ...
    // CHECK-NEXT:     %68 = arith.divsi %67, %c32_i32 {async_agent = dense<[0, 1]> : vector<2xi32>} : i32
    // CHECK-NEXT:     %69 = arith.index_cast %68 {async_agent = dense<[0, 1]> : vector<2xi32>} : i32 to index
    // CHECK-NEXT:     %cst_2 = arith.constant {async_agent = dense<0> : vector<1xi32>} dense<0.000000e+00> : tensor<128x32xf16, #shared>
    // CHECK-NEXT:     %cst_3 = arith.constant {async_agent = dense<0> : vector<1xi32>} dense<0.000000e+00> : tensor<32x128xf16, #shared1>
    // CHECK-NEXT:     %70:3 = scf.for %arg9 = %c0 to %69 step %c1 iter_args(%arg10 = %cst, %arg11 = %57, %arg12 = %66) -> (tensor<128x128xf32, #mma>, tensor<128x32x!tt.ptr<f16, 1>, #blocked1>, tensor<32x128x!tt.ptr<f16, 1>, #blocked>) {
    ...
    // CHECK-NEXT:         %95 = triton_gpu.convert_layout %93 {async_agent = dense<0> : vector<1xi32>} : (tensor<128x32xf16, #blocked1>) -> tensor<128x32xf16, #shared>
    // CHECK-NEXT:         %96 = triton_gpu.convert_layout %94 {async_agent = dense<0> : vector<1xi32>} : (tensor<32x128xf16, #blocked>) -> tensor<32x128xf16, #shared1>
    // CHECK-NEXT:         scf.yield {async_agent = dense<0> : vector<1xi32>} %95, %96 : tensor<128x32xf16, #shared>, tensor<32x128xf16, #shared1>
    // CHECK-NEXT:       } {async_agent = dense<0> : vector<1xi32>}
...
module attributes {"triton_gpu.num-ctas" = 1 : i32, "triton_gpu.num-warps" = 4 : i32, "triton_gpu.enable-warp-specialization" = 1 : i32} {
  ...
  tt.func public @if_in_for_gemm(%arg0: !tt.ptr<f16, 1> {tt.divisibility = 16 : i32}, %arg1: !tt.ptr<f16, 1> {tt.divisibility = 16 : i32}, %arg2: !tt.ptr<f16, 1> {tt.divisibility = 16 : i32}, %arg3: i32 {tt.divisibility = 16 : i32}, %arg4: i32 {tt.divisibility = 16 : i32}, %arg5: i32 {tt.divisibility = 16 : i32}, %arg6: i32 {tt.divisibility = 16 : i32}, %arg7: i32 {tt.divisibility = 16 : i32}, %arg8: i32 {tt.divisibility = 16 : i32}) {
    ...
    // CHECK-NEXT:     %69 = arith.index_cast %68 {async_agent = dense<[0, 1]> : vector<2xi32>} : i32 to index
    // CHECK-NEXT:     %70:3 = scf.for %arg9 = %c0 to %69 step %c1 iter_args(%arg10 = %cst, %arg11 = %57, %arg12 = %66) -> (tensor<128x128xf32, #mma>, tensor<128x32x!tt.ptr<f16, 1>, #blocked1>, tensor<32x128x!tt.ptr<f16, 1>, #blocked>) {
    // CHECK-NEXT:       %89 = arith.index_cast %arg9 {async_agent = dense<0> : vector<1xi32>} : index to i32
    // CHECK-NEXT:       %90 = arith.cmpi ne, %89, %c31_i32 {async_agent = dense<0> : vector<1xi32>} : i32
    // CHECK-NEXT:       %91 = scf.if %90 -> (tensor<128x32xf16, #blocked1>) {
    // CHECK-NEXT:         %99 = arith.select %90, %c31_i32, %c127_i32 {async_agent = dense<0> : vector<1xi32>} : i32
    // CHECK-NEXT:         %100 = tt.splat %99 {async_agent = dense<0> : vector<1xi32>} : (i32) -> tensor<128x32xi32, #blocked1>
    // CHECK-NEXT:         %101 = tt.addptr %arg11, %100 {async_agent = dense<0> : vector<1xi32>} : tensor<128x32x!tt.ptr<f16, 1>, #blocked1>, tensor<128x32xi32, #blocked1>
    // CHECK-NEXT:         %102 = tt.load %101 {async_agent = dense<0> : vector<1xi32>, cache = 1 : i32, evict = 1 : i32, isVolatile = false} : tensor<128x32xf16, #blocked1>
    // CHECK-NEXT:         scf.yield {async_agent = dense<0> : vector<1xi32>} %102 : tensor<128x32xf16, #blocked1>
    // CHECK-NEXT:       } else {
    // CHECK-NEXT:         %99 = tt.load %arg11 {async_agent = dense<0> : vector<1xi32>, cache = 1 : i32, evict = 1 : i32, isVolatile = false} : tensor<128x32xf16, #blocked1>
    // CHECK-NEXT:         scf.yield {async_agent = dense<0> : vector<1xi32>} %99 : tensor<128x32xf16, #blocked1>
    // CHECK-NEXT:       } {async_agent = dense<0> : vector<1xi32>}
    ...
    // CHECK-NEXT:         %100 = tt.splat %99 {async_agent = dense<0> : vector<1xi32>} : (i32) -> tensor<128x32xi32, #blocked1>
    // CHECK-NEXT:         %101 = tt.addptr %arg11, %100 {async_agent = dense<0> : vector<1xi32>} : tensor<128x32x!tt.ptr<f16, 1>, #blocked1>, tensor<128x32xi32, #blocked1>
    // CHECK-NEXT:         scf.yield {async_agent = dense<0> : vector<1xi32>} %101 : tensor<128x32x!tt.ptr<f16, 1>, #blocked1>
    // CHECK-NEXT:       } else {
    // CHECK-NEXT:         %99 = tt.addptr %arg11, %cst_1 {async_agent = dense<0> : vector<1xi32>} : tensor<128x32x!tt.ptr<f16, 1>, #blocked1>, tensor<128x32xi32, #blocked1>
    // CHECK-NEXT:         scf.yield {async_agent = dense<0> : vector<1xi32>} %99 : tensor<128x32x!tt.ptr<f16, 1>, #blocked1>
    // CHECK-NEXT:       } {async_agent = dense<0> : vector<1xi32>}
    // CHECK-NEXT:       %97 = tt.addptr %96, %cst_1 {async_agent = dense<0> : vector<1xi32>} : tensor<128x32x!tt.ptr<f16, 1>, #blocked1>, tensor<128x32xi32, #blocked1>
