...
tt.func @one_dep_async(%lb : index, %ub : index, %step : index,
                 %a_ptr_init : tensor<128x32x!tt.ptr<f16>, #A> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>}) -> () {
  scf.for %iv = %lb to %ub step %step : index {
...
tt.func @one_dep_barrier_wait(%lb : index, %ub : index, %step : index,
                 %a_ptr_init : tensor<128x64x!tt.ptr<f16>, #A> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
...
tt.func @one_load_group(%lb : index, %ub : index, %step : index,
                       %a_ptr_init : tensor<128x32x!tt.ptr<f32>, #A> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                       %b_ptr_init : tensor<128x32x!tt.ptr<f32>, #A> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>}) -> () {
  // CHECK-DAG: %[[MINUS_ONE:.*]] = arith.constant -1
  // CHECK-DAG: %[[ZERO:.*]] = arith.constant 0
  // CHECK-DAG: %[[ONE:.*]] = arith.constant 1
  // CHECK-DAG: %[[NUM_BUFS:.*]] = arith.constant {{.*}} 2 : i32
...
tt.func @two_load_groups(%lb : index, %ub : index, %step : index,
                       %a_ptr_init : tensor<128x32x!tt.ptr<f32>, #A> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                       %b_ptr_init : tensor<128x32x!tt.ptr<f32>, #A> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                       %c_ptr_init : tensor<128x32x!tt.ptr<f32>, #A> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>}) -> () {
  // CHECK-DAG: %[[MINUS_ONE:.*]] = arith.constant -1
  // CHECK-DAG: %[[ZERO:.*]] = arith.constant 0
  // CHECK-DAG: %[[ONE:.*]] = arith.constant 1
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @shmem_pipelining_mmav3(%lb : index, %ub : index, %step : index,
                                              %A_ptr: tensor<128x128x!tt.ptr<f16>, #blocked1> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                                              %B_ptr: tensor<128x128x!tt.ptr<f16>, #blocked1> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>}) -> tensor<128x128xf16, #mma> {
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @no_shmem_pipelining_incompat_layout(
                    ...
                    %A_ptr: tensor<128x128x!tt.ptr<f16>, #blocked1> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                    %B_ptr: tensor<128x128x!tt.ptr<f16>, #blocked1> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>}) -> tensor<128x128xf32, #mma> {
    %true = arith.constant true
    %cst = arith.constant dense<0.000000e+00> : tensor<128x128xf32, #mma>
    %c0_i32 = arith.constant 0 : i32
    %res = scf.for %i = %lb to %ub step %step iter_args(%acc = %cst) -> (tensor<128x128xf32, #mma>) : index {
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @no_shmem_pipelining_other_used(
                      %lb : index, %ub : index, %step : index,
                      %A_ptr: tensor<128x128x!tt.ptr<f16>, #blocked1> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                      %B_ptr: tensor<128x128x!tt.ptr<f16>, #blocked1> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                      %mask: tensor<128x128xi1, #blocked1> {tt.constancy = 128 : i32},
                      %other: tensor<128x128xf16, #blocked1> {tt.constancy = 128 : i32}) -> tensor<128x128xf16, #mma> {
    %true = arith.constant true
    %cst = arith.constant dense<0.000000e+00> : tensor<128x128xf32, #mma>
    %c0_i32 = arith.constant 0 : i32
    %res = scf.for %i = %lb to %ub step %step iter_args(%acc = %cst) -> (tensor<128x128xf32, #mma>) : index {
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @shmem_pipelining_mmav5(%lb : index, %ub : index, %step : index,
                                              %A_ptr: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                                              %B_ptr: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>}) -> tensor<128x128xf16, #blocked> {
    %true = arith.constant true
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @pipelining_mmav5_scaled(%lb : index, %ub : index, %step : index,
                                              %A_ptr: tensor<128x128x!tt.ptr<f8E5M2>, #blocked1> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                                              %B_ptr: tensor<128x128x!tt.ptr<f8E5M2>, #blocked1> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
                                              %A_sc_ptr: tensor<1x2x32x4x4x!tt.ptr<i8>, #blocked2> {tt.divisibility = 16 : i32, tt.contiguity = 16 : i32},
                                              %B_sc_ptr: tensor<1x2x32x4x4x!tt.ptr<i8>, #blocked2> {tt.divisibility = 16 : i32, tt.contiguity = 16 : i32}) -> tensor<128x128xf32, #blocked1> {
    %true = arith.constant true
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @cnd_store_before_mma(%arg0: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>}, %arg1: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>}, %arg2: i32) -> tensor<128x128xf16, #blocked1> {
    %true = arith.constant true
    %cst = arith.constant dense<0.000000e+00> : tensor<128x128xf32, #blocked1>
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @simple_persistent_mmav5(%arg0: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>}, %arg1: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>}, %arg2: i32) -> tensor<128x128xf16, #blocked1> {
    %true = arith.constant true
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  // CHECK: %[[ACC_TM_SLICE:.*]] = ttg.memdesc_index %[[ACC_TM]]{{\[}}%[[FOR_RES]]#2{{\]}}
  // CHECK: %[[LOAD_ACC:.*]], %[[RES_TOK:.*]] = ttng.tmem_load %[[ACC_TM_SLICE]][]
  tt.func public @simple_persistent_mmav5_acc_flag(%arg0: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>}, %arg1: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>}, %arg2: i32) -> tensor<128x128xf16, #blocked1> {
    %true = arith.constant true
    %cst = "init_acc"() : () -> tensor<128x128xf32, #blocked1>
    %cst_0 = "override_acc"() : () -> tensor<128x128xf32, #blocked1>
    %c0_i32 = arith.constant 0 : i32
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @changed_acc_before_mma(%arg0: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>}, %arg1: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>}, %arg2: i32) -> tensor<128x128xf16, #blocked1> {
    %true = arith.constant true
    %cst = arith.constant dense<0.000000e+00> : tensor<128x128xf32, #blocked1>
    %cst_0 = arith.constant dense<2.000000e+00> : tensor<128x128xf32, #blocked1>
    %c0_i32 = arith.constant 0 : i32
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  tt.func public @changed_acc_unpipelineable_operand(%A: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>},
                                                     %B: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>},
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  // CHECK: ttng.tc_gen5_mma {{.*}} {is_async, loop.cluster = 2 : i32, loop.stage = 2 : i32}
  // CHECK: ttng.wait_barrier {{.*}} {loop.cluster = 1 : i32, loop.stage = 3 : i32}
  tt.func public @changed_acc_unpipelineable_operand2(%A: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>},
                                                     %B: tensor<128x128x!tt.ptr<f16>, #blocked> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>},
                                                      %arg1: i32, %arg2: i32, %arg3: i32) {
    %cst = arith.constant dense<0.000000e+00> : tensor<128x128xf32, #blocked1>
    %true = arith.constant true
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "cuda:100", "ttg.threads-per-warp" = 32 : i32} {
  ...
  // CHECK: ttng.tc_gen5_mma {{.*}} {is_async, loop.cluster = 2 : i32, loop.stage = 2 : i32}
  // CHECK: ttng.wait_barrier {{.*}} {loop.cluster = 1 : i32, loop.stage = 3 : i32}
  tt.func public @wait_before_tmem_alloc(%A: tensor<128x128x!tt.ptr<f16>, #blocked1> {tt.contiguity = [1, 16] : tensor<2xi32>, tt.divisibility = [16, 16] : tensor<2xi32>},
                                         %B: tensor<128x128xf16, #blocked1> {tt.contiguity = 16 : i32, tt.divisibility = 16 : i32},
                                         %arg1: i32, %arg2: i32, %arg3: i32) {
    %cst = arith.constant dense<0.000000e+00> : tensor<128x128xf32, #blocked1>
    %true = arith.constant true
    %0 = ttng.tmem_alloc : () -> !ttg.memdesc<128x128xf32, #tmem, #ttng.tensor_memory, mutable>
...
tt.func @conditional_store_race_fix(%lb : index, %ub : index, %step : index,
                 %a_ptr_init : tensor<128x32x!tt.ptr<f16>, #blocked1> {tt.divisibility = [16, 16] : tensor<2xi32>, tt.contiguity = [1, 16] : tensor<2xi32>},
