...
!memdesc1 = !ttg.memdesc<128x64xf16, #shared1, #ttg.shared_memory>
...
//    CHECK: ttng.warp_group_dot %[[A]], {{.*}} : tensor<128x64xf16, #ttg.dot_op<{opIdx = 0, parent = #mma, kWidth = 2}>> * !ttg.memdesc<64x64xf16, #shared, #ttg.shared_memory> -> tensor<128x64xf32, #mma>
tt.func @mma_v3_reg_operand_A(%arg0: tensor<128x64xf16, #mma>, %arg1: !memdesc0, %arg2: tensor<128x64xf32, #mma>) -> tensor<128x64xf32, #mma>{
  %A = ttg.local_alloc %arg0 : (tensor<128x64xf16, #mma>) -> !memdesc1
  %r = ttng.warp_group_dot %A, %arg1, %arg2 : !memdesc1 * !memdesc0 -> tensor<128x64xf32, #mma>
...
#shared = #ttg.shared<{vec = 8, perPhase = 1, maxPhase = 8, order = [1, 0], hasLeadingOffset = true}>
#shared1 = #ttg.shared<{vec = 8, perPhase = 1, maxPhase = 8, order = [1, 0], hasLeadingOffset = true}>
!memdesc0 = !ttg.memdesc<64x64xf8E5M2, #shared, #ttg.shared_memory>
!memdesc1 = !ttg.memdesc<128x64xf8E5M2, #shared1, #ttg.shared_memory>
module attributes {"ttg.target" = "cuda:90", "ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, "ttg.threads-per-warp" = 32 : i32} {
...
//    CHECK: ttng.warp_group_dot %[[A]], {{.*}} : tensor<128x64xf8E5M2, #ttg.dot_op<{opIdx = 0, parent = #mma, kWidth = 4}>> * !ttg.memdesc<64x64xf8E5M2, #shared, #ttg.shared_memory> -> tensor<128x64xf32, #mma>
tt.func @mma_v3_reg_operand_A_fp8(%arg0: tensor<128x64xf8E5M2, #mma>, %arg1: !memdesc0, %arg2: tensor<128x64xf32, #mma>) -> tensor<128x64xf32, #mma>{
  %A = ttg.local_alloc %arg0 : (tensor<128x64xf8E5M2, #mma>) -> !memdesc1
  %r = ttng.warp_group_dot %A, %arg1, %arg2 : !memdesc1 * !memdesc0 -> tensor<128x64xf32, #mma>
  tt.return %r : tensor<128x64xf32, #mma>
}
}
...
//    CHECK: %[[R:.*]] = ttng.warp_group_dot %[[A_CASTED]], %{{.*}}, %{{.*}} : tensor<128x64xf16, #ttg.dot_op<{opIdx = 0, parent = #mma, kWidth = 2}>> * !ttg.memdesc<64x64xf16, #shared, #ttg.shared_memory> -> tensor<128x64xf32, #mma>
  tt.func @mma_v3_reg_push_elementwise(%pa: tensor<128x64x!tt.ptr<bf16>, #blocked>, %dotb: !memdesc0, %dotc: tensor<128x64xf32, #mma>) -> tensor<128x64xf32, #mma>{
    ...
    %dota = ttg.local_alloc %a: (tensor<128x64xf16, #blocked>) -> !memdesc1
    %r = ttng.warp_group_dot %dota, %dotb, %dotc : !memdesc1 * !memdesc0 -> tensor<128x64xf32, #mma>
    tt.return %r : tensor<128x64xf32, #mma>
  }
}
...
//    CHECK: %[[R:.*]] = ttng.warp_group_dot %[[A_NEGATED]], %{{.*}}, %{{.*}} : tensor<128x64xf16, #ttg.dot_op<{opIdx = 0, parent = #mma, kWidth = 2}>> * !ttg.memdesc<64x64xf16, #shared, #ttg.shared_memory> -> tensor<128x64xf32, #mma>
  tt.func @mma_v3_reg_push_elementwise_chained(%pa: tensor<128x64x!tt.ptr<i8>, #blocked>, %dotb: !memdesc0, %dotc: tensor<128x64xf32, #mma>) -> tensor<128x64xf32, #mma>{
    ...
    %dota = ttg.local_alloc %a_negated: (tensor<128x64xf16, #blocked>) -> !memdesc1
    %r = ttng.warp_group_dot %dota, %dotb, %dotc : !memdesc1 * !memdesc0 -> tensor<128x64xf32, #mma>
    tt.return %r : tensor<128x64xf32, #mma>
  }
}
...
// CHECK: ttng.warp_group_dot
  tt.func @mma_reorder_transpose(%t: tensor<64x128xf16, #blocked1>, %dotb: !memdesc0, %dotc: tensor<128x64xf32, #mma>) -> tensor<128x64xf32, #mma>{
