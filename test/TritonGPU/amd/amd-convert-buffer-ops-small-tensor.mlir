...
#expanded_col = #ttg.linear<{register = [[0, 1]], lane = [[0, 2], [0, 4], [0, 0], [0, 0], [0, 0]], warp = [[0, 0], [0, 0]], block = []}>
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32} {
  ...
  tt.func @unary_triton_ops_transitive_nonneg(%arg0: !tt.ptr<bf16> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32}, %arg1: !tt.ptr<bf16> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32}) {
    ...
    %expanded9 = tt.reshape %8 : tensor<8xi32, #blocked2> -> tensor<1x8xi32, #expanded_col>
    %9 = ttg.convert_layout %expanded9 : tensor<1x8xi32, #expanded_col> -> tensor<1x8xi32, #blockedtrans>
...
#expanded_row = #ttg.linear<{register = [[64, 0]], lane = [[0, 0], [0, 0], [0, 0], [1, 0], [2, 0], [4, 0]], warp = [[8, 0], [16, 0], [32, 0]], block = []}>
...
module attributes {"ttg.num-warps" = 8 : i32, ttg.target = "hip:gfx942", "ttg.threads-per-warp" = 64 : i32} {
  tt.func @test_contiguity_set(%arg0: !tt.ptr<f16> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32}, %arg1: !tt.ptr<f16> {tt.divisibility = 16 : i32, tt.pointer_range = 32 : i32}, %stride_am: i32 {tt.divisibility = 16 : i32}) -> tensor<128x64xf16, #blocked> {
    ...
    %expanded2 = tt.reshape %0 : tensor<128xi32, #ttg.slice<{dim = 1, parent = #blocked}>> -> tensor<128x1xi32, #expanded_row>
    %2 = ttg.convert_layout %expanded2 : tensor<128x1xi32, #expanded_row> -> tensor<128x1xi32, #blocked>
