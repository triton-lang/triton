...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.target = "hip:gfx1250", "ttg.threads-per-warp" = 32 : i32} {
  // CHECK-LABEL: llvm.func @wmma_dot_scaled_mxfp8_bf16
  tt.func public @wmma_dot_scaled_mxfp8_bf16(%arg0: tensor<32x128xf8E4M3FN, #blocked>, %arg1: tensor<32x128xi8, #blocked>, %arg2: tensor<32x128x!tt.ptr<bf16>, #blocked>) {
    ...
    // CHECK: %[[S1:.+]] = llvm.extractvalue %arg1[1] : !llvm.struct
    ...
    // The next output uses the next scale, even within the same packed input.
    // CHECK: %[[Z1:.+]] = llvm.zext %[[S1]] : i8 to i32
    // CHECK: %[[P1:.+]] = rocdl.permlanex16 %[[Z1]], %[[Z1]], {{.*}}, true, false : i32, i32
    // CHECK: %[[X1:.+]] = llvm.trunc %[[P1]] : i32 to i8
    // CHECK: %[[W0:.+]] = llvm.insertelement %[[S1]], %{{.+}}[%{{.+}} : i32] : vector<4xi8>
    // CHECK: %[[W1:.+]] = llvm.insertelement %[[X1]], %[[W0]][%{{.+}} : i32] : vector<4xi8>
    // CHECK: %[[SI1:.+]] = llvm.bitcast %[[W1]] : vector<4xi8> to i32
    // CHECK: rocdl.cvt.scale.pk8.bf16.fp8 %{{.+}}, %[[SI1]][8] : vector<8xbf16>
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 4 : i32, ttg.shared = 2048 : i32, ttg.target = "hip:gfx1250", "ttg.threads-per-warp" = 32 : i32} {
  // CHECK-LABEL: llvm.func @cvt_scale_pk8_bf16_fp4
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 1 : i32, ttg.target = "hip:gfx1250", "ttg.threads-per-warp" = 32 : i32} {
  // CHECK-LABEL: llvm.func @cvt_scale_pk8_bf16_fp4_compact
