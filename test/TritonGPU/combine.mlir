...
module attributes {"ttg.num-warps" = 1 : i32, "ttg.num-ctas" = 1 : i32} {
  ...
  // CHECK-LABEL: @hoist_wider_type_when_convert_is_free
  tt.func public @hoist_wider_type_when_convert_is_free(%arg0: tensor<1x32xi16, #blocked>) -> tensor<1x32xi8, #blocked1> {
    // CHECK: %[[CVT:[^ ]+]] = ttg.convert_layout %arg0
    // CHECK: %[[EXT:[^ ]+]] = arith.extsi %[[CVT]]
    // CHECK: %[[ADD:[^ ]+]] = arith.addi %[[EXT]]
    // CHECK: %[[TRUNC:[^ ]+]] = arith.trunci %[[ADD]]
    // CHECK-NOT: ttg.convert_layout
    // CHECK: tt.return %[[TRUNC]]
    %ext = arith.extsi %arg0 : tensor<1x32xi16, #blocked> to tensor<1x32xi32, #blocked>
    %cst = arith.constant dense<1> : tensor<1x32xi32, #blocked>
    %add = arith.addi %ext, %cst : tensor<1x32xi32, #blocked>
    %trunc = arith.trunci %add : tensor<1x32xi32, #blocked> to tensor<1x32xi8, #blocked>
    %cvt = ttg.convert_layout %trunc : tensor<1x32xi8, #blocked> -> tensor<1x32xi8, #blocked1>
    tt.return %cvt : tensor<1x32xi8, #blocked1>
  }
}
