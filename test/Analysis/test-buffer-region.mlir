...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 1 : i32, ttg.shared = 65544 : i32, ttg.target = "cuda:90", ttg.tensor_memory_size = 0 : i32, "ttg.threads-per-warp" = 32 : i32, "ttg.total-num-warps" = 1 : i32} {
  tt.func public @memdesc_index_multiple_access(%idx: i32) {
    ...
    %stages = ttg.memdesc_subslice %0 [3, 0, 0] : !ttg.memdesc<8x32x32xf32, #shared, #smem, mutable> -> !ttg.memdesc<3x32x32xf32, #shared, #smem, mutable, 8x32x32>
    %view = ttg.memdesc_index %stages[%idx] : !ttg.memdesc<3x32x32xf32, #shared, #smem, mutable, 8x32x32> -> !ttg.memdesc<32x32xf32, #shared, #smem, mutable>
...
module attributes {"ttg.num-ctas" = 1 : i32, "ttg.num-warps" = 1 : i32, ttg.shared = 64 : i32, ttg.target = "cuda:90", ttg.tensor_memory_size = 0 : i32, "ttg.threads-per-warp" = 32 : i32, "ttg.total-num-warps" = 1 : i32} {
  tt.func public @noncontiguous_shared_subslice_region() {
    %parent = ttg.local_alloc {allocation.offset = 0 : i32} : () -> !ttg.memdesc<4x4xi32, #shared, #smem, mutable>
    %view = ttg.memdesc_subslice %parent [0, 0] : !ttg.memdesc<4x4xi32, #shared, #smem, mutable> -> !ttg.memdesc<4x2xi32, #shared, #smem, mutable, 4x4>
    // expected-remark @below {{Buffers: [0, 56]}}
    ttg.local_load %view : !ttg.memdesc<4x2xi32, #shared, #smem, mutable, 4x4> -> tensor<4x2xi32, #blocked>
    tt.return
  }

  // expected-remark @below {{All Shared Regions: [0, 56]}}
  tt.func private @print_all_regions() attributes {test.print_all_used_regions} {
    tt.return
  }
}
