...
tt.func @addptr(%arg0: !tt.ptr<i1> {tt.divisibility = 16 : i32}, %arg1: !tt.ptr<i8> {tt.divisibility = 16 : i32}, %arg2: !tt.ptr<i16> {tt.divisibility = 16 : i32}, %arg3: !tt.ptr<i32> {tt.divisibility = 16 : i32}, %arg4: !tt.ptr<i64> {tt.divisibility = 16 : i32}) {
  ...
  %11 = tt.reshape %10 : tensor<128xi32> -> tensor<1x128xi32>
...
tt.func @expanddims() {
  ...
  %3 = tt.reshape %2 : tensor<128xi32> -> tensor<128x1xi32>
  tt.return
}

// -----

tt.func @expanddims_multi_axis(
    %arg0: tensor<4x8xi32> {tt.contiguity = dense<[1, 1]> : tensor<2xi32>, tt.divisibility = dense<[8, 2]> : tensor<2xi32>, tt.constancy = dense<[1, 1]> : tensor<2xi32>}) {
  // expected-remark @below {{contiguity = [1, 1, 1], divisibility = [8, 2, 2], constancy = [1, 1, 1], constant_value = <none>}}
  %0 = tt.reshape %arg0 : tensor<4x8xi32> -> tensor<4x1x8xi32>
  tt.return
}
...
tt.func @broadcast() {
  ...
  %1 = tt.reshape %0 : tensor<128xi32> -> tensor<128x1xi32>
...
tt.func @select(%arg0 : i1, %arg1 : tensor<4xi1>) {
  ...
  %9 = tt.reshape %2 : tensor<128xi1> -> tensor<128x1xi1>
  // expected-remark @below {{contiguity = [1, 1], divisibility = [1, 1], constancy = [128, 1], constant_value = <none>}}
  %10 = tt.reshape %3 : tensor<128xi1> -> tensor<128x1xi1>
  // expected-remark @below {{contiguity = [1, 1], divisibility = [1, 1], constancy = [1, 1], constant_value = <none>}}
