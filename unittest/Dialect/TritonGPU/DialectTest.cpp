...
void testReshape(RankedTensorType srcTy, RankedTensorType dstTy,
                 ...
                 std::optional<bool> expectLegacySuccess,
                 DialectInferLayoutInterface *inferLayout,
  ...
  *couldReshape = false;
  ...
  ASSERT_TRUE(expectLegacySuccess || !dstTy.getEncoding())
  ...
  *couldReshape = true;
  ...
  EXPECT_TRUE(expectLegacySuccess.value_or(true) ||
              isa<LinearEncodingAttr>(inferredEnc))
      << "Expected encoding to be linear or legacy success, but got "
...
TEST_P(InferReshapeOpEncodingTest, DoIt) {
  ...
  bool expectLegacySuccess = std::get<2>(GetParam());
  ...
  testReshape(cast<RankedTensorType>(src), cast<RankedTensorType>(dst),
              expectedDstEnc, expectLegacySuccess, inferLayout,
              /*longErrors=*/true);
}
