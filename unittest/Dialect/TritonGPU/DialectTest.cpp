...
std::string toString(Attribute attr) {
  std::string str;
...
TEST(EncodingTest, verifyDotOp) {
  static MLIRContext ctx;
  ctx.loadDialect<TritonGPUDialect>();
  ...
  auto block = BlockedEncodingAttr::get(&ctx, {1, 1}, {8, 8}, warpsPerCTA,
                                        {1, 0}, ctaLayout);
  auto mmav1 =
      NvidiaMmaEncodingAttr::get(&ctx, 1, 0, warpsPerCTA, ctaLayout, {16, 16});
  auto mmav2 =
      NvidiaMmaEncodingAttr::get(&ctx, 2, 0, warpsPerCTA, ctaLayout, {8, 16});
  auto mmav3 =
      NvidiaMmaEncodingAttr::get(&ctx, 3, 0, warpsPerCTA, ctaLayout, {128, 16});
  auto mfma = AMDMfmaEncodingAttr::get(&ctx, 2, 0, warpsPerCTA, 16, 16, false,
                                       ctaLayout);
  auto wmma = AMDWmmaEncodingAttr::get(&ctx, warpsPerCTA, ctaLayout);
  ...
  for (auto c : cases) {
    auto res = DotOperandEncodingAttr::verify(
        [&]() { return mlir::emitError(loc); }, c.opIdx, c.parent, c.kWidth);
    ASSERT_EQ(res.succeeded(), c.success)
        << "case failed: opIdx " << c.opIdx << " parent " << toString(c.parent)
        << " kWidth " << c.kWidth;
  }
}
