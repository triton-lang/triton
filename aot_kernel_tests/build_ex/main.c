...
#define cuAllocFloatVec(N, ptr)                                                \
  CHECK_CUDA(cuMemAlloc(ptr, N * sizeof(float)), "Allocate Float");

#define VectoDevice(N, src, dst)                                               \
  CHECK_CUDA(cuMemcpyHtoD(dst, src, N * sizeof(float)), "Copy HtoD");
#define DevicetoVec(N, src, dst)                                               \
  CHECK_CUDA(cuMemcpyDtoH(dst, src, N * sizeof(float)), "Copy DtoH");
...
void main() {
  ...
  // CHECK_CUDA(vec_add_64_0d1d2d3d(stream, gX, gY, gZ, numWarps, u_cu, v_cu,
  // out_cu, n_elem), "kernel run");
  printf("Trying 64 Block size:\n");
  CHECK_CUDA(
      vec_add_64(stream, gX, gY, gZ, numWarps, u_cu, v_cu, out_cu, n_elem),
      "kernel run");
  ...
  CHECK_CUDA(
      vec_add_128(stream, gX, gY, gZ, numWarps, u_cu, v_cu, out_cu, n_elem),
      "kernel run");
  DevicetoVec(VEC_SIZE, out_cu, out);
  printf("Out value is %f Expected 30\n", out[15]);
