...
private:
  static arith::CmpIPredicate getPredicate(triton::gpu_rocm::CmpIOp op) {
...
AxisInfoAnalysis::AxisInfoAnalysis(DataFlowSolver &solver)
  ...
  visitors.append<CastOpAxisInfoVisitor<arith::ExtSIOp>,
                  ...
                  CastOpAxisInfoVisitor<triton::gpu_rocm::ConvertLayoutOp>,
  ...
  visitors.append<SelectOpAxisInfoVisitor<mlir::arith::SelectOp>,
                  SelectOpAxisInfoVisitor<triton::gpu_rocm::SelectOp>>();
...
unsigned ModuleAxisInfoAnalysis::getPtrContiguity(Value ptr) {
  ...
  auto uniqueContigPerThread =
      triton::gpu_rocm::getUniqueContigPerThread(layout, tensorTy.getShape());
...
unsigned ModuleAxisInfoAnalysis::getMaskAlignment(Value mask) {
  ...
  if (!axisInfo)
    return 1;
  auto maskOrder = triton::gpu_rocm::getOrder(tensorTy.getEncoding());
