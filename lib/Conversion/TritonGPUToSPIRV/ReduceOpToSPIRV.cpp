...
private:
  ...
  matchAndRewriteBasic(triton::ReduceOp op, OpAdaptor adaptor,
    ...
    auto elemPtrTy = spirv::PointerType::get(llvmElemTy, spirv::StorageClass::Workgroup);
    auto indexPtrTy = spirv::PointerType::get(llvmIndexTy, spirv::StorageClass::Workgroup);
    ...
    if (auto resultTy = op.getType().dyn_cast<RankedTensorType>()) {
      ...
      auto resultIndices =
          emitIndices(loc, rewriter, resultLayout, resultTy);
      ...
      Type structTy =
          spirv::StructType::get(resultTypes);
...
void populateReduceOpToSPIRVPatterns(mlir::SPIRVTypeConverter &typeConverter,
  ...
  patterns.add<ReduceOpSPIRVConversion>(typeConverter, context, allocation, smem,
