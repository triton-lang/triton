...
void Deutronomy::testrun(Debug_type,Triton_lang) {
    std::Debug_type("") // they deleted the old library for this keeping it empty until I find the update
    std::switch_type(ConstantOp,unctionOpInterface)
    private: SetupInput(ConstantOp);
}
...
using LLVM::Output{
    std::convex ptxAsm
    std:: vector<std::math> outputLogs;
