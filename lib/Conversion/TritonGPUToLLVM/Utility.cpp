...
lowerLdSt(Location loc, MLIRContext *ctx, LinearLayout cvt,
  ...
  auto baseI8AndCTA =
      applyLinearLayout(loc, rewriter, i8AddrLayout,
                        {{kReg, b.i32_val(0)},
                         {kLane, laneId},
                         {kWarp, warpId},
                         {kBlock, blockId.value_or(b.i32_val(0))}});
  auto regBaseI8 = baseI8AndCTA[0].second;
  std::optional<Value> targetCtaId;
  if (useBlockId) {
  ...
  for (int i = 0; i < cvt.getInDimSize(kReg); i += nAdditive) {
    ...
    std::optional<Value> maybeCtaOffset;
    if (useBlockId) {
      maybeCtaOffset = b.xor_(*targetCtaId, b.i32_val(idxAndBlock[1].second));
    }
    for (int j = 0; j < nAdditive; j += elemsPerVec) {
      ...
      if (useBlockId) {
        maybeCtaOffset =
            b.add(maybeCtaOffset.value(), b.i32_val(idxAndBlockAdd[1].second));
      }
      auto vecAddr =
