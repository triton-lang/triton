...
Value matrixVectorProd(TritonLLVMOpBuilder &b, const LinearLayout &A, Value x) {
  ...
  // Heuristic: Generate ORs first, then XORs.
  // No good reason why this should be best, but seems to do the job
  Value acc = b.i32_val(0);
  for (auto shifted : ors) {
    acc = b.or_(acc, shifted, /*disjoint=*/true);
  }
  for (auto shifted : xors) {
    acc = b.xor_(acc, shifted);
  }
