...
public:
  ...
  matchAndRewrite(triton::gpu::ConvertLayoutOp op, OpAdaptor adaptor,
    ...
    if (srcLayout.isa<MmaEncodingAttr>() && dstLayout.isa<MmaEncodingAttr>()) {
      ...
      auto dstMmaLayout = dstLayout.cast<MmaEncodingAttr>();
      if (isMmaToMmaShortcut(srcMmaLayout, dstMmaLayout)) {
    ...
    for (unsigned repId = 0; repId < accumNumReplicates; ++repId) {
      ...
      if (repId != 0) {
        ...
        if (op->hasAttr("async_agent")) {
          int barId = getAgentIds(op).front() % 16;
          ...
          rewriter.create<triton::nvgpu::BarSyncOP>(loc, bar, kNumThreads);
        } else {
          barrier();
        }
      }
