...
LinearLayout basisPermutationLayout(const LinearLayout &src,
  ...
  auto dst2 = dst.transposeOuts(llvm::to_vector(src.getOutDimNames()));
  auto dst3 = dst2.transposeIns(llvm::to_vector(src.getInDimNames()));

  // This does the heavy lifting:
  return src.invertAndCompose(dst3);
}
