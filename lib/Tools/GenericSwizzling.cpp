...
LinearLayout optimalSwizzling(const LinearLayout &src, const LinearLayout &dst,
  ...
  const int32_t lenBbasis = std::min<int32_t>(
      ...
      dim - vbasis.size() - blockBases.size());
  // Bases to cover all the tensor
  const int32_t lenSbasis = dim - lenBbasis - vbasis.size() - blockBases.size();

  auto sbasis = computeSegment(bankSrc, bankDst, blockBases, dim, lenSbasis);
  ...
  unionBasis.append(blockBases.begin(), blockBases.end());
...
LinearLayout optimalSwizzlingLdSt(const LinearLayout &src,
  ...
  auto blockSrcSet = SetVector<int32_t>(llvm::from_range_t{},
                                        removeZeros(flatten(srcFlat, kBlock)));
    ...
    while (vbasis.size() < basesPerBank &&
      ...
      if (vbasis.size() < basesPerBank && i < warpDst.size() &&
          !llvm::is_contained(vbasis, warpDst[i] &&
                                          !blockSrcSet.contains(warpDst[i]))) {
