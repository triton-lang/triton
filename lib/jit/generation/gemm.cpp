...
std::string intelblas_gemm::generate_impl(std::string const & suffix, expression_tree const & tree, driver::Device const & device, symbolic::symbols_table const & symbols) const
{
  ...
  stream << "__kernel void intelblas_gemm_buffer_NN( " << std::endl;
  stream << "    const __global float *src0, int off0, " << std::endl;
  stream << "    const __global float *src1, int off1, " << std::endl;
  stream << "    __global float *dst, int offd, " << std::endl;
  ...
  stream << " " << std::endl;
  stream << "    float4 brow; " << std::endl;
  stream << "    float2 arow0, arow1, arow2, arow3, arow4, arow5, arow6, arow7; " << std::endl;
  stream << " " << std::endl;
  stream << "    __global float *dst_write0 = dst + local_x * VEC_SIZE + ( group_x * TILE_N ) + ( group_y * LWG_HEIGHT * TILE_M + local_y * TILE_M) * ldC + offd; " << std::endl;
  stream << " " << std::endl;
  stream << "    const __global float *src0_read = src0 + local_x * ( TILE_K / 8 ) + ( group_y * LWG_HEIGHT * TILE_M + local_y * TILE_M ) * ldA + start_index + off0; " << std::endl;
  stream << " " << std::endl;
  stream << "    const __global float *src1_read0 = src1 + local_x * VEC_SIZE + ( group_x * TILE_N ) + start_index * ldB + off1; " << std::endl;
  stream << " " << std::endl;
  ...
  stream << " " << std::endl;
  stream << "    float4 dot00 = (start_index != 0) ? ((__global float4 *)dst_write0)[0] : beta * ((__global float4 *)dst_write0)[0]; " << std::endl;
  stream << "    float4 dot01 = (start_index != 0) ? ((__global float4 *)(dst_write0 + 1 * ldC))[0] : beta * ((__global float4 *)(dst_write0 + 1 * ldC))[0]; " << std::endl;
  stream << "    float4 dot02 = (start_index != 0) ? ((__global float4 *)(dst_write0 + 2 * ldC))[0] : beta * ((__global float4 *)(dst_write0 + 2 * ldC))[0]; " << std::endl;
  stream << "    float4 dot03 = (start_index != 0) ? ((__global float4 *)(dst_write0 + 3 * ldC))[0] : beta * ((__global float4 *)(dst_write0 + 3 * ldC))[0]; " << std::endl;
  stream << "    float4 dot04 = (start_index != 0) ? ((__global float4 *)(dst_write0 + 4 * ldC))[0] : beta * ((__global float4 *)(dst_write0 + 4 * ldC))[0]; " << std::endl;
  stream << "    float4 dot05 = (start_index != 0) ? ((__global float4 *)(dst_write0 + 5 * ldC))[0] : beta * ((__global float4 *)(dst_write0 + 5 * ldC))[0]; " << std::endl;
  stream << "    float4 dot06 = (start_index != 0) ? ((__global float4 *)(dst_write0 + 6 * ldC))[0] : beta * ((__global float4 *)(dst_write0 + 6 * ldC))[0]; " << std::endl;
  stream << "    float4 dot07 = (start_index != 0) ? ((__global float4 *)(dst_write0 + 7 * ldC))[0] : beta * ((__global float4 *)(dst_write0 + 7 * ldC))[0]; " << std::endl;
  stream << "     " << std::endl;
  stream << "    int end_index = min(start_index + 256, K); " << std::endl;
  stream << "    int w = start_index; " << std::endl;
  stream << "    while( w + TILE_K <= end_index ) { " << std::endl;
  stream << "        arow0 = alpha * ((__global float2 *)(src0_read + row0 * ldA))[0]; " << std::endl;
  stream << "        arow1 = alpha * ((__global float2 *)(src0_read + row1 * ldA))[0]; " << std::endl;
  stream << "        arow2 = alpha * ((__global float2 *)(src0_read + row2 * ldA))[0]; " << std::endl;
  stream << "        arow3 = alpha * ((__global float2 *)(src0_read + row3 * ldA))[0]; " << std::endl;
  stream << "        arow4 = alpha * ((__global float2 *)(src0_read + row4 * ldA))[0]; " << std::endl;
  stream << "        arow5 = alpha * ((__global float2 *)(src0_read + row5 * ldA))[0]; " << std::endl;
  stream << "        arow6 = alpha * ((__global float2 *)(src0_read + row6 * ldA))[0]; " << std::endl;
  stream << "        arow7 = alpha * ((__global float2 *)(src0_read + row7 * ldA))[0]; " << std::endl;
  stream << " " << std::endl;
  stream << "#define MM_DOT_PRODUCT( index, suffix )   \\ " << std::endl;
  stream << "        brow = ((__global float4 *)src1_read0)[0];  src1_read0 += ldB; \\ " << std::endl;
  stream << "        dot00 = mad( (float4)(intel_sub_group_shuffle( arow0, index ).s##suffix), brow, dot00 ); \\ " << std::endl;
  stream << "        dot01 = mad( (float4)(intel_sub_group_shuffle( arow1, index ).s##suffix), brow, dot01 ); \\ " << std::endl;
  stream << "        dot02 = mad( (float4)(intel_sub_group_shuffle( arow2, index ).s##suffix), brow, dot02 ); \\ " << std::endl;
  stream << "        dot03 = mad( (float4)(intel_sub_group_shuffle( arow3, index ).s##suffix), brow, dot03 ); \\ " << std::endl;
  stream << "        dot04 = mad( (float4)(intel_sub_group_shuffle( arow4, index ).s##suffix), brow, dot04 ); \\ " << std::endl;
  stream << "        dot05 = mad( (float4)(intel_sub_group_shuffle( arow5, index ).s##suffix), brow, dot05 ); \\ " << std::endl;
  stream << "        dot06 = mad( (float4)(intel_sub_group_shuffle( arow6, index ).s##suffix), brow, dot06 ); \\ " << std::endl;
  stream << "        dot07 = mad( (float4)(intel_sub_group_shuffle( arow7, index ).s##suffix), brow, dot07 ); \\ " << std::endl;
  stream << " " << std::endl;
  ...
  stream << "        arow0.x = ((w + local_x * 2) < K) ? alpha * (src0_read + row0 * ldA)[0] : 0.0f; " << std::endl;
  stream << "        arow0.y = ((w + local_x * 2 + 1) < K) ? alpha * (src0_read + row0 * ldA)[1] : 0.0f; " << std::endl;
  stream << "        arow1.x = ((w + local_x * 2) < K) ? alpha * (src0_read + row1 * ldA)[0] : 0.0f; " << std::endl;
  stream << "        arow1.y = ((w + local_x * 2 + 1) < K) ? alpha * (src0_read + row1 * ldA)[1] : 0.0f; " << std::endl;
  stream << "        arow2.x = ((w + local_x * 2) < K) ? alpha * (src0_read + row2 * ldA)[0] : 0.0f; " << std::endl;
  stream << "        arow2.y = ((w + local_x * 2 + 1) < K) ? alpha * (src0_read + row2 * ldA)[1] : 0.0f; " << std::endl;
  stream << "        arow3.x = ((w + local_x * 2) < K) ? alpha * (src0_read + row3 * ldA)[0] : 0.0f; " << std::endl;
  stream << "        arow3.y = ((w + local_x * 2 + 1) < K) ? alpha * (src0_read + row3 * ldA)[1] : 0.0f; " << std::endl;
  stream << "        arow4.x = ((w + local_x * 2) < K) ? alpha * (src0_read + row4 * ldA)[0] : 0.0f; " << std::endl;
  stream << "        arow4.y = ((w + local_x * 2 + 1) < K) ? alpha * (src0_read + row4 * ldA)[1] : 0.0f; " << std::endl;
  stream << "        arow5.x = ((w + local_x * 2) < K) ? alpha * (src0_read + row5 * ldA)[0] : 0.0f; " << std::endl;
  stream << "        arow5.y = ((w + local_x * 2 + 1) < K) ? alpha * (src0_read + row5 * ldA)[1] : 0.0f; " << std::endl;
  stream << "        arow6.x = ((w + local_x * 2) < K) ? alpha * (src0_read + row6 * ldA)[0] : 0.0f; " << std::endl;
  stream << "        arow6.y = ((w + local_x * 2 + 1) < K) ? alpha * (src0_read + row6 * ldA)[1] : 0.0f; " << std::endl;
  stream << "        arow7.x = ((w + local_x * 2) < K) ? alpha * (src0_read + row7 * ldA)[0] : 0.0f; " << std::endl;
  stream << "        arow7.y = ((w + local_x * 2 + 1) < K) ? alpha * (src0_read + row7 * ldA)[1] : 0.0f; " << std::endl;
  stream << " " << std::endl;
  stream << "#define MM_DOT_PRODUCT( index, suffix )   \\ " << std::endl;
  stream << "        brow = (w < K) ? ((__global float4 *)src1_read0)[0] : 0.0f;  src1_read0 += ldB; w++; \\ " << std::endl;
  stream << "        dot00 = mad( (float4)(intel_sub_group_shuffle( arow0, index ).s##suffix), brow, dot00 ); \\ " << std::endl;
  stream << "        dot01 = mad( (float4)(intel_sub_group_shuffle( arow1, index ).s##suffix), brow, dot01 ); \\ " << std::endl;
  stream << "        dot02 = mad( (float4)(intel_sub_group_shuffle( arow2, index ).s##suffix), brow, dot02 ); \\ " << std::endl;
  stream << "        dot03 = mad( (float4)(intel_sub_group_shuffle( arow3, index ).s##suffix), brow, dot03 ); \\ " << std::endl;
  ...
  stream << "            __global float4 *dst_write = (__global float4 *)dst_write0; " << std::endl;
  stream << "            dst_write[0] = dot00; dst_write0 += ldC; dst_write = (__global float4 *)dst_write0; " << std::endl;
  stream << "            if(mad24(global_y, 8, 1) < M) { dst_write[0] = dot01; dst_write0 += ldC; dst_write = (__global float4 *)dst_write0; } " << std::endl;
  stream << "            else return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 2) < M) { dst_write[0] = dot02; dst_write0 += ldC; dst_write = (__global float4 *)dst_write0; } " << std::endl;
  stream << "            else return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 3) < M) { dst_write[0] = dot03; dst_write0 += ldC; dst_write = (__global float4 *)dst_write0; } " << std::endl;
  stream << "            else return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 4) < M) { dst_write[0] = dot04; dst_write0 += ldC; dst_write = (__global float4 *)dst_write0; } " << std::endl;
  stream << "            else return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 5) < M) { dst_write[0] = dot05; dst_write0 += ldC; dst_write = (__global float4 *)dst_write0; } " << std::endl;
  stream << "            else return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 6) < M) { dst_write[0] = dot06; dst_write0 += ldC; dst_write = (__global float4 *)dst_write0; } " << std::endl;
  stream << "            else return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 7) < M) { dst_write[0] = dot07; } " << std::endl;
  stream << "        } else if(mad24(global_x, 4, 2) < N) { " << std::endl;
  stream << "            __global float2 *dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            dst_write[0] = dot00.xy; " << std::endl;
  stream << "            dst_write0[2] = dot00.z; " << std::endl;
  stream << "            dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            if(mad24(global_y, 8, 1) < M) { " << std::endl;
  stream << "                dst_write[0] = dot01.xy;  " << std::endl;
  stream << "                dst_write0[2] = dot01.z; " << std::endl;
  stream << "                dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            } else " << std::endl;
  stream << "                return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 2) < M) { " << std::endl;
  stream << "                dst_write[0] = dot02.xy; " << std::endl;
  stream << "                dst_write0[2] = dot02.z; " << std::endl;
  stream << "                dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            } else " << std::endl;
  stream << "                return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 3) < M) { " << std::endl;
  stream << "                dst_write[0] = dot03.xy; " << std::endl;
  stream << "                dst_write0[2] = dot03.z; " << std::endl;
  stream << "                dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            } else " << std::endl;
  stream << "                return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 4) < M) { " << std::endl;
  stream << "                dst_write[0] = dot04.xy; " << std::endl;
  stream << "                dst_write0[2] = dot04.z; " << std::endl;
  stream << "                dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            } else " << std::endl;
  stream << "                return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 5) < M) { " << std::endl;
  stream << "                dst_write[0] = dot05.xy; " << std::endl;
  stream << "                dst_write0[2] = dot05.z; " << std::endl;
  stream << "                dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            } else " << std::endl;
  stream << "                return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 6) < M) { " << std::endl;
  stream << "                dst_write[0] = dot06.xy; " << std::endl;
  stream << "                dst_write0[2] = dot06.z; " << std::endl;
  stream << "                dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            } else " << std::endl;
  stream << "                return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 7) < M) { " << std::endl;
  stream << "                dst_write[0] = dot07.xy; " << std::endl;
  stream << "                dst_write0[2] = dot07.z; " << std::endl;
  stream << "            } " << std::endl;
  stream << "        } else if(mad24(global_x, 4, 1) < N) { " << std::endl;
  stream << "            __global float2 *dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            dst_write[0] = dot00.xy; dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            if(mad24(global_y, 8, 1) < M) { dst_write[0] = dot01.xy; dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; } " << std::endl;
  stream << "            else return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 2) < M) { dst_write[0] = dot02.xy; dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; } " << std::endl;
  stream << "            else return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 3) < M) { dst_write[0] = dot03.xy; dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; } " << std::endl;
  stream << "            else return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 4) < M) { dst_write[0] = dot04.xy; dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; } " << std::endl;
  stream << "            else return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 5) < M) { dst_write[0] = dot05.xy; dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; } " << std::endl;
  stream << "            else return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 6) < M) { dst_write[0] = dot06.xy; dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; } " << std::endl;
  stream << "            else return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 7) < M) { dst_write[0] = dot07.xy; } " << std::endl;
  stream << "        } else { " << std::endl;
  ...
  stream << " " << std::endl;
  stream << "__attribute__((reqd_work_group_size(8, LWG_HEIGHT, 1))) " << std::endl;
  stream << "__kernel void intelblas_gemm_buffer_NT( " << std::endl;
  stream << "    const __global float *src0, int off0, " << std::endl;
  stream << "    const __global float *src1, int off1, " << std::endl;
  stream << "    __global float *dst, int offd, " << std::endl;
  stream << "    int M, " << std::endl;
  ...
  stream << "    const int global_y = get_global_id(1); " << std::endl;
  stream << " " << std::endl;
  stream << "    float8 dot00 = 0.f; " << std::endl;
  stream << "    float8 dot01 = 0.f; " << std::endl;
  stream << "    float8 dot02 = 0.f; " << std::endl;
  stream << "    float8 dot03 = 0.f; " << std::endl;
  stream << "    float8 dot04 = 0.f; " << std::endl;
  stream << "    float8 dot05 = 0.f; " << std::endl;
  stream << "    float8 dot06 = 0.f; " << std::endl;
  stream << "    float8 dot07 = 0.f; " << std::endl;
  stream << "     " << std::endl;
  stream << "    float4 brow0; " << std::endl;
  stream << "    float4 brow1; " << std::endl;
  stream << "    float4 brow2; " << std::endl;
  stream << "    float4 brow3; " << std::endl;
  stream << "    float4 brow4; " << std::endl;
  stream << "    float4 brow5; " << std::endl;
  stream << "    float4 brow6; " << std::endl;
  stream << "    float4 brow7; " << std::endl;
  stream << "     " << std::endl;
  stream << "    __global float *dst_write0 = dst + local_x * VEC_SIZE + ( group_x * TILE_N ) + ( group_y * LWG_HEIGHT * TILE_M + local_y * TILE_M) * ldC + offd; " << std::endl;
  stream << " " << std::endl;
  stream << "    const __global float *src0_read = src0 + local_x * ( TILE_K / 8 ) + ( group_y * LWG_HEIGHT * TILE_M + local_y * TILE_M ) * ldA + off0; " << std::endl;
  stream << " " << std::endl;
  stream << "    const __global float *src1_read0 = src1 + ( group_x * TILE_N ) * ldB + off1; " << std::endl;
  stream << " " << std::endl;
  stream << "    __local float slm_brow[8 * SLM_BLOCK]; " << std::endl;
  stream << "    __local float* slm_brow0; " << std::endl;
  stream << " " << std::endl;
  ...
  stream << "        ((__local float4 *)(slm_brow + mad24(0, SLM_BLOCK, local_index)))[0] = ((__global float4 *)(src1_read0 + mad24(0, ldB, local_index)))[0]; " << std::endl;
  stream << "        ((__local float4 *)(slm_brow + mad24(1, SLM_BLOCK, local_index)))[0] = ((__global float4 *)(src1_read0 + mad24(1, ldB, local_index)))[0]; " << std::endl;
  stream << "        ((__local float4 *)(slm_brow + mad24(2, SLM_BLOCK, local_index)))[0] = ((__global float4 *)(src1_read0 + mad24(2, ldB, local_index)))[0]; " << std::endl;
  stream << "        ((__local float4 *)(slm_brow + mad24(3, SLM_BLOCK, local_index)))[0] = ((__global float4 *)(src1_read0 + mad24(3, ldB, local_index)))[0]; " << std::endl;
  stream << "        ((__local float4 *)(slm_brow + mad24(4, SLM_BLOCK, local_index)))[0] = ((__global float4 *)(src1_read0 + mad24(4, ldB, local_index)))[0]; " << std::endl;
  stream << "        ((__local float4 *)(slm_brow + mad24(5, SLM_BLOCK, local_index)))[0] = ((__global float4 *)(src1_read0 + mad24(5, ldB, local_index)))[0]; " << std::endl;
  stream << "        ((__local float4 *)(slm_brow + mad24(6, SLM_BLOCK, local_index)))[0] = ((__global float4 *)(src1_read0 + mad24(6, ldB, local_index)))[0]; " << std::endl;
  stream << "        ((__local float4 *)(slm_brow + mad24(7, SLM_BLOCK, local_index)))[0] = ((__global float4 *)(src1_read0 + mad24(7, ldB, local_index)))[0]; " << std::endl;
  stream << "        barrier(CLK_LOCAL_MEM_FENCE); " << std::endl;
  stream << " " << std::endl;
  ...
  stream << "            float4 arow; " << std::endl;
  stream << "                             " << std::endl;
  stream << "            brow0 = ((__local float4 *)(slm_brow0 + 0 * SLM_BLOCK))[0]; " << std::endl;
  stream << "            brow1 = ((__local float4 *)(slm_brow0 + 1 * SLM_BLOCK))[0]; " << std::endl;
  stream << "            brow2 = ((__local float4 *)(slm_brow0 + 2 * SLM_BLOCK))[0]; " << std::endl;
  stream << "            brow3 = ((__local float4 *)(slm_brow0 + 3 * SLM_BLOCK))[0]; " << std::endl;
  stream << "            brow4 = ((__local float4 *)(slm_brow0 + 4 * SLM_BLOCK))[0]; " << std::endl;
  stream << "            brow5 = ((__local float4 *)(slm_brow0 + 5 * SLM_BLOCK))[0]; " << std::endl;
  stream << "            brow6 = ((__local float4 *)(slm_brow0 + 6 * SLM_BLOCK))[0]; " << std::endl;
  stream << "            brow7 = ((__local float4 *)(slm_brow0 + 7 * SLM_BLOCK))[0]; " << std::endl;
  stream << "              " << std::endl;
  stream << "#define MM_DOT_PRODUCT( _row, _dot )   \\ " << std::endl;
  stream << "            arow = ((__global float4 *)(src0_read + _row * ldA))[0];                           \\ " << std::endl;
  stream << "            _dot = mad( (float8)(arow.x), (float8)(brow0.x, brow1.x, brow2.x, brow3.x, brow4.x, brow5.x, brow6.x, brow7.x), _dot ); \\ " << std::endl;
  stream << "            _dot = mad( (float8)(arow.y), (float8)(brow0.y, brow1.y, brow2.y, brow3.y, brow4.y, brow5.y, brow6.y, brow7.y), _dot ); \\ " << std::endl;
  stream << "            _dot = mad( (float8)(arow.z), (float8)(brow0.z, brow1.z, brow2.z, brow3.z, brow4.z, brow5.z, brow6.z, brow7.z), _dot ); \\ " << std::endl;
  stream << "            _dot = mad( (float8)(arow.w), (float8)(brow0.w, brow1.w, brow2.w, brow3.w, brow4.w, brow5.w, brow6.w, brow7.w), _dot ); \\ " << std::endl;
  stream << "                         " << std::endl;
  ...
  stream << "        _brow = ((__local float4 *)(slm_brow0 + _row * SLM_BLOCK))[0]; \\ " << std::endl;
  ...
  stream << "    _dot = intel_sub_group_shuffle(_dot, 0) + intel_sub_group_shuffle(_dot, 1) + intel_sub_group_shuffle(_dot, 2) + intel_sub_group_shuffle(_dot, 3) +  \\ " << std::endl;
  stream << "           intel_sub_group_shuffle(_dot, 4) + intel_sub_group_shuffle(_dot, 5) + intel_sub_group_shuffle(_dot, 6) + intel_sub_group_shuffle(_dot, 7); \\ " << std::endl;
  stream << "     " << std::endl;
  ...
  stream << " " << std::endl;
  stream << "    float output = 0.0f; " << std::endl;
  ...
  stream << "    dst_write0[0] = mad(output, alpha, beta * dst_write0[0]); \\ " << std::endl;
  ...
  stream << " " << std::endl;
  stream << "__attribute__((reqd_work_group_size(8, LWG_HEIGHT, 1))) " << std::endl;
  stream << "__kernel void intelblas_gemm_buffer_TN( " << std::endl;
  stream << "    const __global float *src0, int off0, " << std::endl;
  stream << "    const __global float *src1, int off1, " << std::endl;
  stream << "    __global float *dst, int offd, " << std::endl;
  stream << "    int M, " << std::endl;
  ...
  stream << "    __global float *dst_write0 = dst + local_x * VEC_SIZE + ( group_x * TILE_N ) + ( group_y * LWG_HEIGHT * TILE_M + local_y * TILE_M) * ldC + offd; " << std::endl;
  stream << " " << std::endl;
  stream << "    const __global float *src0_read = src0 + (local_x * ( TILE_K / 8 ) + start_index) * ldA + group_y * LWG_HEIGHT * TILE_M + local_y * TILE_M + off0; " << std::endl;
  stream << " " << std::endl;
  stream << "    const __global float *src1_read0 = src1 + local_x * VEC_SIZE + ( group_x * TILE_N ) + start_index * ldB + off1; " << std::endl;
  stream << " " << std::endl;
  stream << "    float4 dot00 = (start_index != 0) ? ((__global float4 *)dst_write0)[0] : (beta * ((__global float4 *)dst_write0)[0]); " << std::endl;
  stream << "    float4 dot01 = (start_index != 0) ? ((__global float4 *)(dst_write0 + ldC))[0] : (beta * ((__global float4 *)(dst_write0 + ldC))[0]); " << std::endl;
  stream << "    float4 dot02 = (start_index != 0) ? ((__global float4 *)(dst_write0 + 2 * ldC))[0] : (beta * ((__global float4 *)(dst_write0 + 2 * ldC))[0]); " << std::endl;
  stream << "    float4 dot03 = (start_index != 0) ? ((__global float4 *)(dst_write0 + 3 * ldC))[0] : (beta * ((__global float4 *)(dst_write0 + 3 * ldC))[0]); " << std::endl;
  stream << "    float4 dot04 = (start_index != 0) ? ((__global float4 *)(dst_write0 + 4 * ldC))[0] : (beta * ((__global float4 *)(dst_write0 + 4 * ldC))[0]); " << std::endl;
  stream << "    float4 dot05 = (start_index != 0) ? ((__global float4 *)(dst_write0 + 5 * ldC))[0] : (beta * ((__global float4 *)(dst_write0 + 5 * ldC))[0]); " << std::endl;
  ...
  stream << "        float8 arow0 = alpha * ((__global float8 *)src0_read)[0]; " << std::endl;
  stream << "        float8 arow1 = alpha * ((__global float8 *)(src0_read + ldA))[0]; " << std::endl;
  stream << " " << std::endl;
  stream << "#define MM_DOT_PRODUCT( _arow ) \\ " << std::endl;
  stream << "        brow = ((__global float4 *)src1_read0)[0];  src1_read0 += ldB; \\ " << std::endl;
  stream << "        dot00 = mad( (float4)(_arow.s0), brow, dot00 ); \\ " << std::endl;
  stream << "        dot01 = mad( (float4)(_arow.s1), brow, dot01 ); \\ " << std::endl;
  stream << "        dot02 = mad( (float4)(_arow.s2), brow, dot02 ); \\ " << std::endl;
  stream << "        dot03 = mad( (float4)(_arow.s3), brow, dot03 ); \\ " << std::endl;
  stream << "        dot04 = mad( (float4)(_arow.s4), brow, dot04 ); \\ " << std::endl;
  stream << "        dot05 = mad( (float4)(_arow.s5), brow, dot05 ); \\ " << std::endl;
  stream << "        dot06 = mad( (float4)(_arow.s6), brow, dot06 ); \\ " << std::endl;
  stream << "        dot07 = mad( (float4)(_arow.s7), brow, dot07 ); \\ " << std::endl;
  stream << " " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow0, 0 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow1, 0 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow0, 1 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow1, 1 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow0, 2 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow1, 2 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow0, 3 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow1, 3 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow0, 4 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow1, 4 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow0, 5 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow1, 5 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow0, 6 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow1, 6 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow0, 7 ) ); " << std::endl;
  stream << "        MM_DOT_PRODUCT( intel_sub_group_shuffle( arow1, 7 ) ); " << std::endl;
  stream << "#undef MM_DOT_PRODUCT " << std::endl;
  stream << " " << std::endl;
  ...
  stream << "        float8 arow0 = ((start_index + local_x * 2) < K) ? (alpha * ((__global float8 *)src0_read)[0]) : 0.0f; " << std::endl;
  stream << "        float8 arow1 = ((start_index + local_x * 2 + 1) < K) ? (alpha * ((__global float8 *)(src0_read + ldA))[0]) : 0.0f; " << std::endl;
  stream << " " << std::endl;
  stream << "#define MM_DOT_PRODUCT( _arow ) \\ " << std::endl;
  stream << "        brow = (start_index < K) ? ((__global float4 *)src1_read0)[0] : 0.0f;  src1_read0 += ldB; start_index++; \\ " << std::endl;
  stream << "        dot00 = mad( (float4)(_arow.s0), brow, dot00 ); \\ " << std::endl;
  stream << "        dot01 = mad( (float4)(_arow.s1), brow, dot01 ); \\ " << std::endl;
  stream << "        dot02 = mad( (float4)(_arow.s2), brow, dot02 ); \\ " << std::endl;
  stream << "        dot03 = mad( (float4)(_arow.s3), brow, dot03 ); \\ " << std::endl;
  ...
  stream << "        } else if(mad24(global_x, 4, 2) < N) { " << std::endl;
  stream << "            __global float2 *dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            dst_write[0] = dot00.xy; dst_write0[2] = dot00.z; " << std::endl;
  stream << "            dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            if(mad24(global_y, 8, 1) < M) { " << std::endl;
  stream << "                dst_write[0] = dot01.xy; dst_write0[2] = dot01.z; " << std::endl;
  stream << "                dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            } else " << std::endl;
  stream << "                return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 2) < M) { " << std::endl;
  stream << "                dst_write[0] = dot02.xy; dst_write0[2] = dot02.z; " << std::endl;
  stream << "                dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            } else " << std::endl;
  stream << "                return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 3) < M) { " << std::endl;
  stream << "                dst_write[0] = dot03.xy; dst_write0[2] = dot03.z; " << std::endl;
  stream << "                dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            } else " << std::endl;
  stream << "                return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 4) < M) { " << std::endl;
  stream << "                dst_write[0] = dot04.xy; dst_write0[2] = dot04.z; " << std::endl;
  stream << "                dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            } else " << std::endl;
  stream << "                return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 5) < M) { " << std::endl;
  stream << "                dst_write[0] = dot05.xy; dst_write0[2] = dot05.z; " << std::endl;
  stream << "                dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            } else " << std::endl;
  stream << "                return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 6) < M) { " << std::endl;
  stream << "                dst_write[0] = dot06.xy; dst_write0[2] = dot06.z; " << std::endl;
  stream << "                dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            } else " << std::endl;
  stream << "                return; " << std::endl;
  stream << "            if(mad24(global_y, 8, 7) < M) { " << std::endl;
  stream << "                dst_write[0] = dot07.xy; dst_write0[2] = dot07.z; " << std::endl;
  stream << "            } " << std::endl;
  stream << "        } else if(mad24(global_x, 4, 1) < N) { " << std::endl;
  stream << "            __global float2 *dst_write = (__global float2 *)dst_write0; " << std::endl;
  stream << "            dst_write[0] = dot00.xy; dst_write0 += ldC; dst_write = (__global float2 *)dst_write0; " << std::endl;
  ...
  stream << " " << std::endl;
  stream << "__attribute__((reqd_work_group_size(8, LWG_HEIGHT, 1))) " << std::endl;
  stream << "__kernel void intelblas_gemm_buffer_TT( " << std::endl;
  stream << "    const __global float *src0, int off0, " << std::endl;
  stream << "    const __global float *src1, int off1, " << std::endl;
  stream << "    __global float *dst, int offd, " << std::endl;
  stream << "    int M, " << std::endl;
  ...
  stream << "    const int global_y = get_global_id(1); " << std::endl;
  stream << " " << std::endl;
  stream << "    float8 dot0 = 0.f; " << std::endl;
  stream << "    float8 dot1 = 0.f; " << std::endl;
  stream << "    float8 dot2 = 0.f; " << std::endl;
  stream << "    float8 dot3 = 0.f; " << std::endl;
  stream << " " << std::endl;
  stream << "    float16 brow0; " << std::endl;
  stream << "    float16 brow1; " << std::endl;
  stream << "    float16 brow2; " << std::endl;
  stream << "    float16 brow3; " << std::endl;
  stream << " " << std::endl;
  stream << "    __global float *dst_write0 = dst + local_x * VEC_SIZE + ( group_x * TILE_N ) + ( group_y * LWG_HEIGHT * TILE_M + local_y * TILE_M) * ldC + offd; " << std::endl;
  stream << " " << std::endl;
  stream << "    const __global float *src0_read = src0 + (local_x * ( TILE_K / 8 ) + start_index) * ldA + group_y * LWG_HEIGHT * TILE_M + local_y * TILE_M + off0; " << std::endl;
  stream << " " << std::endl;
  stream << "    const __global float *src1_read0 = src1 + (local_x * VEC_SIZE + ( group_x * TILE_N )) * ldB + start_index + off1; " << std::endl;
  stream << " " << std::endl;
  stream << "    float4 dot00 = (start_index != 0) ? ((__global float4 *)dst_write0)[0] : (beta * ((__global float4 *)dst_write0)[0]); " << std::endl;
  stream << "    float4 dot01 = (start_index != 0) ? ((__global float4 *)(dst_write0 + ldC))[0] : (beta * ((__global float4 *)(dst_write0 + ldC))[0]); " << std::endl;
  stream << "    float4 dot02 = (start_index != 0) ? ((__global float4 *)(dst_write0 + 2 * ldC))[0] : (beta * ((__global float4 *)(dst_write0 + 2 * ldC))[0]); " << std::endl;
  ...
  stream << "    int end_index = min(start_index + 256, K); " << std::endl;
  stream << "    while( start_index + TILE_K <= end_index ) { " << std::endl;
  stream << "        brow0 = ((__global float16 *)src1_read0)[0]; " << std::endl;
  stream << "        brow1 = ((__global float16 *)(src1_read0 + ldB))[0]; " << std::endl;
  stream << "        brow2 = ((__global float16 *)(src1_read0 + 2 * ldB))[0]; " << std::endl;
  stream << "        brow3 = ((__global float16 *)(src1_read0 + 3 * ldB))[0]; " << std::endl;
  stream << " " << std::endl;
  stream << "        float8 arow0 = alpha * ((__global float8 *)src0_read)[0]; " << std::endl;
  stream << "        float8 arow1 = alpha * ((__global float8 *)(src0_read + ldA))[0]; " << std::endl;
  stream << " " << std::endl;
  stream << "#define MM_DOT_PRODUCT( _brow, _dot) \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow0, 0 ), (float8)_brow.s0, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow1, 0 ), (float8)_brow.s1, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow0, 1 ), (float8)_brow.s2, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow1, 1 ), (float8)_brow.s3, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow0, 2 ), (float8)_brow.s4, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow1, 2 ), (float8)_brow.s5, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow0, 3 ), (float8)_brow.s6, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow1, 3 ), (float8)_brow.s7, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow0, 4 ), (float8)_brow.s8, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow1, 4 ), (float8)_brow.s9, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow0, 5 ), (float8)_brow.sa, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow1, 5 ), (float8)_brow.sb, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow0, 6 ), (float8)_brow.sc, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow1, 6 ), (float8)_brow.sd, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow0, 7 ), (float8)_brow.se, _dot ); \\ " << std::endl;
  stream << "        _dot = mad( intel_sub_group_shuffle( arow1, 7 ), (float8)_brow.sf, _dot ); \\ " << std::endl;
  stream << " " << std::endl;
  ...
  stream << " " << std::endl;
  stream << "    if(start_index < end_index) { " << std::endl;
  stream << "        brow0 = ((__global float16 *)src1_read0)[0];  src1_read0 += ldB; " << std::endl;
  stream << "        brow1 = ((__global float16 *)src1_read0)[0];  src1_read0 += ldB; " << std::endl;
  stream << "        brow2 = ((__global float16 *)src1_read0)[0];  src1_read0 += ldB; " << std::endl;
  stream << "        brow3 = ((__global float16 *)src1_read0)[0]; " << std::endl;
  stream << " " << std::endl;
  stream << "        float8 arow0 = alpha * ((__global float8 *)src0_read)[0]; " << std::endl;
  stream << "        float8 arow1 = alpha * ((__global float8 *)(src0_read + ldA))[0]; " << std::endl;
  stream << " " << std::endl;
  stream << "#define MM_DOT_PRODUCT( _brow, _dot) \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow0, 0 ), (float8)_brow.s0, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow1, 0 ), (float8)_brow.s1, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow0, 1 ), (float8)_brow.s2, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow1, 1 ), (float8)_brow.s3, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow0, 2 ), (float8)_brow.s4, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow1, 2 ), (float8)_brow.s5, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow0, 3 ), (float8)_brow.s6, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow1, 3 ), (float8)_brow.s7, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow0, 4 ), (float8)_brow.s8, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow1, 4 ), (float8)_brow.s9, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow0, 5 ), (float8)_brow.sa, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow1, 5 ), (float8)_brow.sb, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow0, 6 ), (float8)_brow.sc, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow1, 6 ), (float8)_brow.sd, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow0, 7 ), (float8)_brow.se, _dot ) : _dot; \\ " << std::endl;
  stream << "        _dot = (w++ < K) ? mad( intel_sub_group_shuffle( arow1, 7 ), (float8)_brow.sf, _dot ) : _dot; \\ " << std::endl;
  stream << " " << std::endl;
  stream << "        int w = start_index; " << std::endl;
  stream << "        MM_DOT_PRODUCT( brow0, dot0 ); " << std::endl;
  ...
  stream << "    } " << std::endl;
  stream << " " << std::endl;
  stream << "    dot00 += (float4)(dot0.s0, dot1.s0, dot2.s0, dot3.s0); " << std::endl;
  stream << "    dot01 += (float4)(dot0.s1, dot1.s1, dot2.s1, dot3.s1); " << std::endl;
  stream << "    dot02 += (float4)(dot0.s2, dot1.s2, dot2.s2, dot3.s2); " << std::endl;
  stream << "    dot03 += (float4)(dot0.s3, dot1.s3, dot2.s3, dot3.s3); " << std::endl;
  stream << "    dot04 += (float4)(dot0.s4, dot1.s4, dot2.s4, dot3.s4); " << std::endl;
  stream << "    dot05 += (float4)(dot0.s5, dot1.s5, dot2.s5, dot3.s5); " << std::endl;
  stream << "    dot06 += (float4)(dot0.s6, dot1.s6, dot2.s6, dot3.s6); " << std::endl;
  stream << "    dot07 += (float4)(dot0.s7, dot1.s7, dot2.s7, dot3.s7); " << std::endl;
  stream << " " << std::endl;
  stream << "    if(global_x * 4 < N && global_y * 8 < M) { " << std::endl;
  stream << "        if(mad24(global_x, 4, 3) < N) { " << std::endl;
  stream << "            __global float4 *dst_write = (__global float4 *)dst_write0; " << std::endl;
...
void intelblas_gemm::enqueue(driver::CommandQueue & queue, driver::Program const & program, std::string const & suffix, runtime::execution_handler const & control)
{
  ...
  std::string name[4] = {"intelblas_gemm_buffer_NN", "intelblas_gemm_buffer_NT", "intelblas_gemm_buffer_TN", "intelblas_gemm_buffer_TT"};
