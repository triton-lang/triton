 ...
 */
auto getRepShapeForCvtLayout(const triton::gpu::ConvertLayoutOp &op)
    -> std::vector<unsigned> {
  ...
  const auto srcShapePerCTATile =
      getShapePerCTATile(srcLayout, srcTy.getShape());
  const auto dstShapePerCTATile =
      getShapePerCTATile(dstLayout, dstTy.getShape());
 ...
 */
auto getScratchConfigForCvtLayout(const triton::gpu::ConvertLayoutOp &op,
  ...
  const auto srcContigPerThread =
      getUniqueContigPerThread(srcLayout, srcTy.getShape())[inOrd[0]];
  const auto dstContigPerThread =
...
// Additional modernizations and refactoring can be applied to other functions
// as needed.
