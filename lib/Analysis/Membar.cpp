...
void MembarAnalysis::insertBarrier(Operation *op, OpBuilder *builder) {
  ...
  auto barrierOp =
      builder->create<triton::gpu::LocalMemoryBarrierOp>(op->getLoc());
}

void MembarAnalysis::update(Operation *op, BlockInfo *blockInfo,
                            FuncBlockInfoMapT *funcBlockInfoMap,
                            OpBuilder *builder) {
  if (isa<gpu::BarrierOp, triton::gpu::LocalMemoryBarrierOp>(op)) {
  ...
  if (isa<triton::gpu::AsyncWaitOp, triton::nvidia_gpu::TMAStoreWaitOp>(op) &&
      !isa<gpu::BarrierOp, triton::gpu::LocalMemoryBarrierOp>(
          op->getNextNode())) {
