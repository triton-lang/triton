...
Attribute makeEncodingFromLinearLayout(MLIRContext *ctx, LinearLayout ll) {
  if (isLinearEncodingCompatible(ll))
...
SmallVector<unsigned> getWarpsPerCTA(Attribute layout,
                                     ArrayRef<int64_t> shape) {
  return dispatchEncoding(layout, shape,
                          [](auto enc) { return enc.getWarpsPerCTA(); });
}
...
SmallVector<unsigned> getWarpOrder(DistributedEncodingTrait layout,
                                   ArrayRef<int64_t> shape) {
  return dispatchEncoding(Attribute(layout), shape,
                          [](auto enc) { return enc.getWarpOrder(); });
}
...
SmallVector<unsigned> getCTAsPerCGA(Attribute layout) {
  ...
  } else if (auto genericLinear = dyn_cast<GenericLinearEncodingAttr>(layout)) {
...
LinearEncodingAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (!isLinearEncodingCompatible(linearLayout)) {
    return emitError()
           << "LinearEncodingAttr requires non-swizzled bases (at most one "
              "non-zero per basis) and bijectivity after removing broadcasts";
  }
...
static SmallVector<unsigned> getContigImpl(const LinearLayout &ll,
                                           MLIRContext *ctx, const char *inDim,
...
GenericLinearEncodingAttr::verify(function_ref<InFlightDiagnostic()> emitError,
  ...
  if (!linearLayout.isSurjective()) {
    return emitError() << "GenericLinearEncodingAttr requires a surjective "
                          "layout";
  }
...
SmallVector<unsigned> GenericLinearEncodingAttr::getContig(
    const char *inDim, SmallVector<unsigned int> lowerContig) const {
  return getContigImpl(getLinearLayout(), getContext(), inDim, lowerContig,
                       getOrder());
}
...
unsigned GenericLinearEncodingAttr::getTotalElemsPerThread(
