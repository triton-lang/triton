...
SmallVector<unsigned> getUniqueContigPerThread(Attribute layout,
  ...
  llvm::outs() << "Calling getUniqueContigPerThread ... rank = " << rank
               << "\n";
  llvm::outs() << "contigPerThread.size() = " << contigPerThread.size() << "\n";
  llvm::outs() << layout << "\n";
...
unsigned AMDMfmaEncodingAttr::getTotalElemsPerThreadForOperands(
  ...
  // Note: here one "element" refers to one vector of kWidth elements in the
  // operand This is used by the TypeConverter
  // TODO(Lixun): Refactor to make it less confusing
