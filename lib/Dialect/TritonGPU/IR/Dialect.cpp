...
bool isExpensiveCat(CatOp cat, Attribute targetEncoding) {
  ...
  // the operands.
  auto lhsTy = cat.getLhs().getType();
  int64_t operandRegs =
      getNumNonBroadcastRegisters(lhsTy.getShape(), lhsTy.getEncoding()) * 2;
  int64_t resultRegs =
...
struct TritonGPUInferLayoutInterface
  ...
  LogicalResult verifyCatOpEncodingCompatibility(Operation *op) const override {
    ...
    int64_t operandRegs =
        getNumNonBroadcastRegisters(lhsTy.getShape(), lhsTy.getEncoding()) * 2;
    int64_t resultRegs = getNumNonBroadcastRegisters(
        cat.getType().getShape(), cat.getType().getEncoding());
