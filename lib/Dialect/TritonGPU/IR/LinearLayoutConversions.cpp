...
AMDMfmaEncodingAttr::toLinearLayout(ArrayRef<int64_t> shape) const {
  ...
  if (getMDim() == 32) {
    ...
    if (elementType->getIntOrFloatBitWidth() <= 32) {
      ...
      // For 64 bit mfma with 16x16 output, each of the 64 threads holds 4 elements across 8 VGPRs.
      // each 64 bit element is split across pairs of 2 VGPRs each.
      // The first VGPR holds the first 32 bits and second holding the last 32 bits.
      //
      // For the register (i.e., element) dimension, these 4 elements are along
      // the matrix C's M dimension, with 4 consecutive elements spanning 4 rows.
      //
      // For the lane (i.e., thread) dimension, these threads are along the
      // matrix C's N dimension, with each group of 16 consecutive threads covering a whole
      // adjacent row. Unlike the <=32 bit cases, there's no row gaps between the groups.
      tileLayout = LinearLayout(
        {{kRegister, {{0, 4}, {0, 8}}},
