...
LinearLayout replaceFirstRepWithLane(LinearLayout inLayout, MLIRContext *ctx) {
  const auto rank = static_cast<int>(inLayout.getOutDims().size());
  auto outDimNames = standardOutDimNames(ctx, rank);
  ...
  // We want to "move" the third register basis (index 2)
  // into the fifth lane basis slot (index 4), if present.
  StringAttr kRegister = S("register");
  StringAttr kLane = S("lane");

  constexpr int kRegisterIndexToRemove = 2;
...
LinearLayout chooseScaledWmmaScaleLayout(MLIRContext *ctx, int dotOperandIdx,
  ...
  auto nonOpSelLayout = combineCtaCgaWithShape(
      ctaLayout, CGAEncodingAttr::getDefault(ctx, /*rank=*/rank),
      dotOperandShape);
  ...
  // next tile computed by the same warp (aka it's first repetition), if there
  // is one. So register base that naturally representes first repetition needs
  // to be moved to lane base that representes lane 16.
  // Since for a single tile thread holds 4 vals, we move register base 2, to
  // lane base 4.
  auto retLayout = replaceFirstRepWithLane(nonOpSelLayout, ctx);
  return retLayout;
}
