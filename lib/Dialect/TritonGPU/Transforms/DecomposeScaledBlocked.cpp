...
TypedValue<RankedTensorType> DecomposeScaledBlocked::broadcastScale(
  ...
  shape.push_back(1);
  auto expandScale = ReshapeOp::create(rewriter, loc, shape, scale);
  expandScale.setEfficientLayout(true);
  scale = expandScale;
  ...
  auto broadcastScale = BroadcastOp::create(
      rewriter, loc, scale.getType().clone(scaleShape), scale);
...
TypedValue<RankedTensorType> DecomposeScaledBlocked::extendAndBroadcastScale(
  ...
  auto reshapeScale = broadcastScale(rewriter, scaledDotOp, scale16, kDim);
