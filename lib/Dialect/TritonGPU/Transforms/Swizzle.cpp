...
struct SwizzlePass: public TritonGPUSwizzleBase<SwizzlePass> {
  ...
  SwizzleInfo getSwizzleMMA(int opIdx, triton::gpu::MmaEncodingAttr retEncoding, RankedTensorType ty) {
    ...
    int perPhase = 128 / (ty.getShape()[order[0]] * ty.getElementType().getIntOrFloatBitWidth());
    perPhase = std::max<int>(perPhase, 1);
    // index of the inner dimension in `order`
    int inner = (opIdx == 0) ? 0 : 1;
    if (version == 1){
        int maxPhase = (order[inner] == 1 ? 8 : 4) / perPhase;
        // TODO: handle rep (see https://github.com/openai/triton/blob/master/lib/codegen/analysis/layout.cc#L209)
        int vec = 1;
        return SwizzleInfo{vec, perPhase, maxPhase};
    }
    else if (version == 2){
        auto eltTy = ty.getElementType();
        std::vector<size_t> mat_shape = {8, 8, 4 * 64 / eltTy.getIntOrFloatBitWidth()};
