...
// fails.  :)
static bool checkLoadShapeAgainstMMATile(Operation *loadOp) {
  // Go through transitive uses of the loadOp in the same block.
  // If the user is a DotOp ...
  for (Operation *user : loadOp->getUsers()) {
    ...
    if (auto dot = dyn_cast<tt::DotOp>(user)) {
      if (auto dotEnc = dyn_cast<ttg::NvidiaMmaEncodingAttr>(
        ...
        auto loadTy = cast<RankedTensorType>(loadOp->getResultTypes()[0]);
    ...
    // recursion
    bool retCode = checkLoadShapeAgainstMMATile(user);
    if (retCode)
      return true;
  }
  return false;
...
static bool checkIncompatibleDotEncoding(Operation *loadOp) {
  // Go through transitive uses of the loadOp in the same block.
  // If the user is a DotOp ...
  for (Operation *user : loadOp->getUsers()) {
    if (user->getBlock() != loadOp->getBlock())
      continue;
    // recursion
    bool retCode = checkIncompatibleDotEncoding(user);
    if (retCode)
      return true;
  }
  return false;
...
assignMemoryLayouts(scf::ForOp &forOp,
  ...
  llvm::MapVector<Operation *, LoadInfo> loadToInfo;
  llvm::SmallVector<Operation *> pipelinedLoads;
  for (Operation &op : forOp.getBody()->without_terminator()) {
    if (op.hasAttr("loop.load"))
      pipelinedLoads.push_back(&op);
  }

  for (auto *op : pipelinedLoads) {
    if (loadToInfo.count(op))
      // TODO pawel: err, we'd need to verify that the distance is the same
      continue;
    LLVM_DEBUG({
      LDBG("check loop.load ");
      op->dump();
    });
    if (checkLoadShapeAgainstMMATile(op)) {
      LDBG("-- load shape against MMA tile failed");
      continue;
    }
    if (checkIncompatibleDotEncoding(op)) {
      assert(true && "TODO: handle incompatible DotEncoding");
      continue;
    }

    // Go through each transitive user.
    for (auto use : getTransitiveUserInBlock(op)) {
      LLVM_DEBUG({
        LDBG("-- check use ");
        use->dump();
      });
    ...
    LDBG("-- do not have shared encoding");
    assert(loadToInfo.count(op));
    assert(loadToInfo[op].sharedEncoding);
  }
  return loadToInfo;
}
...
static llvm::MapVector<Operation *, LoadInfo> getLoopSchedule(scf::ForOp forOp,
                                                              int numStages) {
...
bool mlir::triton::preProcessLoopAndGetSchedule(
  ...
  llvm::MapVector<Operation *, LoadInfo> loadToInfo =
      getLoopSchedule(forOp, numStages);
  if (loadToInfo.empty())
    return false;
