...
// Return true if an operation may modify global memory. Unknown effects and
// effects on the default resource may alias global memory; explicitly disjoint
// resources, such as shared memory, do not.
  ...
  return llvm::any_of(
      *effects, [](const MemoryEffects::EffectInstance &effect) {
        ...
        if (effect.getResource() == SideEffects::DefaultResource::get())
          return true;
        return !effect.getResource()->isDisjointFrom(GlobalMemory::get());
      });
}

static bool hasSingleGlobalReadEffect(Operation *op) {
  auto effects = getEffectsRecursively(op);
  return effects && effects->size() == 1 &&
         isa<MemoryEffects::Read>(effects->front().getEffect()) &&
         isa<GlobalMemory>(effects->front().getResource());
}

// Only hoist operations that are safe to move and "cheap" (i.e. only scalar
...
static bool canHoistLoopBoundComputation(Operation *op, bool allowGlobalReads) {
  auto isScalar = [](Type type) {
    return type.isIntOrIndexOrFloat() || isa<PointerType>(type);
  };
  return (isMemoryEffectFree(op) ||
          (allowGlobalReads && hasSingleGlobalReadEffect(op))) &&
...
static bool isOuterLoopInvariant(mlir::DominanceInfo &domInfo, scf::ForOp outer,
                                 bool allowGlobalReads, ArrayRef<Value> values,
                                 llvm::SetVector<Operation *> &toHoist) {
  return getDominatingValueSetOpsToHoist(
      domInfo, outer, values, toHoist,
      [allowGlobalReads](Operation *op) {
        return canHoistLoopBoundComputation(op, allowGlobalReads);
      },
      [&](BlockArgument arg) {
...
static void fuseOneLevel(LoopNestNode *parent, mlir::DominanceInfo &domInfo) {
  ...
  bool allowGlobalReads = !mayModifyGlobalMemory(outer);
  ...
  for (LoopNestNode *child : parent->children) {
    ...
    if (isOuterLoopInvariant(
            domInfo, outer, allowGlobalReads,
...
static LogicalResult speculateInnerLoopLength(scf::ForOp outerLoop,
  ...
  llvm::SetVector<Operation *> toHoist;
  bool allowGlobalReads = !mayModifyGlobalMemory(outerLoop);
  if (!isOuterLoopInvariant(domInfo, outerLoop, allowGlobalReads,
