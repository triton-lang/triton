...
// we know they will never divide by zero. A scalar `tt.load` is the one read
// admitted: the outer loop may run zero times, in which case the source program
// never executes the load, so `hoistBoundOpsBefore` masks it with the outer
// loop trip test once it sits above the loop.
static bool canHoistLoopBoundComputation(Operation *op) {
  ...
  return (isMemoryEffectFree(op) || isa<LoadOp>(op)) &&
         llvm::all_of(op->getOperandTypes(), isScalar) &&
         llvm::all_of(op->getResultTypes(), isScalar);
}

// Hoist the loop bound computations above `outer`. A hoisted `tt.load` is
// speculated across the outer loop trip test, so it gets that test as its
// mask, the same way `triton-licm` guards a load it moves out of a loop.
static void hoistBoundOpsBefore(scf::ForOp outer,
  ...
  for (Operation *op : toHoist) {
    ...
    if (!loadOp)
      continue;
    IRRewriter rewriter(loadOp);
    Value runs = arith::CmpIOp::create(
        rewriter, outer.getLoc(),
        outer.getUnsignedCmp() ? arith::CmpIPredicate::ult
                               : arith::CmpIPredicate::slt,
        outer.getLowerBound(), outer.getUpperBound());
...
static void fuseOneLevel(LoopNestNode *parent, mlir::DominanceInfo &domInfo) {
  ...
  // be fusible.
  hoistBoundOpsBefore(outer, toHoist);
...
static LogicalResult speculateInnerLoopLength(scf::ForOp outerLoop,
  ...
  hoistBoundOpsBefore(outerLoop, toHoist);
