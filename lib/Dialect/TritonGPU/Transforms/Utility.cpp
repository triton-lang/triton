...
static Attribute inferSrcEncoding(triton::ReshapeOp op, Attribute encoding) {
  if (op.getRequireSliced()) {
    auto srcEncoding =
        dyn_cast<ttg::SliceEncodingAttr>(op.getSrc().getType().getEncoding());
    if (!srcEncoding || op.getType().getEncoding() != srcEncoding.getParent()) {
      return {};
    }
    auto parent = dyn_cast<ttg::DistributedEncodingTrait>(encoding);
    if (!parent)
      return {};
    return ttg::SliceEncodingAttr::get(op.getContext(), *op.getExpandDimsAxis(),
                                       parent);
  }
