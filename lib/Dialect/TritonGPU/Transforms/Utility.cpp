...
void runDeadBlockArgumentElimination(Operation *op) {
  // Use RunLivenessAnalysis which wraps DataFlowSolver with
  // DeadCodeAnalysis + LivenessAnalysis. This is cached by the pass manager.
  dataflow::RunLivenessAnalysis la{op};

  // Clean up dead block arguments in for loops by replacing them with
  // their init values.
