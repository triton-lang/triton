  ...
  for (Operation &op : forOp.getBody()->without_terminator()) {
    if (auto cvtOp = dyn_cast<ttg::ConvertLayoutOp>(op)) {
      ...
      if (isLoadFromTensorPtr(loadOp)) {
        ...
        if (loadsEmptyBarriers.count(loadOp)) {
          auto users = mapping.lookup(loadOp).getUsers();
          DenseMap</*consumer=*/Operation *, /*stage=*/int> consumerStageMap;
          ...
          auto CTALayout = ttg::getCTALayout(
              loadOp.getType().cast<RankedTensorType>().getEncoding());
          ConsumerReleaseInfo info{
              loopIterIdx, pipelineIterIdx, curPhase,  curIV,
              step,        upperBound,      CTALayout, consumerStageMap};
          consumerReleaseMap[loadsEmptyBarriers[loadOp]] = info;
        }
        ...
        if (!loadsCanShareBarriers[loadOp]) {
          ...
          Value fullBarrier = mBarBuilder.create<ttng::ExtractMBarrierOp>(
              loadOp.getLoc(), mBarTy, loadsFullBarriers[loadOp], loopIterIdx);
...
void LoopPipeliner::prefetchNextIteration(scf::ForOp newForOp,
  ...
  Value insertSliceIndex = getBoundedIterationValue(
      builder, pipelineIterIdx, numStagesVal, pipelineIterIdx, initVal);
  loopIterIdx = newForOp.getRegionIterArgs()[ivIdx + 2];
  Value nextLoopIterIdx = builder.create<arith::AddIOp>(
      nextIV.getLoc(), loopIterIdx,
      builder.create<arith::ConstantIntOp>(nextIV.getLoc(), 1, 32));
  Value extractSliceIndex = getBoundedIterationValue(
      builder, nextLoopIterIdx, numStagesVal, nextLoopIterIdx, initVal);
    ...
    if (validLoads.contains(op->getResult(0))) {
      ...
      if (mode && isLoadFromTensorPtr(loadOp)) {
        ...
        if (loadsEmptyBarriers.count(loadOp)) {
          ...
          Value emptyPhase =
              builder.create<arith::ConstantIntOp>(loc, 1, /*bitWidth*/ 1);
          builder.create<ttng::MBarrierWaitOp>(loc, emptyBarrier, emptyPhase);
