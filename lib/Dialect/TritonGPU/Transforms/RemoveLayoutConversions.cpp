...
constexpr llvm::StringLiteral kForceCGARematerialization =
    "ttg.force_cga_rematerialization";
...
std::optional<Attribute> cloneWithCGALayout(RankedTensorType type,
  ...
  int numWarps = lookupNumWarps(scope);
  OpBuilder builder(scope);
  int threadsPerWarp = lookupThreadsPerWarp(builder);
  ...
  if (auto slice = dyn_cast<SliceEncodingAttr>(layout)) {
    auto parent = dyn_cast<BlockedEncodingAttr>(slice.getParent());
    if (!parent)
      return std::nullopt;
    auto newParent = BlockedEncodingAttr::get(
        ...
        cgaLayout);
    return SliceEncodingAttr::get(type.getContext(), slice.getDim(), newParent);
  }
...
// `preferredEncoding`. Dot operand encodings planned by AssignCTALayouts carry
// additional parent-layout decisions, so reuse the preferred encoding when it
// describes the same operand. Other producers retain their existing intra-CTA
// layout and only adopt the preferred CGA layout.
...
getEncodingInCGA(Value value, Attribute preferredEncoding, Operation *scope) {
  ...
  auto currentDot = dyn_cast<DotOperandEncodingAttr>(type.getEncoding());
  auto preferredDot = dyn_cast<DotOperandEncodingAttr>(preferredEncoding);
  if (currentDot && preferredDot &&
      currentDot.getOpIdx() == preferredDot.getOpIdx() &&
      currentDot.getKWidth() == preferredDot.getKWidth())
    return preferredEncoding;

  return cloneWithCGALayout(type, getCGALayout(preferredEncoding), scope);
}

// Rematerialize the producer slice of a marked conversion in the requested CTA
// layout. Unlike the normal profitability-driven rematerializer, this rewrite
// treats global loads as leaves: their tensor operands are converted to the new
// blocked layout, and the load itself is cloned. This preserves coalescing
// choices while avoiding a cross-CTA conversion of the loaded value.
class ForcedCGARematerialization {
public:
  explicit ForcedCGARematerialization(ConvertLayoutOp root) : root(root) {}
  ...
  LogicalResult run() {
    ...
    Value newSrc = rewrite(root.getSrc());
    root.getSrcMutable().assign(newSrc);
    return success();
  }
...
private:
  LogicalResult plan(Value value, Attribute encoding) {
    ...
    if (auto load = dyn_cast<LoadOp>(op)) {
      // Duplicating a volatile load changes observable behavior when the
      // original value has other users.
    ...
    for (auto [index, operand] : llvm::enumerate(op->getOpOperands())) {
      ...
      if (auto upcast = dyn_cast<UpcastFpOpInterface>(op))
  ...
  Value rewrite(Value value) {
    ...
    if (isa<LoadOp>(op)) {
      for (OpOperand &operand : op->getOpOperands()) {
        ...
        Value converted = ConvertLayoutOp::create(builder, op->getLoc(),
                                                  newType, operand.get());
    ...
    newOp->removeAttr(kForceCGARematerialization);
...
void forceCGARematerialization(ModuleOp module) {
  SmallVector<ConvertLayoutOp> converts;
  module.walk([&](ConvertLayoutOp convert) {
    if (convert->hasAttr(kForceCGARematerialization))
      converts.push_back(convert);
  });

  for (ConvertLayoutOp convert : converts) {
    if (failed(ForcedCGARematerialization(convert).run()))
      LDBG("failed forced CGA rematerialization for " << convert);
    convert->removeAttr(kForceCGARematerialization);
  }
}
...
public:
  ...
  void runOnOperation() override {
    ...
    // Honor layout decisions made by AssignCTALayouts before canonicalization
    // or forward propagation can rewrite the marked conversion boundaries.
    forceCGARematerialization(m);
