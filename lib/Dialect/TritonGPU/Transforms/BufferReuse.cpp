...
static void reuseBuffers(FuncOp func) {
  ...
  LiveBuffersAnalysis analysis(func, preds);
  ...
  for (auto [id, buffer] : llvm::enumerate(analysis.getBuffers())) {
  ...
  size_t numBuffers = analysis.getBuffers().size();
  bool changed;
  do {
    changed = false;
    ...
    for (MergedBuffer *mergedBuffer : mergedBuffers) {
      if (mergedBuffer->ops.empty())
        continue;
      BitVector liveMask = analysis.getLiveBufferMask(mergedBuffer->ids);
