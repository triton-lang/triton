...
static Interval<int> getLiveIntervals(Value value, Liveness &liveness,
  ...
  while (!users.empty()) {
    ...
    if (!isa<MemDescSubviewOp, MemDescReinterpretOp>(user))
      continue;
...
static Operation *getAlloc(Value value) {
  while (true) {
    if (auto allocOp = value.getDefiningOp<TMEMAllocOp>())
      return allocOp;
    if (auto subviewOp = value.getDefiningOp<MemDescSubviewOp>()) {
      value = subviewOp.getSrc();
      continue;
    }
    if (auto reinterpOp = value.getDefiningOp<MemDescReinterpretOp>()) {
