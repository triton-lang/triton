...
void ClusterBarrierAnalysis::update(Operation *op, BlockInfo *blockInfo,
  ...
  if (scratchBufferId != Allocation::InvalidBufferId) {
    ...
    // Scratch-buffer ops synchronize their internal write->read transition.
    blockInfo->sync();
...
struct ClusterBarrierInsertionPass
  ...
  void runOnOperation() override {
    ModuleAllocation moduleAllocation(getOperation());
    runClusterBarrierInsertion(moduleAllocation, computeCapability);
  }
};
