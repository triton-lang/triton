  ...
  if (srcEnc) {
    auto result = cast<DialectInferLayoutInterface>(&srcEnc.getDialect())
                      ->inferReshapeOpEncoding(
                          srcTy.getShape(), srcEnc, shape, dstEnc, allowReorder,
                          /*requireSliced=*/false, state.location);
...
LogicalResult ReshapeOp::canonicalize(ReshapeOp op, PatternRewriter &rewriter) {
  if (op.getEfficientLayout() || op.getRequireSliced())
    return failure();
  ...
  if (auto parentReshape = dyn_cast<ReshapeOp>(definingOp)) {
    ...
    rewriter.replaceOpWithNewOp<ReshapeOp>(
        ...
        op.getEfficientLayout(), op.getRequireSliced());
    return success();
  }
