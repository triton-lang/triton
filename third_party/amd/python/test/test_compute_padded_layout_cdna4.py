...
def test_compute_padded_layout_cdna4_basic():
    ...
    r = amd.compute_padded_layout_cdna4(op_idx=0, k_width=8, mfma_non_k_dim=16, k_dim=64, non_k_dim=128, elem_bytes=2,
                                        is_k_contig=True)
...
def test_compute_padded_layout_cdna4_invalid_mfma_dim():
    ...
    r = amd.compute_padded_layout_cdna4(op_idx=0, k_width=8, mfma_non_k_dim=64, k_dim=64, non_k_dim=128, elem_bytes=2,
                                        is_k_contig=True)
    assert r is None
...
def test_compute_padded_layout_cdna4_invalid_kwidth():
    ...
    from triton._C.libtriton import amd
    r = amd.compute_padded_layout_cdna4(op_idx=0, k_width=2, mfma_non_k_dim=16, k_dim=64, non_k_dim=128, elem_bytes=2,
                                        is_k_contig=True)
    assert r is None
...
def test_compute_padded_layout_cdna4_invalid_elem_bytes():
    ...
    from triton._C.libtriton import amd
    r = amd.compute_padded_layout_cdna4(op_idx=0, k_width=8, mfma_non_k_dim=16, k_dim=64, non_k_dim=128, elem_bytes=4,
                                        is_k_contig=True)
    assert r is None
...
def test_compute_padded_layout_cdna4_op_b():
    ...
    from triton._C.libtriton import amd
    r = amd.compute_padded_layout_cdna4(op_idx=1, k_width=8, mfma_non_k_dim=16, k_dim=64, non_k_dim=128, elem_bytes=2,
                                        is_k_contig=True)
    assert r is not None
    assert r["interval"] > 0
    assert len(r["bases"]) > 0
...
def test_compute_padded_layout_cdna4_fp8():
    ...
    from triton._C.libtriton import amd
    r = amd.compute_padded_layout_cdna4(op_idx=0, k_width=16, mfma_non_k_dim=16, k_dim=128, non_k_dim=128, elem_bytes=1,
                                        is_k_contig=True)
    assert r is not None
    assert r["interval"] > 0
