...
@pytest.mark.parametrize("input_size, block_size, num_blocks, increment, initial_offset, expect_optimization", [
    (256 * 32, 256, 32, 256, 0, True),  # step forward
    (256 * 32, 256, 32, -256, 256 * 31, True),  # step backward
    (2**29 - 1, 256, 2, 1, 2**29 - 1 - 256 - 1, True)  # step forward near the upper limit of befferized tensor size
])
def test_buffer_op_ptr_optimization(input_size, block_size, num_blocks, increment, initial_offset, expect_optimization,
                                    device):
    ...
    for i in range(num_blocks):
        for j in range(block_size):
            ref_output[j] += X[(initial_offset + increment * i + j) % input_size]

    grid = (1, )

    pgm = kernel[grid](X, Y, block_size, num_blocks, increment, initial_offset, num_warps=1)

    listing = pgm.asm["ttgir"]
