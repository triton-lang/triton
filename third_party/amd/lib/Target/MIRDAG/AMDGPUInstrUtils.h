...
// on names.
//
// String matching is brittle: it silently breaks if an opcode is renamed.
// Remove this file and switch to the target predicates once a public AMDGPU
...
inline bool isVMEMOp(const MachineInstr &MI) {
  ...
  return Name.starts_with("BUFFER_LOAD") || Name.starts_with("BUFFER_STORE") ||
         Name.starts_with("BUFFER_ATOMIC") || Name.starts_with("GLOBAL_LOAD") ||
         Name.starts_with("GLOBAL_STORE") ||
         Name.starts_with("GLOBAL_ATOMIC") || Name.starts_with("FLAT_LOAD") ||
         Name.starts_with("FLAT_STORE") || Name.starts_with("FLAT_ATOMIC");
}
