...
ChangeResult IntegerValueRangeLatticeWithMeet::meet(
    const dataflow::AbstractSparseLattice &lattice) {
  auto lattice_ =
      static_cast<const dataflow::IntegerValueRangeLattice &>(lattice);
  IntegerValueRange oldRange = getValue();
  IntegerValueRange incomingRange = lattice_.getValue();
  IntegerValueRange inferredRange = incomingRange;
  if (!oldRange.isUninitialized()) {
    assert(!incomingRange.isUninitialized() &&
           "expected initialized incoming range");
    LDBG("old range: " << oldRange);
    LDBG("incoming range: " << incomingRange);
    inferredRange = oldRange.getValue().intersection(incomingRange.getValue());
    LDBG("inferred range: " << inferredRange);
    if (!(oldRange.getValue() == incomingRange.getValue())) {
      LDBG("change!");
      // TODO(max): this doesn't work because IntegerValueRangeLattice::value is
      // private. this->value = inferredRange;
