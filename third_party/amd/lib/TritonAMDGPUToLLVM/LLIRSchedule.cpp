...
struct Utils {
  ...
  // Fence-aware fixup. PR #10383 lowers workgroup barriers as the triple
  //   fence release ; s.barrier ; fence acquire    (MMRA-annotated, LDS scope)
  // llirSched recognizes/relocates the s.barrier and the MFMAs but NOT the
  // fences (they classify as Other). The release fence therefore drifts up
  // next to the producing LDS store, so the backend lowers it to an
  // s_waitcnt lgkmcnt(0) that drains LDS *before* the hot MFMA run instead of
  // at the barrier, exposing LDS latency the pinned (manual-waitcnt) schedule
  // hid. Re-glue each barrier's release fence to immediately before it and its
  // acquire fence to immediately after it, restoring release;barrier;acquire so
  // the backend places the drain at the barrier (overlapped by the MFMAs).
  static bool isFenceWithOrdering(const Instruction &I, AtomicOrdering Ord) {
    const auto *F = dyn_cast<FenceInst>(&I);
    return F && F->getOrdering() == Ord;
  }

  static void reglueFencesToBarriers(BasicBlock &BB) {
    // Collect barriers and the (identical, LDS-scoped) release/acquire fences
    // into pools. The fences are interchangeable (same syncscope + !mmra), so
    // we don't need to recover the original pairing across the loop back-edge;
    // we just need EACH barrier to have a release immediately before it and an
    // acquire immediately after it. That keeps the backend's fence-derived
    // lgkmcnt(0) at the barrier (overlapped by the MFMAs) instead of stranded
    // next to the producing LDS store.
    SmallVector<Instruction *, 8> Bars, Rels, Acqs;
    for (Instruction &I : BB) {
      if (isFenceWithOrdering(I, AtomicOrdering::Release))
        Rels.push_back(&I);
      else if (isFenceWithOrdering(I, AtomicOrdering::Acquire))
        Acqs.push_back(&I);
      else if (classifyInstruction(I) == InstClass::sBarrier)
        Bars.push_back(&I);
    }
    for (size_t i = 0; i < Bars.size(); ++i) {
      if (i < Rels.size())
        Rels[i]->moveBefore(Bars[i]->getIterator());
      if (i < Acqs.size())
        Acqs[i]->moveAfter(Bars[i]);
    }
  }
  ...
  static bool isGFX12Plus(StringRef Arch) {
    mlir::triton::amdgpu::TargetFeatures TF(Arch);
    auto family = TF.getISAFamily();
...
public:
  ...
  void runOnLoop(Function &F, Loop &MainLoop, LoopInfo &LI, StringRef Arch) {
    ...
    // Re-glue MMRA release/acquire fences to their barrier (see
    // reglueFencesToBarriers). On by default; opt out with
    // TRITON_LLIR_FENCE_REGLUE=0.
    const char *reglueEnv = getenv("TRITON_LLIR_FENCE_REGLUE");
    bool reglueFences = !reglueEnv || reglueEnv[0] != '0';
    for (BasicBlock *BB : MainLoop.blocks()) {
      LLVM_DEBUG(dbgs() << "BB: " << BB->getName() << "\n");
      analyzeBBMFMA(*BB, BBMFMAMap);
      scheduleBB(*BB, BBMFMAMap, Arch);
      if (reglueFences)
        Utils::reglueFencesToBarriers(*BB);
    }
    ...
    if (BasicBlock *epilogue = Utils::findEpilogueBlock(&MainLoop, LI)) {
      ...
      if (reglueFences)
        Utils::reglueFencesToBarriers(*epilogue);
    }
  }
...
struct LLIRSchedulePass : FunctionPass {
  ...
  std::string Arch;
  // Set to true when the pass actually schedules a kernel (found a main loop).
  // Used by the caller to decide whether LLVM's machine schedulers should be
  // disabled; if the pass bails out, they must stay enabled.
  bool *DidSchedule = nullptr;

  LLIRSchedulePass(StringRef Arch = "", bool *DidSchedule = nullptr)
      : FunctionPass(ID), Arch(Arch.str()), DidSchedule(DidSchedule) {}
...
bool runLLIRSchedulePass(llvm::Function &F, llvm::StringRef arch) {
  bool didSchedule = false;
  llvm::legacy::FunctionPassManager FPM(F.getParent());
  FPM.add(new LLIRSchedulePass(arch, &didSchedule));
