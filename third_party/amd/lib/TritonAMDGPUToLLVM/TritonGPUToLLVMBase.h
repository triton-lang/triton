...
public:
  ...
  //       result = emitBaseIndexForLayout(loc, rewriter, parentLayout,
  //       parentTy,
  //                                       withCTAOffset);
  ...
  //     }
  //     if (withCTAOffset) {
  //       auto CTAOffset = emitCTAOffsetForLayout(loc, rewriter, layout,
  //       shape); assert(CTAOffset.size() == result.size() && "Rank mismatch");
  ...
  //   // MFMA output tile consists of repeated "dot operand B" layout groups
  //   along
  ...
  //       result =
  //           emitIndicesForDistributedLayout(loc, b, mma, type,
  //           withCTAOffset);
  //     } else if (auto mfmaLayout = layout.dyn_cast<MfmaEncodingAttr>()) {
  ...
  //       result =
  //           emitIndicesForDistributedLayout(loc, b, slice, type,
  //           withCTAOffset);
  //     } else {
  //       llvm_unreachable(
...
private:
  ...
  //     auto maxWarps =
  //         ceil<unsigned>(shapePerCTA[k], sizePerThread[k] *
  //         threadsPerWarp[k]);
  ...
  //   for (unsigned k = 0; k < rank; ++k) {
  //     // 1 CTA tile in minimum if shapePerCTA[k] is less than
  //     shapePerCTATile[k] for (unsigned blockOffset = 0; blockOffset <
  //     tilesPerDim[k];
  //          ++blockOffset)
  ...
  //   // TODO: seems like the apttern below to get `rep`/`spw` appears quite
  //   often
  //   // A info
  //   auto aRep = mmaLayout.getMMAv1Rep(0);
  //   auto aSpw = mmaLayout.getMMAv1ShapePerWarp(0);
  ...
  //     // level in the layout definition, and the tiling order of
  //     warpGrp->warp
  ...
  //   } else {
  //     multiDimWarpId = delinearize(rewriter, loc, warpId, _warpsPerCTA,
  //     order);
  //   }
  ...
  //   multiDimBase[1] = add(mul(i32_val(2), urem(laneId, i32_val(4))),
  //   offWarp1); return multiDimBase;
  // }
  ...
  // SmallVector<Value>
  // emitBaseIndexForMfmaLayout(Location loc, ConversionPatternRewriter
  // &rewriter,
