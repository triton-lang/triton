...
struct SqrtOpConversion
  ...
  SmallVector<Value> createDestOps(mlir::math::SqrtOp op, OpAdaptor adaptor,
    ...
    // Only handle f32 and f64 operators here.
    if (bits != 32 && bits != 64)
      return {};

    // The `llvm.amdgcn.sqrt.(f32|f64)` instruction is lowered directly to
    // v_sqrt_f32/v_sqrt_f64, while `llvm.sqrt.(f32|f64)` is lowered to
    // v_rsq_f32/v_rsq_f64 plus some auxiliary post-processes.
    std::string funcName = ftz ? "llvm.amdgcn.sqrt" : "llvm.sqrt";
    funcName += ".f" + std::to_string(bits);
