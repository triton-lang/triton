...
def test_record(method, tmp_path: pathlib.Path):
    ...
    with instrumentation(temp_file):
        ...
        start_clock_val = int.from_bytes(start_tag.numpy().tobytes(), 'little') & 0x7ff << 32 | int.from_bytes(start_clock.numpy().tobytes(), 'little')
        end_clock_val = int.from_bytes(end_tag.numpy().tobytes(), 'little') & 0x7ff << 32 | int.from_bytes(end_clock.numpy().tobytes(), 'little')
