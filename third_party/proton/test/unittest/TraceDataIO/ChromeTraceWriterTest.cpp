...
TEST_F(ChromeTraceWriterTest, EmitsMergedCallPathBars) {
  auto metadata1 = std::make_shared<KernelMetadata>();
  metadata1->kernelName = "kernel0";
  metadata1->scopeName = {{1, "s1"}};
  metadata1->callPathFrames = {CallPathFrame{"a"}, CallPathFrame{"b"}};
  auto result1 = createDefaultResult(1, 1, 1);
  result1->blockTraces[0].blockId = 0;
  result1->blockTraces[0].procId = 0;
  result1->blockTraces[0].initTime = 0;
  result1->blockTraces[0].traces[0].uid = 0;
  result1->blockTraces[0].traces[0].profileEvents[0].first->cycle = 1220000;
  result1->blockTraces[0].traces[0].profileEvents[0].second->cycle = 1620000;
  result1->blockTraces[0].traces[0].profileEvents[0].first->scopeId = 1;
  result1->blockTraces[0].traces[0].profileEvents[0].second->scopeId = 1;

  auto metadata2 = std::make_shared<KernelMetadata>();
  metadata2->kernelName = "kernel1";
  metadata2->scopeName = {{1, "s1"}};
  metadata2->callPathFrames = {CallPathFrame{"a"}, CallPathFrame{"b"}};
  auto result2 = createDefaultResult(1, 1, 1);
  result2->blockTraces[0].blockId = 0;
  result2->blockTraces[0].procId = 0;
  result2->blockTraces[0].initTime = 10000000;
  result2->blockTraces[0].traces[0].uid = 0;
  result2->blockTraces[0].traces[0].profileEvents[0].first->cycle = 1220000;
  result2->blockTraces[0].traces[0].profileEvents[0].second->cycle = 1620000;
  result2->blockTraces[0].traces[0].profileEvents[0].first->scopeId = 1;
  result2->blockTraces[0].traces[0].profileEvents[0].second->scopeId = 1;
  ...
  auto data = readJsonTrace(chromeTracePath);
  std::vector<json> callPathEvents;
  for (const auto &event : data["traceEvents"]) {
    if (event["cat"] == "call_path") {
      callPathEvents.push_back(event);
    }
  }

  ASSERT_EQ(callPathEvents.size(), 2);
  EXPECT_EQ(callPathEvents[0]["name"], "a");
  EXPECT_EQ(callPathEvents[1]["name"], "b");
  EXPECT_DOUBLE_EQ(callPathEvents[0]["ts"], 0.0);
  EXPECT_DOUBLE_EQ(callPathEvents[0]["dur"], 10400.0);
  EXPECT_EQ(callPathEvents[1]["args"]["call_stack"][0], "a");
  EXPECT_EQ(callPathEvents[1]["args"]["call_stack"][1], "b");
}
