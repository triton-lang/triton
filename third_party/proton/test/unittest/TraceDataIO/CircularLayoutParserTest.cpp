...
TEST_F(CircularLayoutParserTest, SingleEvent) {
  testData = {
      ...
      0xef, 0xcd, 0xab, 0x89, // global time 1
      0x67, 0x45, 0x23, 0x01, //
      0x10, 0x32, 0x54, 0x76, // global time 2
      0x98, 0xba, 0xdc, 0xfe, //
      0x08, 0x07, 0x06, 0x05, // global time 3
      0x04, 0x03, 0x02, 0x01, //
      // num events
      0xff, 0x00, 0x00, 0x00,
      // profiled data
  ...
  EXPECT_EQ(result->blockTraces[0].globalTimes[0], 0x0123456789abcdef);
  EXPECT_EQ(result->blockTraces[0].globalTimes[1], 0xfedcba9876543210);
  EXPECT_EQ(result->blockTraces[0].globalTimes[2], 0x0102030405060708);
