...
TEST(DecoderTest, DecodeMetricExtension) {
  ...
  EXPECT_EQ(
      std::get<int64_t>(decodeMetricValue(entry->metricType, entry->rawValue)),
      -7);
}

TEST(DecoderTest, DecodeMetricValues) {
  EXPECT_EQ(std::get<uint64_t>(decodeMetricValue(EntryMetricType::BOOL, 0xff)),
            1);
  EXPECT_EQ(std::get<int64_t>(decodeMetricValue(EntryMetricType::I8, 0xff)),
            -1);
  EXPECT_EQ(std::get<int64_t>(decodeMetricValue(EntryMetricType::I16, 0xffff)),
            -1);
  EXPECT_EQ(
      std::get<int64_t>(decodeMetricValue(EntryMetricType::I32, 0xffffffff)),
      -1);
  EXPECT_EQ(std::get<uint64_t>(decodeMetricValue(EntryMetricType::U8, 0x1ff)),
            0xff);
  EXPECT_EQ(
      std::get<uint64_t>(decodeMetricValue(EntryMetricType::U16, 0x1ffff)),
      0xffff);
  EXPECT_EQ(
      std::get<uint64_t>(decodeMetricValue(EntryMetricType::U32, 0xffffffff)),
      0xffffffff);
  EXPECT_DOUBLE_EQ(
      std::get<double>(decodeMetricValue(EntryMetricType::F16, 0x3e00)), 1.5);
  EXPECT_DOUBLE_EQ(
      std::get<double>(decodeMetricValue(EntryMetricType::F16, 0x0001)),
      0x1p-24);
  EXPECT_DOUBLE_EQ(
      std::get<double>(decodeMetricValue(EntryMetricType::BF16, 0x3fc0)), 1.5);
  EXPECT_DOUBLE_EQ(
      std::get<double>(decodeMetricValue(EntryMetricType::F32, 0x3fc00000)),
      1.5);
