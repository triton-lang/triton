...
void init_triton_proton(py::module_ &m) {
  ...
  m.def("create_proton_record",
        [](TritonOpBuilder &opBuilder, bool isStart,
          ...
          auto nameAttr = mlir::StringAttr::get(opBuilder.getContext(),
                                                llvm::StringRef(name));
          opBuilder.create<proton::RecordOp>(
              isStart, nameAttr, mlir::Value(), mlir::StringAttr(),
  ...
  m.def("create_proton_async_record_start",
        [](TritonOpBuilder &opBuilder,
           const std::string &name) -> mlir::Value {
          auto nameAttr = mlir::StringAttr::get(opBuilder.getContext(), name);
          auto op = opBuilder.create<proton::AsyncRecordOp>(
              mlir::TypeRange{
                  mlir::IntegerType::get(opBuilder.getContext(), 32)},
              true, nameAttr, mlir::Value());
          return op.getResultToken();
        });

  m.def("create_proton_async_record_end",
        [](TritonOpBuilder &opBuilder, mlir::Value token) -> void {
          opBuilder.create<proton::AsyncRecordOp>(
              mlir::TypeRange{}, false, mlir::StringAttr(), token);
        });
