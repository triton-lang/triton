...
Value TargetInfo::realtime(ConversionPatternRewriter &rewriter,
                           Location loc) const {
  auto b = TritonLLVMOpBuilder(loc, rewriter);
  StringRef realtimeIntrinsicName = "llvm.amdgcn.s.memrealtime";
  Value realtimeVal = LLVM::createLLVMIntrinsicCallOp(
                          rewriter, loc, realtimeIntrinsicName, i64_ty, {})
                          .getResult(0);
  ...
  return b.mul(realtimeVal, b.i64_val(10));
}
