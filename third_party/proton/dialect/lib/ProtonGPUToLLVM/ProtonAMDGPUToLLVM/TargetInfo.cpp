...
Value TargetInfo::timestamp(ConversionPatternRewriter &rewriter,
                            Location loc) const {
  auto b = TritonLLVMOpBuilder(loc, rewriter);
  StringRef timestampIntrinsicName = "llvm.amdgcn.s.memrealtime";
  Value timestampVal = LLVM::createLLVMIntrinsicCallOp(
                           rewriter, loc, timestampIntrinsicName, i64_ty, {})
                           .getResult(0);
  ...
  return b.mul(timestampVal, b.i64_val(10));
}
