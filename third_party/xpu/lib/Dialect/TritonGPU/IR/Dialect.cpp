...
unsigned getTotalElemsPerThread(Attribute layout, ArrayRef<int64_t> shape,
                                Type eltTy) {
  if (auto tritonXPUAttr =
          mlir::dyn_cast<triton::xpu::TritonXPU_AttrTrait>(layout)) {
    return tritonXPUAttr.getTotalElemsPerThread(shape, eltTy);
  } else if (auto tritonGPUAttr = mlir::dyn_cast<TritonGPU_AttrTrait>(layout)) {
...
SmallVector<unsigned> getShapePerCTATile(Attribute layout,
  ...
  if (auto clusterLayout =
    ...
    for (unsigned d = 0, n = clusterLayout.getOrder().size(); d < n; ++d)
      shape.push_back(clusterLayout.getSizePerCore()[d] *
                      clusterLayout.getCoresPerGroup()[d] *
                      clusterLayout.getGroupsPerCluster()[d]);
    return shape;
...
SmallVector<unsigned> getOrder(Attribute layout) {
  if (auto clusterLayout = dyn_cast<triton::xpu::ClusterLayoutAttr>(layout)) {
    return SmallVector<unsigned>(clusterLayout.getOrder().begin(),
                                 clusterLayout.getOrder().end());
  } else if (auto blockedLayout = dyn_cast<BlockedEncodingAttr>(layout)) {
...
SmallVector<unsigned> getCTASplitNum(Attribute layout) {
  ...
  if (auto clusterLayout =
          mlir::dyn_cast<triton::xpu::ClusterLayoutAttr>(layout)) {
    return SmallVector<unsigned>(clusterLayout.getSizePerCore().size(), 1);
  } else if (auto distributedLayout =
                 mlir::dyn_cast<DistributedEncodingTrait>(layout)) {
