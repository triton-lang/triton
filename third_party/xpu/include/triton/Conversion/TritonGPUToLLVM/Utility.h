...
emitOffsetForClusterLayout(const triton::xpu::ClusterLayoutAttr &clusterLayout,
  ...
  auto shapePerCTATile = mlir::triton::gpu::getShapePerCTATile(clusterLayout);
  auto shapePerCTA = triton::gpu::getShapePerCTA(clusterLayout, shape);
...
unpackLLElements(Location loc, Value llvmStruct,
  ...
  if (llvmStruct.getType().isIntOrIndexOrFloat() ||
      ...
      isa<VectorType>(llvmStruct.getType()))
    return {llvmStruct};
  ArrayRef<Type> types =
