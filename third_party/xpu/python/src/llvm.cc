...
bool isElfStackSizeOOB(std::string ElfObject) {
      ...
      [](llvm::Module *mod, const llvm::OptimizationLevel &opt,
...

        {
          ...
          if (!LMSizeLimitStr.empty()) {
            ...
            if (LMSizeLimitSRf.getAsInteger(10, LMSizeLimit)) {
              llvm::report_fatal_error(
                  "Invalid value for LLVM_ERROR_LM_SIZE: " + LMSizeLimitSRf);
            }
          }
          llvm::StringMap<llvm::cl::Option *> optMap =
              llvm::cl::getRegisteredOptions();
        ...
        tuningOptions.SimpleLoopUnswitching =
            false; // To Avoid Copying When If Else is in For
        tuningOptions.MemCpyOpt = false; // To Void Selecting Memset Instruction
        //===-----------------------------------------------------------===//

        if (!triple.empty()) {
          mod->setTargetTriple(triple.c_str());
        } else {
          mod->setTargetTriple("xpu3");
        }
