...
def make_algo_decls(name: str, metas: Sequence[KernelLinkerMeta]) -> str:
    return f"""
int {name}(XPUStream stream, {gen_signature_with_full_args(metas[-1])});
void load_{name}();
void unload_{name}();
    """
...
def make_global_decl(meta: KernelLinkerMeta) -> str:
    return f"""
int {meta.orig_kernel_name}_default(XPUStream stream, {gen_signature_with_full_args(meta)});
int {meta.orig_kernel_name}(XPUStream stream, {gen_signature_with_full_args(meta)}, int algo_id);
...
def make_default_algo_kernel(meta: KernelLinkerMeta) -> str:
    src = f"int {meta.orig_kernel_name}_default(XPUStream stream, {gen_signature_with_full_args(meta)}){{\n"
...
def make_kernel_hints_dispatcher(name: str, metas: Sequence[KernelLinkerMeta]) -> str:
    ...
    for meta in sorted(metas, key=lambda m: -m.num_specs):
        src += f"int {meta.orig_kernel_name}_{meta.sig_hash}_{meta.suffix}(XPUStream stream, {gen_signature(meta)});\n"
    src += "\n"

    src += (f"int {name}(XPUStream stream, {gen_signature_with_full_args(metas[-1])}){{")
    src += "\n"
    for meta in sorted(metas, key=lambda m: -m.num_specs):
        cond_fn = (  #
...
def make_kernel_meta_const_dispatcher(meta: KernelLinkerMeta) -> str:
    src = f"int {meta.orig_kernel_name}(XPUStream stream, {gen_signature_with_full_args(meta)}, int algo_id){{\n"
...
def make_func_pointers(names: str, meta: KernelLinkerMeta) -> str:
    ...
    src = f"typedef int (*kernel_func_t)(XPUStream stream, {gen_signature_with_full_args(meta)});\n"
