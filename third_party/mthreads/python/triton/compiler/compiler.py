...
def adaptConfigToTorch_2_2(fn, **kwargs):
    # TODO: Get correct cc here.
    try:
        from torch._dynamo.device_interface import CudaInterface
        device = CudaInterface.device
        cc = CudaInterface.get_compute_capability(device)
    except:
        cc = kwargs["cc"]
    attrs = AttrsDescriptor(
        divisible_by_16=kwargs["configs"][0].divisible_by_16,
        equal_to_1=kwargs["configs"][0].equal_to_1,
    )

    src = ASTSource(fn, kwargs["signature"], kwargs["constants"], attrs)
    target = GPUTarget(kwargs["device_type"], cc, 128
                       # TODO: Warp size is hard-code now.
...
def compile(fn, **kwargs):
    # Adapt Triton3.0.0 to torch2.2.0.
    ...
    for ext, compile_ir in list(stages.items())[first_stage:]:
        ...
        if ext == "mubin":
            metadata_group[ir_filename] = next_module[1]  # Get mubin path.
        else:
...
class CompiledKernel:
    ...
    def __init__(self, src, metadata_group, hash):
        ...
        # Adapt to torch2.2.
        self.num_warps = self.packed_metadata[0]
        self.num_ctas = self.packed_metadata[1]
        self.shared = self.packed_metadata[2]
        self.clusterDims = self.packed_metadata[3:6]

    def _init_handles(self):

        def adaptToTorch_2_2Wrapper(fn):

            def wrapper(grid_0, grid_1, grid_2, num_warps, num_ctas, clusterDimX, clusterDimY, clusterDimZ, shared,
                        stream, cu_function, launch_metadata, launch_enter_hook, launch_exit_hook, *args):

                metadata = (num_warps, num_ctas, shared, clusterDimX, clusterDimY, clusterDimZ)

                return fn(grid_0, grid_1, grid_2, stream, cu_function, metadata, launch_metadata, launch_enter_hook,
        ...
        self.c_wrapper = adaptToTorch_2_2Wrapper(self._run)
        self.run = self._run
