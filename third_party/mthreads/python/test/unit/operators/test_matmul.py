...
def test_op(BLOCK_M, BLOCK_N, BLOCK_K, SPLIT_K, NWARP, NSTAGE, M, N, K, AT, BT, ADTYPE, BDTYPE, INPUT_PRECISION,
            F8_FASTACCUM, ACC_DTYPE, OUTPUT_DTYPE, device):
    torch.manual_seed(0)
    ...
    def init_input(m, n, dtype, acc_dtype):
        if 'float8' in dtype:
            ...
            sign = torch.randint(2, size=(m, n), device=device, dtype=torch.int8) * 128
            val = torch.randint(2**3 - 1, size=(m, n), device=device, dtype=torch.int8) << 7 - ewidth
            return sign | val
        if dtype == "int8":
            return torch.randint(-128, 127, (m, n), device=device, dtype=torch.int8)
        ...
        ret = (2.**exponents).to(getattr(torch, dtype)).to(device)
        return ret
