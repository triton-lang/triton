...
def make_pair(shape, device="musa", alpha=1e-2, beta=0., trans=False, data=None, dtype=torch.float32):
    if data is None:
...
def test_attention_fwd_bwd(
    ...
    qkvs = [
        torch.nn.Parameter(input_scale * torch.randn(qkv_shape), requires_grad=True).to(dtype).musa() for _ in range(3)
    ]
    ...
    attn_mask = 1e6 * (-1 + (attn_mask.reshape((1, 1, n_ctx, n_ctx)).musa()))
    torch_q.retain_grad()
    torch_k.retain_grad()
    torch_v.retain_grad()
