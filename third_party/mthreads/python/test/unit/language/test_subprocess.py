...
def test_assert(func_type: str, device: str):
    ...
    proc = subprocess.Popen([sys.executable, assert_path, func_type, device], stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE, shell=False)
...
def test_assert_nested(caller_type, callee_type, device):
    ...
    proc = subprocess.Popen([sys.executable, assert_path, caller_type, callee_type, device], stdout=subprocess.PIPE,
                            stderr=subprocess.PIPE, shell=False)
    _, errs = proc.communicate()
    errs = errs.splitlines()
    num_errs = 0
