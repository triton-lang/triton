...
static PyObject *py_get_max_threads_per_threadgroup(PyObject *self,
                                                    PyObject *args) {
  if (metal_init() != 0) {
    return PyLong_FromLong(0);
  }
...
static PyObject *py_load_binary(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "ss#i", &name, &binary_data, &binary_len,
                        &shared_memory)) {
    return NULL;
  }
  ...
  id<MTLLibrary> library = [g_device
      newLibraryWithData:dispatch_data_create([data bytes], [data length], NULL,
                                              NULL)
                   error:&error];
...
static PyObject *py_dispatch(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "OiiiiiiiO", &pso_capsule, &grid_x, &grid_y,
...
static PyMethodDef MetalMethods[] = {
    ...
    {"get_max_threads_per_threadgroup", py_get_max_threads_per_threadgroup,
