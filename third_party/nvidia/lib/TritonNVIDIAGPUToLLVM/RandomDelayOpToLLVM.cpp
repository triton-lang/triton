...
namespace {
struct RandomDelayOpConversion
    : public ConvertOpToLLVMPattern<triton::nvidia_gpu::RandomDelayOp> {
  using ConvertOpToLLVMPattern<
      triton::nvidia_gpu::RandomDelayOp>::ConvertOpToLLVMPattern;

  LogicalResult
  matchAndRewrite(triton::nvidia_gpu::RandomDelayOp op, OpAdaptor adaptor,
                  ConversionPatternRewriter &rewriter) const override {
    TritonLLVMOpBuilder llvmBuilder(op.getLoc(), rewriter);
    ...
    if (!murmurhash3FinishFn) {
      ...
      auto hashXorShiftRight = [&](Value inputHash,
                                   int32_t shiftValue) -> Value {
    ...
    rewriter.replaceOpWithNewOp<LLVM::InlineAsmOp>(
        op, TypeRange(), ValueRange{delay},       // operands
...
void mlir::triton::NVIDIA::populateRandomDelayOpToLLVMPatterns(
    LLVMTypeConverter &typeConverter, RewritePatternSet &patterns,
    PatternBenefit benefit) {
  patterns.add<RandomDelayOpConversion>(typeConverter, benefit);
  return;
}
