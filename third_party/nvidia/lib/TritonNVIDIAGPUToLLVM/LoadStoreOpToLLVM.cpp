...
struct AtomicRMWOpConversion
  ...
  matchAndRewrite(triton::AtomicRMWOp op, OpAdaptor adaptor,
    ...
    const bool doLLVMFallback =
        valueElemTy.isBF16() && getNVIDIAComputeCapability(moduleOp) < 90;
    auto llvmAtomicBinOp =
        doLLVMFallback ? matchAtomicOp(atomicRmwAttr) : std::nullopt;
    auto llvmAtomicMemOrdering =
        doLLVMFallback ? getMemoryOrdering(op.getSem()) : std::nullopt;
