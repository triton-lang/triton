...
struct AsyncBulkCopyGlobalToLocalOpConversion
  ...
  LogicalResult matchAndRewrite(ttng::AsyncBulkCopyGlobalToLocalOp op,
                                OpAdaptor adaptor,
                                ConversionPatternRewriter &rewriter) const override {
    if (!op.isSupported(targetInfo.getComputeCapability()))
      return op.emitError("linear bulk copy requires compute capability 9.0 or newer");
    ...
    auto barrier = LLVM::getSharedMemoryObjectFromStruct(
        ...
        getTypeConverter()->convertType(op.getBarrier().getType().getElementType()),
        rewriter);
    ...
    PTXBuilder ptx;
    auto &copy = *ptx.create("cp.async.bulk.shared::cta.global.mbarrier::complete_tx::bytes");
    copy(ptx.newAddrOperand(dst.getShmemAffineBase(loc, rewriter, op.getDst().getType()), "r"),
...
void mlir::triton::NVIDIA::populateLoadStoreOpToLLVMPatterns(
  ...
  patterns.add<AsyncBulkCopyGlobalToLocalOpConversion>(typeConverter, targetInfo,
                                                      benefit);
