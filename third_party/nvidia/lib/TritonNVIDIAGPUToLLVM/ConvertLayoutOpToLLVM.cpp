...
struct ConvertLayoutOpSwizzlingConversion
  ...
  matchAndRewrite(ConvertLayoutOp op, OpAdaptor adaptor,
    ...
    if (cvtNeedsSharedMemory(srcTy, dstTy) &&
        !conversion.isTrivialOver({kBlock})) {
      auto loc = op.getLoc();
  ...
  SmallVector<Value> transferWithinBlockSwizzling(
    ...
    // Checked in the lowering. ldmatrix/stmatrix do not support non-trivial
    // block dimensions We lower these cases in the generic ld/st lowering
    assert(storeCvt.isTrivialOver({kBlock}) && loadCvt.isTrivialOver({kBlock}));
