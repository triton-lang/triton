...
LogicalResult convertDotImpl(const LLVMTypeConverter &typeConverter,
  ...
  // along M.
  SmallVector<unsigned> aOperandShape = {
      mmaSizeM, static_cast<unsigned>(llvm::PowerOf2Ceil(mmaSizeK))};
  ...
  auto buildContinuation =
    ...
    auto loader = DotOpMmaSmemLoader::build(
        loc, rewriter, ty, base, lhs ? aOperandShape : bLoaderShape,
        lhs ? 0 : 1, 5, true);
    if (failed(loader))
      return failure();
    ...
    MemDescOperand a =
        aInTmem
            ...
            : MemDescOperand{
                  ...
                  std::nullopt};
    Value b = op.decompressB
                  ? loadB(k, n)
                  : loadSmem(*bLoader, bNextLoader, bTensorTy,
                             n * bOperandShape[1], op.getKOffset(k),
                             op.getKSize(k), false);
