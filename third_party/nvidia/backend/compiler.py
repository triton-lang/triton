...
class CUDABackend(BaseBackend):
    ...
    def make_llir(self, src, metadata, options, capability):
        ...
        mod = src
        has_gsan_tma = False
        if options.instrumentation_mode == "gsan":

            def check_gsan_tma(operation):
                nonlocal has_gsan_tma
                has_gsan_tma |= operation.get_name() in (
        ...
        # Sanitizer instrumentation creates large functions; aggressive LLVM
        # vector transformations can expand them faster than they simplify.
        if has_gsan_tma:
            optimization_level = llvm.OPTIMIZE_O2
        elif options.instrumentation_mode:
            optimization_level = llvm.OPTIMIZE_O1
        else:
            optimization_level = llvm.OPTIMIZE_O3
        # Work around ptxas rejecting PTX generated by the LLVM SLP vectorizer on sm_80.
        llvm.optimize_module(
            llvm_mod,
            optimization_level,
            disable_slp_vectorizer=capability == 80 or bool(options.instrumentation_mode),
            disable_vector_combine=bool(options.instrumentation_mode),
        )
    ...
    def make_cubin(self, src, metadata, opt, capability):
        ...
        with tempfile.NamedTemporaryFile(delete=False, mode='w', suffix='.ptx') as fsrc, \
            ...
            # Fast compilation can miscompile ConSan barrier instrumentation.
            # Keep its safe optimization level independent of PTX size.
            if not knobs.nvidia.disable_ptxas_opt:
                if "consan" in opt.instrumentation_mode:
                    ptx_extra_options += ["--opt-level", "1"]
                elif any(mode in opt.instrumentation_mode for mode in ["fpsan", "gsan"]):
