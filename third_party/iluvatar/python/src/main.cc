...
using BackendInitFunc = void (*)(pybind11::module &&);

BackendInitFunc load_backend_init_func(const char *backend_name) {
  const std::string func_name = std::string("init_triton_") + backend_name;
  void *symbol = load_backend_symbol(backend_name, func_name.c_str());
  return reinterpret_cast<BackendInitFunc>(symbol);
}
...
#define INIT_BACKEND(name)                                                     \
  do {                                                                         \
    try {                                                                      \
      static auto func = load_backend_init_func(#name);                        \
      func(m.def_submodule(#name));                                            \
    } catch (const std::exception &e) {                                        \
      std::cerr << "Failed to load backend " #name ": " << e.what()            \
                << std::endl;                                                  \
    }                                                                          \
  } while (0);
