...
@functools.lru_cache()
def cuda_home_dirs():
    loc = subprocess.check_output(["whereis", "ixsmi"]).decode().strip().split()
    default_dir = '/usr/local/corex'
    if (len(loc) > 1):
        default_dir = os.path.dirname(os.path.dirname(loc[1]))
...
def include_dirs():
    if is_corex():
        ...
        return include_dir + [cu_include_dir]
    else:
        return include_dir
...
class CudaUtils(object):
    ...
    def __init__(self):
        mod, self.cache_path = compile_module_from_src(
            Path(os.path.join(dirname, "driver.c")).read_text(), "cuda_utils")
...
typedef CUresult (*cuLaunchKernelEx_t)(const CUDA_LAUNCH_PARAMS* config, CUfunction f, void** kernelParams, void** extra);
...
static void _launch(int gridX, int gridY, int gridZ, int num_warps, int num_ctas, int clusterDimX, int clusterDimY, int clusterDimZ, int shared_memory, CUstream stream, CUfunction function{', ' + arg_decls if len(arg_decls) > 0 else ''}) {{
  ...
  if (gridX*gridY*gridZ > 0) {{
    if (num_ctas == 1) {{
      CUDA_CHECK(cuLaunchKernel(function, gridX, gridY, gridZ, {warp_size}*num_warps, 1, 1, shared_memory, stream, params, 0));
    }} else {{
      CUDA_LAUNCH_PARAMS config;
...
class CudaLauncher(object):
    ...
    def __init__(self, src, metadata):
        ...
        mod, _ = compile_module_from_src(src, "__triton_launcher")
