...
Perhaps confusingly, compile-time scalar folding follows Python semantics instead: when all the inputs are scalars, ``int // int`` and ``int % int`` are evaluated with Python floor division (the remainder takes the divisor's sign), so a mixed-sign computation can give a different result depending on whether it was folded or executed at runtime. Floor division on floating-point operands is also accepted when every input is a scalar, even though the same expression on tensors is a semantic error; floating-point division with ``/`` performs regular floating-point division in both cases.
